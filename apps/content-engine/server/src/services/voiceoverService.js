...
export async function generateVoiceover({ scriptId, voiceModel, stability, similarityBoost }) {
  if (!process.env.ELEVENLABS_API_KEY) {
    throw new Error('ELEVENLABS_API_KEY not configured — add it to server/.env')
  }
  ...
  const uploadRes = await fetch(
    `${process.env.SUPABASE_URL}/storage/v1/object/voiceovers/${fileName}`,
    {
      method:  'POST',
      headers: {
  ...
  const fileUrl = `${process.env.SUPABASE_URL}/storage/v1/object/public/voiceovers/${fileName}`
