...
export default function Voiceover() {
  ...
  async function handleGenerate() {
    if (!scriptId) { setError('Selecione um roteiro primeiro'); return }
    setGenerating(true)
    setError(null)
    try {
  ...
  return (
    <div>
      <PageHeader
        title="Geração de Narração"
        description="ElevenLabs TTS — gere áudio a partir dos roteiros"
        action={
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
        <h3 className="font-semibold text-gray-800 mb-4">Configuração de Voz</h3>
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Modelo de Voz</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Similaridade — {similarityBoost}%</label>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-gray-500 border-b border-gray-100">
              <th className="px-6 py-3 font-medium">ID do Roteiro</th>
              <th className="px-6 py-3 font-medium">Voz</th>
          ...
          <tbody>
            {voiceovers.length === 0
              ? <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-400 text-sm">Nenhuma narração ainda — selecione um roteiro e gere acima.</td></tr>
              : voiceovers.map((v) => (
