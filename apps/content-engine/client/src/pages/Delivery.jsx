...
export default function Delivery() {
  ...
  async function handleSend() {
    if (!scriptId || !editorContact) { setError('Roteiro e contato do editor são obrigatórios'); return }
    setSending(true)
    setError(null)
    try {
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Envie roteiros e narrações ao seu editor via WhatsApp"
        action={
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
        <h3 className="font-semibold text-gray-800 mb-4">Nova Entrega</h3>
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        <div className="px-6 py-4 border-b border-gray-100">
          <h3 className="font-semibold text-gray-800">Histórico de Entregas</h3>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-gray-500 border-b border-gray-100">
              <th className="px-6 py-3 font-medium">Roteiro</th>
              <th className="px-6 py-3 font-medium">Editor</th>
              <th className="px-6 py-3 font-medium">Narração</th>
          ...
          <tbody>
            {deliveries.length === 0
              ? <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400 text-sm">Nenhuma entrega ainda.</td></tr>
              : deliveries.map((d) => (
