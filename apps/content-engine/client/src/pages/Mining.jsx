...
// Português = Brasil (ML MLB + Amazon BR). Español = México + Espanha (ML MLM + Amazon MX/ES).
// English = EUA + Canadá (Amazon only).
const REGIONS = [
  { code: 'pt', label: 'Português', langLabel: 'Português', sublabel: 'Brasil',          hasML: true,  mlSite: 'MLB' },
  { code: 'es', label: 'Español',   langLabel: 'Español',   sublabel: 'España & México', hasML: true,  mlSite: 'MLM' },
  { code: 'en', label: 'English',   langLabel: 'English',   sublabel: 'US & Canada',     hasML: false, mlSite: null  },
]
          ...
          <div style={{ padding: '0 20px 16px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {[
                ...
                { icon: Star,    label: 'Avaliação',   value: product.rating > 0 ? `${product.rating}${product.reviewCount > 0 ? ` · ${product.reviewCount.toLocaleString('pt-BR')}` : ''}` : '—',  color: '#fbbf24', bg: 'rgba(251,191,36,0.10)' },
...
export default function Mining() {
  ...
  async function handleRun({ region = 'pt', marketplace, siteFilter, category, sortBy: runSortBy }) {
    ...
    try {
      ...
      // ML proxy: used for Brasil (MLB) and México (MLM). Non-ML regions (English)
      // go straight to the Worker SerpAPI/Creators path via /mining/run.
      if (reg.hasML && marketplace === 'mercadolivre_direct') {
        ...
        const params = new URLSearchParams({ q: category, limit: '20', sort: mlSort, site: reg.mlSite ?? 'MLB' })
