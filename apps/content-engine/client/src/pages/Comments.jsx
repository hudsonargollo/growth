...
export default function Comments() {
  const { data, refetch, loading, error: listError } = useApi('/comments')
  const jobs = data?.jobs ?? []

  const [running, setRunning] = useState(false)
  const [error,   setError]   = useState(null)

  // Surface any error returned inside the 200 payload (from our graceful catch in the worker)
