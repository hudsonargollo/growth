...
export async function runMiningSession(env, { marketplace, category }) {
  ...
  const { error: sErr } = await db.from('mining_sessions').insert({ id: sessionId, marketplace, category, status: 'in_progress' })
  if (sErr) throw new Error(sErr.message)

  // TODO: replace with real MercadoLibre / Amazon PA-API calls
  ...
  const entries = saved.map((p) => ({ id: uid(), productId: p.id, market: marketplace, freshnessScore: 1.0, provenance: `session-${sessionId}` }))
  const { error: eErr } = await db.from('catalog_entries').insert(entries)
  if (eErr) throw new Error(eErr.message)

  await db.from('mining_sessions').update({ status: 'completed', productCount: saved.length, completedAt: new Date().toISOString() }).eq('id', sessionId)
