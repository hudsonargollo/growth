/**
 * Tektone DevTools Extension - Service Worker
 *
...
async function getSettings() {
  return chrome.storage.sync.get({
    ...
    autoScan: 'panel',        // 'panel' = scan when Tektone UI opens, 'devtools' = scan when DevTools opens
  });
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.action === 'findings' && tabId) {
    ...
    chrome.scripting.executeScript({
      ...
      // Detector will post tektone-ready, content script handles the rest
    }).catch((err) => {
      console.warn('[tektone] Fallback injection failed:', err);
    });
    sendResponse({ ok: true });
  }
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  if (port.name.startsWith('tektone-devtools-')) {
    const tabId = parseInt(port.name.replace('tektone-devtools-', ''), 10);
  ...
  if (port.name.startsWith('tektone-panel-')) {
    const tabId = parseInt(port.name.replace('tektone-panel-', ''), 10);
  ...
  // Connecting the sidebar is a strong signal of "user engaged with Tektone"
  // so we trigger a scan if no findings exist yet (matches the panel port behavior).
  if (port.name.startsWith('tektone-sidebar-')) {
    const tabId = parseInt(port.name.replace('tektone-sidebar-', ''), 10);
    if (!panelPorts.has(tabId)) panelPorts.set(tabId, new Set());
    panelPorts.get(tabId).add(port);
