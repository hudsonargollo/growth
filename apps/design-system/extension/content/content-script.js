/**
 * Tektone DevTools Extension - Content Script
 *
 ...
 */
(function () {
  if (window.__TEKTONE_CS_LOADED__) return;
  window.__TEKTONE_CS_LOADED__ = true;
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.action === 'scan') {
      ...
      window.postMessage({ source: 'tektone-command', action: 'toggle-overlays' }, '*');
      sendResponse({ ok: true });
      ...
      window.postMessage({ source: 'tektone-command', action: 'highlight', selector: msg.selector }, '*');
      sendResponse({ ok: true });
    } else if (msg.action === 'unhighlight') {
      window.postMessage({ source: 'tektone-command', action: 'unhighlight' }, '*');
      sendResponse({ ok: true });
    }
    return true;
  });
  ...
  window.addEventListener('message', (e) => {
    ...
    if (e.data.source === 'tektone-overlays-toggled') {
      chrome.runtime.sendMessage({
  ...
  function sendScanCommand() {
    const msg = { source: 'tektone-command', action: 'scan' };
  ...
  function injectAndScan() {
    ...
    script.dataset.tektoneExtension = 'true';
    pendingScan = true;
