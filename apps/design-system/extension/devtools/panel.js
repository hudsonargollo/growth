/**
 * Tektone DevTools Extension - Panel
 *
...
function getPort() {
  if (port) return port;
  port = chrome.runtime.connect({ name: `tektone-panel-${tabId}` });
...
// Maps each anti-pattern to the most relevant Tektone skill(s) for fixing it.
...
async function formatFindingsForCopy(findings) {
  if (!findings.length) return 'Tektone found no anti-patterns on this page.';
  const url = await getInspectedUrl();
  const lines = ['# Tektone findings'];
  ...
  if (skills.length) {
    lines.push(`Suggested Tektone skills to fix: ${skills.join(', ')}`);
    lines.push('');
  }

  lines.push('---');
  lines.push('Detected by [Tektone](https://tektone.design). Skills are suggestions, not required.');
  return lines.join('\n');
}
...
async function formatSingleFindingForCopy(item, finding) {
  ...
  const lines = [`# Tektone: ${finding.name}`];
  if (url) lines.push(`URL: ${url}`);
  ...
  lines.push('');
  lines.push(`Suggested Tektone skill(s) to fix: ${fixSkillFor(finding.type)}`);
  return lines.join('\n');
}
