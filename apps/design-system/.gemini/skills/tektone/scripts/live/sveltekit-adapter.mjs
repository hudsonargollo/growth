...
export const SVELTE_LIVE_ROOT_COMPONENT = 'src/lib/tektone/TektoneLiveRoot.svelte';
export const SVELTE_LAYOUT_MARKER_OPEN = '<!-- tektone-live-svelte-start -->';
export const SVELTE_LAYOUT_MARKER_CLOSE = '<!-- tektone-live-svelte-end -->';
export const SVELTE_ROOT_IMPORT = "import TektoneLiveRoot from '$lib/tektone/TektoneLiveRoot.svelte';";
...
export function buildSvelteLiveRootComponent(port) {
  ...
  const HOST_ID = 'tektone-live-root';

  onMount(() => {
    let host = document.querySelector('tektone-live-root#' + HOST_ID) || document.getElementById(HOST_ID);
    if (!host) {
      host = document.createElement('tektone-live-root');
      host.id = HOST_ID;
      document.body.appendChild(host);
    }

    host.dataset.tektoneLiveAdapter = 'sveltekit';
    ...
    if (!root.querySelector('style[data-tektone-live-reset]')) {
      const reset = document.createElement('style');
      reset.dataset.tektoneLiveReset = 'true';
      reset.textContent = ':host, :host *, * { box-sizing: border-box; }';
      root.appendChild(reset);
    }

    window.__TEKTONE_LIVE_ADAPTER__ = 'sveltekit';
    window.__TEKTONE_LIVE_UI_ROOT__ = root;
    window.__TEKTONE_LIVE_CHROME_MOUNT__ = {
      adapter: 'sveltekit',
      version: 1,
      host,
      root,
    ...
    script.src = LIVE_URL;
    script.async = true;
    script.dataset.tektoneLiveScript = 'true';
    document.head.appendChild(script);

    return () => {
      script.remove();
      if (window.__TEKTONE_LIVE_UI_ROOT__ === root) delete window.__TEKTONE_LIVE_UI_ROOT__;
      if (window.__TEKTONE_LIVE_CHROME_MOUNT__?.root === root) delete window.__TEKTONE_LIVE_CHROME_MOUNT__;
      if (window.__TEKTONE_LIVE_ADAPTER__ === 'sveltekit') delete window.__TEKTONE_LIVE_ADAPTER__;
    };
  });
</script>
`;
