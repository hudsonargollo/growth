/**
 * Tektone Live Variant Mode - Browser Script
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">.
 * The server prepends window.__TEKTONE_TOKEN__ and window.__TEKTONE_PORT__
 * before this code.
 *
...
(function () {
  ...
  if (window.__TEKTONE_LIVE_INIT__) return;
  window.__TEKTONE_LIVE_INIT__ = true;

  const TOKEN = window.__TEKTONE_TOKEN__;
  const PORT = window.__TEKTONE_PORT__;
  if (!TOKEN || !PORT) {
    window.__TEKTONE_LIVE_INIT__ = false; // reset so the real load can init
    return;
  }
  ...
  const PREFIX = 'tektone-live';
  const MANUAL_APPLY_STATE_TTL_MS = 15 * 60 * 1000;
  const sessionState = window.__TEKTONE_LIVE_SESSION__?.createLiveBrowserSessionState({
    prefix: PREFIX,
    storage: localStorage,
    idFactory: () => crypto.randomUUID().replace(/-/g, '').slice(0, 8),
  });
  if (!sessionState) {
    console.error('[tektone] live-browser-session.js was not loaded. Live mode cannot start safely.');
    window.__TEKTONE_LIVE_INIT__ = false;
    return;
  }
  ...
  // window.__TEKTONE_VOCAB__ when it serves /live.js (same injection path as
  ...
  const VOCAB = Array.isArray(window.__TEKTONE_VOCAB__) ? window.__TEKTONE_VOCAB__ : [];
  const ICONS = {};
  ...
  function liveUiRoot() {
    const root = window.__TEKTONE_LIVE_UI_ROOT__;
  ...
  window.__TEKTONE_LIVE_CHROME_CORE__ = {
    version: 1,
    adapter: window.__TEKTONE_LIVE_ADAPTER__ || 'dom',
  ...
  function showHighlight(el) {
    ...
    if (el.hasAttribute?.('data-tektone-insert-placeholder')) return;
    const r = el.getBoundingClientRect();
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.('[data-tektone-placeholder-resize]')?.dataset.tektonePlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [root, ...root.querySelectorAll('[data-tektone-editable], [data-tektone-original-text], [data-tektone-text-wrap]')];
    for (const node of nodes) {
      const runtimeEditable = node.hasAttribute('data-tektone-editable')
        || node.hasAttribute('data-tektone-original-text');
      node.removeAttribute('data-tektone-editable');
      node.removeAttribute('data-tektone-original-text');
      node.removeAttribute('data-tektone-text-wrap');
      if (runtimeEditable) {
  ...
  function applyPlaceholderSizingStyles(placeholder, sizing) {
    placeholder.dataset.tektonePlaceholderWidth = sizing.kind;
  ...
  function materializePlaceholderWidth(placeholder) {
    ...
    const kind = placeholder.dataset.tektonePlaceholderWidth;
    ...
    placeholder.dataset.tektonePlaceholderWidth = 'explicit';
  }
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === 'GENERATING' || state === 'CYCLING')) {
      const wrapper = document.querySelector('[data-tektone-variants="' + currentSessionId + '"]');
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll('[data-tektone-variant]:not([data-tektone-variant="original"])').length;
  ...
  function isInsertGeneratingSession() {
    ...
    const wrapper = document.querySelector('[data-tektone-variants="' + currentSessionId + '"]');
    return !!wrapper && wrapper.dataset.tektoneMode === 'insert';
  }
  ...
  function buildPlaceholderResizeHandles() {
    ...
    for (const spec of specs) {
      ...
      handle.dataset.tektonePlaceholderResize = spec.edge;
  ...
  function createInsertPlaceholder(anchor, position, layoutAxis) {
    ...
    placeholder.setAttribute('data-tektone-insert-placeholder', 'true');
  ...
  function buildConfigureRow() {
    ...
    input.type = 'text';
    input.placeholder = selectedAction === 'tektone' ? 'describe what you want…' : 'refine further (optional)…';
    ...
    if (!uiGetById(PREFIX + '-configure-input-style')) {
      ...
      s.textContent =
        '@keyframes tektone-configure-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
        '#' + PREFIX + '-input::placeholder { color: ' + BP.textDim + '; opacity: 1; }' +
        '#' + PREFIX + '-configure-voice[data-listening="true"] svg { animation: tektone-configure-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  function buildSavingRow() {
    ...
    const spinner = el('div', {
      ...
      animation: 'tektone-spin 0.6s linear infinite',
      flexShrink: '0',
    });
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn('[tektone] Refusing to render empty variant cycling state:', reason);
  ...
  // variant wrapper:
  //
  //   <div data-tektone-variant="1"
  //        data-tektone-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector('[data-tektone-variant="' + visibleVariant + '"]');
  }

  function parseVariantParams(variantEl) {
    // Svelte component variants can't carry a `data-tektone-params` attribute:
    ...
    if (!variantEl) return [];
    const raw = variantEl.getAttribute('data-tektone-params');
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn('[tektone] Invalid data-tektone-params JSON:', err.message);
      return [];
    }
  }
  ...
  function wrapMixedContentTextNodes(rootEl) {
    ...
    if (hasText && hasElement) {
      for (const node of children) {
        if (node.nodeType === 3 && /\S/.test(node.nodeValue || '')) {
          ...
          wrap.dataset.tektoneTextWrap = 'true';
    ...
    for (const child of Array.from(rootEl.children)) {
      if (!child.dataset || !child.dataset.tektoneTextWrap) {
  ...
  function unwrapMixedContentTextNodes(rootEl) {
    if (!rootEl || rootEl.nodeType !== 1) return;
    const wraps = rootEl.querySelectorAll('[data-tektone-text-wrap="true"]');
  ...
  function enableInlineEdit(targetEl) {
    ...
    for (const row of rows) {
      ...
      row.el.dataset.tektoneEditable = 'true';
      row.el.dataset.tektoneOriginalText = row.text;
  ...
  function disableInlineEdit(opts = {}) {
    for (const row of inlineEditRows) {
      ...
      delete row.el.dataset.tektoneEditable;
      delete row.el.dataset.tektoneOriginalText;
  ...
  function restoreInlineEditDrafts() {
    for (const row of inlineEditRows) {
      if (inlineEditDrafts.has(row.el)) {
        row.el.textContent = row.el.dataset.tektoneOriginalText;
      }
    }
  }
  ...
  function documentRefClassSuffix(el) {
    ...
    for (const cls of el.classList) {
      if (!cls || cls.indexOf('tektone-') === 0) continue;
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList ? [...el.classList].filter((cls) => cls.indexOf('tektone-') !== 0) : [],
      originalText,
      newText,
      textContent: (el.textContent || '').slice(0, 500),
  ...
  function nearbyEditableTextsForManualEdit(rows, activeEl, originalText, newText) {
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList ? [...row.el.classList].filter((cls) => cls.indexOf('tektone-') !== 0) : [],
        text,
      });
  ...
  async function applyEditing() {
    ...
    try {
      ...
      console.error('[tektone] manual edit stash failed:', err);
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent = '@keyframes tektone-spin { to { transform: rotate(360deg); } }';
    uiAppendStyle(style);
  }
  ...
  async function fetchPendingCount() {
    try {
      ...
      console.warn('[tektone] failed to fetch pending count:', err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        console.warn('[tektone] some copy edits failed:', result.failed);
        ...
        if (n > 0) {
          ...
          console.warn('[tektone] apply returned no verified edits:', result);
          showToast('No edits applied - see console', 4000);
        }
      }
    } catch (err) {
      console.error('[tektone] commit failed:', err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      console.error('[tektone] discard failed:', err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      console.error('[tektone] repair retry failed:', err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      console.error('[tektone] manual Apply rollback failed:', err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn('[tektone] skipped unsafe copy edit DOM restore for', failures, 'edit(s). Refresh to reset the page DOM.');
    }
    return failures;
  }
  ...
  function mixedTextWrapRestoreHint(el) {
    if (!el || !el.dataset || el.dataset.tektoneTextWrap !== 'true' || !el.parentElement) return null;
  ...
  function directMixedTextRestoreNodes(parent) {
    return Array.from(parent?.childNodes || []).filter((node) => {
      ...
      return node.nodeType === 1
        && node.dataset
        && node.dataset.tektoneTextWrap === 'true'
  ...
  function syncEditBadgeHitProxies() {
    ...
    for (const target of targets) {
      ...
      if (!proxy) {
        ...
        proxy.type = 'button';
        proxy.tabIndex = -1;
        proxy.dataset.tektoneEditBadgeProxy = 'true';
  ...
  function initEditBadge() {
    ...
    if (!uiGetById(PREFIX + '-edit-badge-focus-style')) {
      ...
      s.textContent =
        ...
        '[data-tektone-editable="true"] { outline: none !important; box-shadow: none !important; }' +
        '[data-tektone-editable="true"]:focus { outline: none !important; box-shadow: none !important; }' +
        '[data-tektone-editable="true"]:focus-visible { outline: none !important; box-shadow: none !important; }';
      uiAppendStyle(s);
    }
  }
  ...
  async function showVariantInDOM(sessionId, num) {
    ...
    const wrapper = document.querySelector('[data-tektone-variants="' + sessionId + '"]');
    if (!wrapper) return false;
    for (const child of wrapper.children) {
      const v = child.dataset ? child.dataset.tektoneVariant : null;
      if (!v) continue;
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString('<div id="tektone-anchor">' + originalMarkup + '</div>', 'text/html');
    return doc.getElementById('tektone-anchor')?.firstElementChild || null;
  }
  ...
  function loadSvelteRuntime(runtimeModule) {
    const modulePath = runtimeModule || '/src/lib/tektone/__runtime.js';
  ...
  // componentDir (keyed by variant number), because a `data-tektone-params`
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error('[tektone] Failed to mount Svelte variant ' + variantNum + ' for ' + sessionId + ':', err);
      return false;
    }
  }
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      const existingWrapper = document.querySelector('[data-tektone-variants="' + sessionId + '"]');
      ...
      if (!liveEl?.parentElement) {
        console.warn('[tektone] Could not find original element in live DOM.');
        arrivedVariants = Number(manifest.count) || expectedVariants || 1;
        expectedVariants = arrivedVariants;
      ...
      wrapper.dataset.tektoneVariants = sessionId;
      wrapper.dataset.tektoneVariantCount = String(manifest.count || expectedVariants || 1);
      wrapper.dataset.tektonePreview = 'svelte-component';
      wrapper.style.display = 'contents';

      const mountTarget = document.createElement('div');
      mountTarget.dataset.tektoneComponentMount = sessionId;
      ...
      positionBar();
      saveSession();
      console.log('[tektone] Mounted ' + arrivedVariants + ' Svelte component variants.');
    } catch (err) {
      console.error('[tektone] Failed to mount Svelte component variants:', err);
  ...
  function abortSvelteComponentInjection(sessionId, message) {
    try {
      if (svelteComponentSession?.sessionId === sessionId) {
        ...
        const orphan = document.querySelector('[data-tektone-variants="' + sessionId + '"]');
        if (orphan) orphan.remove();
      }
    } catch (err) {
      console.warn('[tektone] Svelte component abort cleanup failed:', err);
    }
    hideShaderOverlay();
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        const startMark = '<!-- tektone-variants-start ' + sessionId + ' -->';
        const endMark = '<!-- tektone-variants-end ' + sessionId + ' -->';
        ...
        srcWrapper = doc.querySelector('[data-tektone-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.warn('[tektone] Variant wrapper not found in source file.');
          return;
        }
        ...
        if (existingWrapper) {
          ...
          const origContent = srcWrapper.querySelector('[data-tektone-variant="original"] > :first-child');
        ...
        const variants = wrapper.querySelectorAll('[data-tektone-variant]:not([data-tektone-variant="original"])');
        arrivedVariants = variants.length;
        expectedVariants = parseInt(wrapper.dataset.tektoneVariantCount || arrivedVariants);
        if (arrivedVariants <= 0) {
        ...
        positionBar();
        saveSession();
        console.log('[tektone] Injected ' + arrivedVariants + ' variants from source file.');
      })
      .catch(err => {
        console.error('[tektone] Failed to fetch source:', err);
  ...
  function updateSelectedElement() {
    ...
    if (svelteComponentSession?.sessionId === currentSessionId) {
      const anchor = resolveSvelteComponentAnchor();
      if (anchor && !anchor.__tektoneFrozenAnchor) selectedElement = anchor;
      return;
    }
    const wrapper = document.querySelector('[data-tektone-variants="' + currentSessionId + '"]');
    if (!wrapper) return;
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.tektoneVariant || '0', 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-tektone-variant="' + index + '"]');
  ...
  function startScrollLock(sessionId, initialTargetY) {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-tektone-variants="' + sessionId + '"]')) {
          schedule('mutation-in-wrapper');
          return;
        }
        for (const n of m.addedNodes) {
          if (n.nodeType === 1 && (n.matches?.('[data-tektone-variants="' + sessionId + '"]') || n.querySelector?.('[data-tektone-variants="' + sessionId + '"]'))) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-tektone-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.('[data-tektone-variants]')) { dominated = true; break; }
        for (const n of m.addedNodes) {
          if (n.nodeType !== 1) continue;
          // Direct hit: the added node itself is the wrapper or a variant.
          if (n.dataset?.tektoneVariants || n.dataset?.tektoneVariant) {
          ...
          if (n.querySelector?.('[data-tektone-variants],[data-tektone-variant]')) {
            dominated = true; break;
          }
        }
        if (dominated) break;
      ...
      if (selectedElement && !document.body.contains(selectedElement)) {
        const isInsert = wrapper.dataset.tektoneMode === 'insert';
        if (isInsert) {
      ...
      const expected = parseInt(wrapper.dataset.tektoneVariantCount || '0');
      ...
      if (arrivedVariants >= expectedVariants && expectedVariants > 0) {
        ...
        recoveryWaitingForAnchor = false;
        hideShaderOverlay();
        if (wrapper.dataset.tektoneMode === 'insert') finalizeInsertSession();
        updateSelectedElement();
        showOrUpdateCyclingBar();
        disableInlineEdit();
        refreshParamsPanel();
  ...
  function startScrollTracking() {
    function tick() {
      if (state === 'CONFIGURING' || state === 'GENERATING' || state === 'CYCLING') {
        ...
        if (hiTarget && !hiTarget.hasAttribute?.('data-tektone-insert-placeholder')) {
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast('No PRODUCT.md found. Variants will be brand-agnostic. Run /tektone init to generate one.', 7000);
          console.log('[tektone] Live mode connected.');
        ...
        case 'error':
          ...
          if (maybeCompleteSteer(msg)) break;
          console.error('[tektone] Error:', msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log('[tektone] SSE connection lost. Retry ' + sseRetries + '/' + SSE_MAX_RETRIES + '...');
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      console.log('[tektone] Live server unreachable. Cleaning up UI.');
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        console.error('[tektone] Failed to send event:', err);
        throw err;
      }
      console.debug('[tektone] Dropped optional live event:', err);
      return null;
    }
  ...
  function handleKeyDown(e) {
    ...
    if (e.target.isContentEditable && inlineEditRows.some((r) => r.el === e.target)) {
      ...
      e.preventDefault();
      e.stopPropagation();
      const original = e.target.dataset.tektoneOriginalText;
  ...
  function shouldUseAncestorCropShaderProxy(el) {
    ...
    const adapter = String(window.__TEKTONE_LIVE_ADAPTER__ || '').toLowerCase();
    if (adapter === 'svelte' || adapter === 'sveltekit') return true;
    if (currentPreviewMode === 'svelte-component' || svelteComponentSession) return true;
    const wrapper = el?.closest?.('[data-tektone-variants]');
    return wrapper?.dataset?.tektonePreview === 'svelte-component';
  }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn('[tektone] Svelte ancestor crop capture failed, falling back to element capture:', err);
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn('[tektone] capture failed, proceeding without screenshot:', err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn('[tektone] annotation upload failed:', uploadRes.status);
        }
      } catch (err) {
        console.warn('[tektone] annotation upload failed:', err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      console.warn('[tektone] shader setup failed:', err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      console.warn('[tektone] shader bitmap decode failed:', err);
  ...
  async function handleAccept() {
    ...
    const acceptWrapper = document.querySelector('[data-tektone-variants="' + currentSessionId + '"]');
    ...
    const acceptedIsSvelteComponent = svelteComponentSession?.sessionId === acceptedSessionId
      || acceptWrapper?.dataset?.tektonePreview === 'svelte-component';
  ...
  function snapshotAcceptedVariantDom(sessionId, variantId) {
    const wrapper = document.querySelector('[data-tektone-variants="' + sessionId + '"]');
    const accepted = wrapper?.querySelector?.('[data-tektone-variant="' + variantId + '"]');
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return matches.length > 0
      && matches.every((el) => !el.closest('[data-tektone-variants],[data-tektone-variant],[data-tektone-carbonize]'));
  }
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    if (!sessionId) return [];
    return [...new Set([
      ...document.querySelectorAll('[data-tektone-variants="' + sessionId + '"]'),
      ...document.querySelectorAll('[data-tektone-carbonize="' + sessionId + '"]'),
    ])];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    if (acceptedDomAlreadyClean(pending)) return;
    if (pending?.id && document.querySelector('[data-tektone-variants="' + pending.id + '"]')) return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    const style = wrapper.querySelector('style[data-tektone-css]');
    if (style && !document.querySelector('style[data-tektone-accepted-css="' + sessionId + '"]')) {
      const promotedStyle = style.cloneNode(true);
      promotedStyle.setAttribute('data-tektone-accepted-css', sessionId);
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    const wrapper = document.querySelector('[data-tektone-variants]');
    if (wrapper && wrapper.dataset.tektonePreview !== 'svelte-component') return false;
  ...
  function cleanup() {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      const wrapper = document.querySelector('[data-tektone-variants="' + cleanupSessionId + '"]');
      if (wrapper) wrapper.style.display = 'none';
      setTimeout(function() {
        if (!cleanupSessionId) return;
        const lateWrapper = document.querySelector('[data-tektone-variants="' + cleanupSessionId + '"]');
        if (!lateWrapper) return;
        const orig = lateWrapper.querySelector('[data-tektone-variant="original"]');
        if (orig) {
  ...
  // If a [data-tektone-variants] wrapper exists in the DOM, the agent wrote
  ...
  function resumeSession() {
    ...
    const sessionId = wrapper.dataset.tektoneVariants;
    ...
    // wrapper holds a single mount target, not [data-tektone-variant] nodes,
    ...
    if (wrapper.dataset.tektonePreview === 'svelte-component'
    ...
    if (wrapper.dataset.tektonePreview === 'svelte-component') {
      ...
      currentSessionId = sessionId;
      expectedVariants = Number(wrapper.dataset.tektoneVariantCount)
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.tektoneVariantCount || '0');
    const variants = wrapper.querySelectorAll('[data-tektone-variant]:not([data-tektone-variant="original"])');
    arrivedVariants = variants.length;
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            console.warn('[tektone] shader resume failed:', err);
          }
        })();
      }
    }
  ...
  const PICK_PREFS_KEY = 'tektone-live-pick';
  const INTERACTION_PREFS_KEY = 'tektone-live-interaction';
  ...
  function detectPageTheme() {
    try {
      // Dev override: set localStorage 'tektone-dev-theme' to 'light' or
      // 'dark' to preview the opposite palette without actually changing the
      // page bg. Used for screenshots and theme QA.
      const override = localStorage.getItem('tektone-dev-theme');
  ...
  function attachSteerFocusGuard() {
    if (window.__TEKTONE_STEER_FOCUS_GUARD__) return;
    window.__TEKTONE_STEER_FOCUS_GUARD__ = true;
  ...
  function steerFocusDebugEnabled() {
    try { return localStorage.getItem('tektone-steer-debug') === '1'; } catch { return false; }
  }

  function steerFocusLog(reason, extra) {
    if (!steerFocusDebugEnabled()) return;
    console.log('[tektone.steer]', reason, {
      state,
      pickActive,
  ...
  function attachSteerFocusDebug() {
    if (!steerFocusDebugEnabled()) return;
    if (window.__TEKTONE_STEER_FOCUS_DEBUG__) return;
    window.__TEKTONE_STEER_FOCUS_DEBUG__ = true;
  ...
  function buildSteerProcessingDots() {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(el('span', {
        ...
        animation: 'tektone-steer-dot 1.05s ease-in-out ' + (i * 0.14) + 's infinite',
      }));
    }
    return wrap;
  }
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      console.warn('[tektone.voice] recognition error:', code);
    ...
    try {
      ...
      console.warn('[tektone.voice] start failed:', err);
      stopVoice({
        suppressSubmit: true,
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + '-page-chat-style')) {
      ...
      s.textContent =
        '@keyframes tektone-steer-dot { 0%, 70%, 100% { opacity: 0.28; transform: scale(0.82); } 35% { opacity: 1; transform: scale(1); } }' +
        '@keyframes tektone-steer-processing { 0%, 100% { border-color: oklch(70% 0.12 188 / 0.28); box-shadow: 0 0 0 0 oklch(70% 0.12 188 / 0); } 50% { border-color: oklch(82% 0.07 188 / 0.55); box-shadow: 0 0 14px oklch(70% 0.12 188 / 0.18); } }' +
        '@keyframes tektone-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
        '#' + PREFIX + '-page-chat[data-processing="true"] { animation: tektone-steer-processing 1.6s ease-in-out infinite; }' +
        ...
        '#' + PREFIX + '-page-chat-voice[data-listening="true"] svg { animation: tektone-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  // Tektone mark - same paths as site/components/Header.astro + favicon.svg.
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute('aria-label', connected
      ? 'Tektone live mode'
      : 'Tektone live mode - agent not polling');
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + '-bar-focus-style')) {
      ...
      s.textContent =
        ...
        '}' +
        '@keyframes tektone-agent-dot { 0%, 100% { opacity: 0.45; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1); } }' +
        '#' + PREFIX + '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: tektone-agent-dot 1.4s ease-in-out infinite; }' +
    ...
    // Brand mark - kinpaku Tektone icon (site header / favicon paths).
    const brand = el('span', {
      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    ...
    brand.setAttribute('aria-label', 'Tektone live mode - agent not polling');
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll('.tektone-overlay').forEach(o => {
      o.style.pointerEvents = (pickActive || insertActive) ? 'none' : '';
    });
    syncPageInteractionCursor();
  }

  let detectReady = false; // true once detect script posts 'tektone-ready'
  ...
  function toggleDetect() {
    ...
    if (detectActive) {
      ...
      window.postMessage({ source: 'tektone-command', action: 'remove' }, '*');
  ...
  function loadDetectScript() {
    ...
    s.dataset.tektoneExtension = 'true';
  ...
  function onDetectMessage(e) {
    ...
    if (e.data.source === 'tektone-ready') {
      detectReady = true;
    ...
    if (e.data.source === 'tektone-results') {
  ...
  function teardown() {
    ...
    state = 'IDLE';
    window.__TEKTONE_LIVE_INIT__ = false;
    console.log('[tektone] Live mode exited.');
  }

  //
  // Design System Panel - visualizes the project's .tektone/design.json sidecar
  //

  const DESIGN_PREFS_KEY = 'tektone-live-design-panel';
  ...
  let designState = {
    ...
    sidecar: null,          // .tektone/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  // Neutral panel palette - deliberately NOT Tektone-branded. The panel is
  // a viewer of the project's design system, not an Tektone surface.
  const DP = {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>/tektone document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .tektone/design.json.</strong> Run <code>/tektone document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>/tektone document</code> also generates a <code>.tektone/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function cssSafe(v) {
    ...
    // .tektone/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log('[tektone] Live variant mode ready. Hover over elements to pick one.');
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log('[tektone] Resumed deferred session ' + currentSessionId + ' (post-hydration).');
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log('[tektone] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
