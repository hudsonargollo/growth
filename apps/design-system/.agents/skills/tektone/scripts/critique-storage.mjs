 ...
 *
 * Each run of /tektone critique writes a per-target snapshot to
 *   .tektone/critique/<timestamp>__<slug>.md
 * with a small YAML frontmatter carrying the score + P0/P1 counts.
 *
 * /tektone polish reads the latest matching snapshot at start as its
...
import { getCritiqueDir } from './lib/tektone-paths.mjs';
...
export function writeSnapshot({ slug, meta, body, cwd = process.cwd(), now = new Date() }) {
  ...
  // TEKTONE_CRITIQUE_META env var) could clobber them, leaving the
...
function main(argv) {
  ...
  switch (cmd) {
    ...
    case 'write': {
      ...
      let meta = {};
      const metaArg = process.env.TEKTONE_CRITIQUE_META;
      if (metaArg) {
