 ...
 *
 * Usage:
 *   npx tektone wrap --id SESSION_ID --count N --query "hero-combined-left" [--file path]
 *
...
export async function wrapCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: tektone wrap [options]
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx ? [
    indent + '<div data-tektone-variants="' + id + '" data-tektone-variant-count="' + count + '" ' + styleContents + '>',
    indent + '  ' + commentSyntax.open + ' tektone-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' Original ' + commentSyntax.close,
    indent + '  <div data-tektone-variant="original">',
    reindentOriginal('    '),
    indent + '  </div>',
    indent + '  ' + commentSyntax.open + ' Variants: insert below this line ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' tektone-variants-end ' + id + ' ' + commentSyntax.close,
    indent + '</div>',
  ] : [
    indent + commentSyntax.open + ' tektone-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '<div data-tektone-variants="' + id + '" data-tektone-variant-count="' + count + '" ' + styleContents + '>',
...
function detectStyleMode(filePath) {
  ...
  if (ext === '.astro') {
    return {
      ...
      styleTag: '<style is:inline data-tektone-css="SESSION_ID">',
    };
  }
  return {
    mode: 'scoped',
    styleTag: '<style data-tektone-css="SESSION_ID">',
  };
}

function buildCssSelectorPrefixExamples(styleMode, count) {
  if (styleMode !== 'astro-global-prefixed') return [];
  return Array.from({ length: count }, (_, i) => `[data-tektone-variant="${i + 1}"]`);
}
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === 'astro-global-prefixed') {
    return {
      ...
      rulePattern: '[data-tektone-variant="N"] > .variant-class { ... }',
      selectorExamples: variantNumbers.map((n) => `[data-tektone-variant="${n}"] > .variant-class`),
      requirements: [
        ...
        'Prefix every preview selector with the matching [data-tektone-variant="N"] selector.',
  ...
  return {
    ...
    styleTag: styleMode.styleTag,
    strategy: 'scope-rule',
    rulePattern: '@scope ([data-tektone-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map((n) => `@scope ([data-tektone-variant="${n}"]) { :scope > .variant-class { ... } }`),
    requirements: [
      'Use @scope blocks keyed to each [data-tektone-variant="N"] wrapper.',
    ...
    forbidden: [
      'Do not use global [data-tektone-variant="N"] selector prefixes for this styleMode.',
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes('data-tektone-variant')) continue;
