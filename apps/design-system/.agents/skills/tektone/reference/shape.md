...
**Output**: A design brief that can be handed off to $tektone craft, or directly to $tektone for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Once confirmed, the brief is complete. The user can now hand it to $tektone, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use $tektone craft instead, which runs this command internally.)
