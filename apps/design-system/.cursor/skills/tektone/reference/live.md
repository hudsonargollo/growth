...
```bash
node .cursor/skills/tektone/scripts/live.mjs
```
...
```
LOOP:
  node .cursor/skills/tektone/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node .cursor/skills/tektone/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node .cursor/skills/tektone/scripts/live-status.mjs
node .cursor/skills/tektone/scripts/live-resume.mjs --id SESSION_ID
node .cursor/skills/tektone/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .cursor/skills/tektone/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node .cursor/skills/tektone/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node .cursor/skills/tektone/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .cursor/skills/tektone/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .cursor/skills/tektone/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node .cursor/skills/tektone/scripts/live-server.mjs stop
```
...
```bash
node .cursor/skills/tektone/scripts/detect-csp.mjs
```
