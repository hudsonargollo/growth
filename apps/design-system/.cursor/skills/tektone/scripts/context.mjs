 ...
 *   3. $TEKTONE_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
// can offer `npx tektone skills update`. Everything here is best-effort and
// silent on failure: a network problem, sandbox, or missing cache must never
// block context output or print an error.

const UPDATE_HOST = (process.env.TEKTONE_UPDATE_HOST || 'https://tektone.design').replace(/\/$/, '');
const UPDATE_CACHE_PATH =
  process.env.TEKTONE_UPDATE_CACHE || path.join(os.homedir(), '.tektone', 'update-check.json');
...
export function resolveContextDir(cwd = process.cwd()) {
  ...
  const envDir = process.env.TEKTONE_CONTEXT_DIR;
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer Tektone skill is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Before continuing, ask the user once: "A newer Tektone (v${latestVersion}) is available. ` +
    `Update now? It runs \`npx tektone skills update\`." ` +
    `If they agree, run \`npx tektone skills update\` (the update applies to the next session, not this one). ` +
 ...
 * the agent never nags. Opt out entirely with TEKTONE_NO_UPDATE_CHECK=1.
 */
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.TEKTONE_NO_UPDATE_CHECK) return null;
