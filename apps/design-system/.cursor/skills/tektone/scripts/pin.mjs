 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /tektone audit.
...
const PIN_MARKER = '<!-- tektone-pinned-skill -->';
...
/**
 * Find harness skill directories that have an tektone skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have tektone installed
    const tektoneDir = join(skillsDir, 'tektone');
    if (existsSync(tektoneDir) || existsSync(join(skillsDir, 'i-tektone'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /tektone ${command}.`;
...
This is a pinned shortcut for \`{{command_prefix}}tektone ${command}\`.

Invoke {{command_prefix}}tektone ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with tektone installed found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /tektone ${command} to access it.`);
  } else {
