...
async function runVisualContrastFallback(page, serializedGroups, options, profile, target) {
  ...
  if (options?.visualContrastBrowser !== false) {
    const browserFindings = await profileFindingsAsync(profile, {
      ...
      browserAnalyses = await page.evaluate(async ({ maxCandidates, scrollOffscreen }) => {
        if (typeof window.tektoneAnalyzeVisualContrast !== 'function') return [];
        return window.tektoneAnalyzeVisualContrast({ maxCandidates, scrollOffscreen });
  ...
  if (candidates.length === 0) {
    candidates = await profileStepAsync(profile, {
      ...
      if (typeof window.tektoneCollectVisualContrastCandidates !== 'function') return [];
      return window.tektoneCollectVisualContrastCandidates({ maxCandidates });
    }, { maxCandidates }));
  }
  ...
  try {
    ...
    results = await profileFindingsAsync(profile, {
      ...
      serializedGroups = await page.evaluate(() => {
        if (!window.tektoneDetect) return [];
        return window.tektoneDetect({ decorate: false, serialize: true });
      });
