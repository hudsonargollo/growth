 ...
 *
 *  'cli':     static analysis or jsdom (works with `npx tektone detect`
...
export const LAYER_DESCRIPTIONS = {
  cli: 'Deterministic. Runs from `npx tektone detect` on files, no browser required.',
  browser: 'Deterministic, but needs real browser layout. Runs via the browser extension or Puppeteer, not the plain CLI.',
  llm: 'Not caught by any deterministic detector. Flagged by /tektone critique during its LLM design review.',
};
...
/**
 * Anti-patterns that live in the /tektone skill's DON'T list but
