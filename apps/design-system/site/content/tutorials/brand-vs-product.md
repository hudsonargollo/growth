...
tagline: "The setup choice that keeps Tektone from applying the wrong design defaults."
order: 3
description: "Understand the brand/product choice in /tektone init: marketing surfaces where the impression is the product, and app surfaces where design helps users finish a task."
---

## The short version

During `/tektone init`, the first important choice is this:
...
When `PRODUCT.md` stores the right choice, every command adjusts. `/tektone typeset` will not push editorial display type into a dense admin screen. `/tektone colorize` will not make a campaign page timid because product UIs usually need restraint.
...
```
/tektone polish the marketing homepage as a brand surface
```

```
/tektone audit the billing settings as a product surface
```
...
- **Skipping the choice.** Tektone can still work, but it will ask more questions and lean on safer defaults.
