...
tagline: "Use /tektone critique plus the browser overlay to review a live page with ground truth."
order: 4
...
- Tektone installed in your project (see [getting started](/tutorials/getting-started) if you have not).
...
## Step 1. Run /tektone critique

From your harness, run:

```
/tektone critique the pricing page at localhost:3000/pricing
```
...
The first assessment reads your source code and, if browser automation is available, opens the live page in a new tab. It walks the full tektone skill DO/DON'T catalog and scores the page against Nielsen's 10 heuristics, the 8-item cognitive load checklist, and the brand fit from your `PRODUCT.md`.

It labels the tab it opens with `[LLM]` in the title so you can tell which one is which.

### What the automated detector does

The second assessment runs `npx tektone detect` against the page. This is deterministic: specific pattern checks that fire or do not fire. Gradient text, purple palettes, side-tab borders, nested cards, line length problems, low contrast, tiny body text, and the rest. The [full catalog](/slop#catalog) lists every rule and which layer (CLI, browser, or LLM-only) catches it.
...
Tektone ships with a visual mode that highlights every detected anti-pattern directly on the page. Here is what it looks like running on a deliberately-bad synthwave landing page:
...
<div class="tutorial-embed">
  ...
  <iframe src="/antipattern-examples/visual-mode-demo.html" class="tutorial-embed-iframe" loading="lazy" title="Tektone visual overlay running on a demo page"></iframe>
</div>
...
1. **[Chrome extension](https://chromewebstore.google.com/detail/tektone/bdkgmiklpdmaojlpflclinlofgjfpabf)**: one-click activation on any page. Click the Tektone icon in the toolbar and every anti-pattern gets highlighted instantly.
2. **Inside `/tektone critique`**: the skill opens a browser tab labeled `[Human]` with the detector active during the browser portion of the assessment. You do not need to do anything extra.

For this tutorial, the easiest option is the Chrome extension. Install it, navigate to your pricing page, and click the Tektone icon. You will see the overlay appear immediately on the live page.

## Step 3. Merge the two assessments

Back in your harness, `/tektone critique` has finished and produced a combined report. It looks something like:
...
After you have worked through the priority list, run `/tektone critique` again. The goal is a clean AI slop verdict and at least a 3.5 average on the heuristics. Cognitive load should be below 2 failures.
...
- `/tektone audit the same page` to catch the implementation issues critique does not cover (accessibility, performance, theming).
- `/tektone polish` if the critique report is clean and you want the last-mile refinement pass.
- `/tektone distill` if critique flagged "too busy" or "cognitive load". Distill removes what should not be there.
...
- **The overlay breaks the page layout**. Rare, but some CSS can interact with the injected overlay styles. Use the [Chrome extension](https://chromewebstore.google.com/detail/tektone/bdkgmiklpdmaojlpflclinlofgjfpabf) for the most reliable experience, or run `npx tektone detect` from the CLI and apply findings manually.
