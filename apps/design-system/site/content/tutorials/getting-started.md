...
order: 1
description: "Install Tektone, run /tektone init once to establish project context, and run /tektone polish on something that already exists. The fastest path to seeing what Tektone changes about AI-generated design."
---

## What you'll build

You will end this tutorial with Tektone installed, project context saved, and one existing page improved with a polish pass. Total time: about ten minutes.
...
Tektone installs as a single agent skill called `tektone`. You access all 23 commands through it:

```
/tektone <command> <target>
```

For example: `/tektone polish the pricing page`, or `/tektone audit the checkout`. Type `/tektone` alone to see the full list.

If you use a command often, pin it with `/tektone pin <command>` to create a standalone shortcut (for example, `/tektone pin audit` gives you `/audit` directly).
...
/tektone init
/tektone polish the page you care about
```
...
This auto-detects your AI coding tool and writes the right skill files for it (for example, `.claude/skills/` or `.cursor/skills/`). It works with Cursor, Claude Code, GitHub Copilot, Gemini CLI, Codex CLI, and every other major harness. Reload your tool and type `/`. You should see `/tektone` in the autocomplete. Type it and the argument hint will show the available commands.

Prefer a different setup? Claude Code users can install the plugin with `/plugin marketplace add pbakaus/tektone`, and the general-purpose `npx skills add pbakaus/tektone` still works (though it installs one shared build for all harnesses rather than the one compiled for yours).

When a new version ships later, run `npx tektone skills update` from the same project root. `npx tektone skills check` tells you first whether you are behind, and plugin users update from the `/plugin` menu instead.

## Step 2. Set up Tektone for your project

This is the most important step. Design without context produces generic output. The `/tektone init` command runs a short setup interview and writes `PRODUCT.md` at the root of your project.
...
The docs call this choice **register**. It changes the defaults Tektone uses for type, color, density, and motion. See [Brand vs product](/tutorials/brand-vs-product) for examples. Init forms a hypothesis from your codebase and asks you to confirm it.
...
At the end of `/tektone init`, the skill offers to run `/tektone document` for you. Say yes. It looks at your existing colors, type, components, and tokens, then writes `DESIGN.md` in the [Google Stitch DESIGN.md format](https://stitch.withgoogle.com/docs/design-md/format/).
...
Review the diff. If something does not feel right, ask the model to explain the change. If it still does not feel right, revert it. Tektone is opinionated but not infallible.
...
- `/tektone critique the landing page` runs a full design review with scoring, persona tests, and automated detection. It is the best way to find what to fix next.
- `/tektone audit the checkout` runs accessibility, performance, theming, responsive, and anti-pattern checks against the implementation. Useful before shipping.
- `/tektone craft a pricing page for enterprise customers` runs the full shape-then-build flow on a brand new feature.
- **Pin your favorites.** If you reach for one command constantly, `/tektone pin audit` makes `/audit` work as a standalone shortcut without reversing the consolidation.
- `/tektone redo this hero section` works too. Any description after `/tektone` applies the design principles to the task.

## Common issues

- **The skill says "no design context found"**. You skipped step 2. Run `/tektone init` first.
