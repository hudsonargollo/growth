...
<div class="docs-viz-hero">
  <div class="docs-viz-file">
    <div class="docs-viz-file-header">
      ...
      <span class="docs-viz-file-status">Output of /tektone shape</span>
    </div>
    ...
    <div class="docs-viz-file-footer">Hand it to <code>/tektone</code>, <code>/tektone craft</code>, or any implementation flow.</div>
  </div>
...
Most AI-generated UIs fail not because of bad code, but because of skipped thinking. The model jumps to "here is a card grid" without asking "what is the user trying to accomplish". `/tektone shape` inverts that order.
...
You answer naturally. The skill asks follow-ups, not a form. At the end it produces a design brief: a structured artifact you can hand to `/tektone` or any other implementation skill.

Note: if you want the full flow (discovery interview, then straight into building), use `/tektone craft` instead. It runs `/tektone shape` internally, then continues into implementation with visual iteration. `/tektone shape` standalone is for when you want just the brief, so you can take it to whatever implementation approach you prefer.

## Try it

```
/tektone shape a daily digest email preferences page
```
...
From there you can hand the brief to `/tektone`, `/tektone polish`, or any other skill. Or just use it as a reference while you build by hand.
