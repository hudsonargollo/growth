...
For projects with no code yet (fresh `init` run, nothing built), there is a seed mode: `/tektone document --seed` asks five quick strategic questions (color strategy, type direction, motion style, references, anti-references) and writes a scaffold. Re-run in scan mode once there is code.
