...
<div class="docs-viz-hero">
  <div class="docs-viz-report">
    <div class="docs-viz-report-head">
      <div>
        <div class="docs-viz-report-title">/tektone audit the checkout flow</div>
  ...
  <p class="docs-viz-caption">Five dimensions scored 0 to 4, each finding tagged P0 (blocks release) to P3 (polish). Audit documents; it doesn't fix. Route the findings into <code>/tektone harden</code>, <code>/tektone polish</code>, or <code>/tektone optimize</code>.</p>
</div>

## When to use it

`/tektone audit` is the technical counterpart to `/tektone critique`. Where `/tektone critique` asks "does this feel right", `/tektone audit` asks "does this hold up". It runs accessibility, performance, theming, responsive design, and anti-pattern checks against the implementation, scores each dimension 0 to 4, and produces a plan with P0 to P3 severity ratings.
...
Audit does not fix anything. It documents. Route the findings to `/tektone polish`, `/tektone harden`, or `/tektone optimize` depending on the category.

## Try it

```
/tektone audit the checkout flow
```
...
Hand the P0s to `/tektone harden`, the theming and typography P1s to `/tektone typeset` and `/tektone polish`, the rest to `/tektone polish`.

## Pitfalls

- **Confusing it with `/tektone critique`.** Audit is implementation quality. Critique is design quality. Run both for a full picture.
