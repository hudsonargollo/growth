...
```
/tektone colorize the dashboard
```
...
- **Expecting it to fix the AI color palette problem.** If your design already has purple gradients and cyan neon, you need `/tektone quieter` first, then colorize can rebuild.
- **Using it on already-colorful interfaces.** That is a `/tektone quieter` job. Colorize adds, it does not subtract.
