...
// After the v3.0 consolidation, all commands except the root "tektone" are
// sub-commands of /tektone. The renderer handles the display label directly
// (bare name for sub-commands, "/tektone" for the root). This map is kept
...
export class PeriodicTable {
	...
	showTooltip(el, cmd) {
		...
		// because they're names, not invocations — the invocation is /tektone <name>.
		let relParts = [];
	...
	createElement(cmd, category) {
		...
		const invocation = cmd === 'tektone'
			? '/tektone'
			: cmd.startsWith('tektone ')
				? `/${cmd}`
				: `/tektone ${cmd}`;
