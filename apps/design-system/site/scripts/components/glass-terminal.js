...
const PALETTE_EXCLUDED = new Set(['tektone', 'init', 'extract', 'document', 'live']);
...
function renderDesktopLayout(container, commands) {
    ...
    // too (when they were rendered as 'tektone craft' etc.) but are now
    // first-class sub-commands that should appear in the gallery.
    const deprecated = new Set(['teach-tektone', 'frontend-design', 'arrange', 'normalize', 'onboard', 'tektone craft', 'tektone teach', 'tektone extract']);
...
function renderSpread(cmd, index, isActive) {
    ...
    // The root skill is rendered as tektone; sub-commands are rendered as
    // /tektone on a smaller line above the command name, so the command name
    // stays the visual anchor at full display size.
    const isRoot = cmd.id === 'tektone';
    const nameHTML = isRoot
        ? 'tektone'
        : `<span class="spread-namespace"><span class="spread-slash">/</span>tektone</span>${cmd.id}`;
...
function renderMobileLayout(container, commands) {
    ...
    // Carousel pills show bare command names for sub-commands, and /tektone
    // for the root entry.
    const carouselHTML = commands.map((cmd, i) => `
        <button class="mobile-cmd-pill${i === 0 ? ' active' : ''}" data-id="${cmd.id}">
            ${cmd.id === 'tektone' ? '/tektone' : cmd.id}
        </button>
    `).join('');
    ...
    const infoCardsHTML = commands.map((cmd, i) => {
        ...
        // because the invocation is /tektone <name>, not /<name>.
        if (relationship) {
            if (relationship.pairs) {
        ...
        const cardName = cmd.id === 'tektone'
            ? '/tektone'
            : `<span class="mobile-cmd-namespace">/tektone</span> ${cmd.id}`;
