...
The `<post-update-cleanup>` block at the top of `.claude/skills/tektone/SKILL.md` runs `scripts/cleanup-deprecated.mjs` and then self-deletes. **Do not run it in this repo, and do not delete the block.** This repo is the source of truth for the Tektone skill; the cleanup script scans for "leftover" skill files to remove and its judgment doesn't fit a repo where the skill lives in `skill/`. The block is also regenerated from `skill/SKILL.src.md` whenever `bun run build:release` syncs tracked harness output, so any deletion churns back on the next sync. Skip the step silently and proceed.
...
There is **one** user-invocable skill, `tektone`, with **23 commands** underneath it. Users type `/tektone polish`, `/tektone audit`, etc. The skill is defined in `skill/`:
...
- `scripts/pin.mjs` — creates/removes lightweight redirect shims so users can have `/audit` as a standalone shortcut that delegates to `/tektone audit`.
...
PRODUCT.md at the project root carries a `## Register` section with a bare value (`brand` or `product`). `/tektone init` asks about register first because it shapes every downstream answer.
...
The build system compiles the tektone skill from `skill/` to provider-specific formats in `dist/`. The default build is source-first and does not sync tracked root harness folders; the release build performs the tracked distribution sync:
...
- `{{available_commands}}` — auto-populated list of commands (from `TEKTONE_SUB_COMMANDS` in `scripts/lib/utils.js`)
...
TEKTONE_E2E_ONLY=vite8-react-modal bun run test:live-e2e # scope to one fixture
TEKTONE_E2E_DEBUG=1 bun run test:live-e2e                # dump page DOM + dev-server tail on failure
```
...
**LLM agent (opt-in)**: set `TEKTONE_E2E_AGENT=llm` to swap the fake agent for `tests/live-e2e/agents/llm-agent.mjs`, which calls Claude (default Haiku 4.5) via `@anthropic-ai/sdk`. Requires `ANTHROPIC_API_KEY` in env; the test runner skips with a clear message when it's unset. Override the model with `TEKTONE_E2E_LLM_MODEL=claude-sonnet-4-6` if Haiku produces unreliable JSON. Caching is on — live.md is the cacheable prefix, and after the first call subsequent fixtures pay only the cache-read rate. Pass rate on a typical sweep is 18/19; the modal fixture's intrinsic state-loss flake is amplified by LLM latency and may need a re-run. **This path hits the API and costs money** — keep it out of CI unless you really want it there.
...
TEKTONE_SKILL_BEHAVIOR_MODELS=gemini-3.1-flash-lite bun run test:skill-behavior   # scope to one provider
TEKTONE_SKILL_BEHAVIOR_VERBOSE=1 bun run test:skill-behavior          # dump per-scenario trace JSON to stderr (use when iterating)
```
...
**Auth** lives in repo-root `.env` (copied from `~/code/tektone-evals/.env`, gitignored). Providers skip cleanly when their key is unset; they don't fail.
...
6. `/tektone polish` → loads `reference/polish.md`
7. `/tektone audit` → loads `reference/audit.md`
8. existing SvelteKit project → agent reads at least one project code file
9. `context.mjs` emits `UPDATE_AVAILABLE` (seeded newer version) → agent surfaces it but does **not** auto-run `npx tektone skills update`
...
The CLI lives in this repo under `cli/`: `cli/bin/` (entry + sub-commands), `cli/engine/` (the detect-antipatterns rule engine + browser variant), `cli/lib/` (helpers shared by CLI and Cloudflare Pages Functions). Published to npm as `tektone`.

```bash
npx tektone detect [file-or-dir-or-url...]   # detect anti-patterns
npx tektone detect --fast --json src/         # regex-only, JSON output
npx tektone live                              # start browser overlay server
npx tektone skills install                    # install skills
npx tektone --help                            # show help
```
...
All commands live under `/tektone`. To add a new one:
...
4. Add the command name to `TEKTONE_SUB_COMMANDS` in `scripts/lib/utils.js`
...
The eval framework lives in a separate private repo at `~/code/tektone-evals/`. It measures whether the `/tektone` skill improves or harms AI-generated frontend design by running the same brief through a model with and without the skill loaded.
...
```bash
cd ~/code/tektone-evals
bun run serve            # dashboard on http://localhost:8723
```

The eval runners read this repo's skill from `../tektone/skill/` and staged provider skills from `../tektone/build/_data/dist/*`. Run `bun run build` in this repo before an eval sweep if you want the Claude/Gemini staged skills to reflect your latest edits.
