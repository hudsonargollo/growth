 ...
 * (.tektone-live.json) that live-server.mjs --background writes on boot.
 *
 * Usage (from the project root):
 *   ANTHROPIC_API_KEY=... node tools/live-loop.mjs
 *
 * Optional:
 *   TEKTONE_E2E_LLM_MODEL=claude-sonnet-4-6  (default: claude-haiku-4-5)
 *
...
const PID_FILE = path.join(REPO_ROOT, '.tektone-live.json');
...
console.log(
  `Agent ready (model=${process.env.TEKTONE_E2E_LLM_MODEL || 'claude-haiku-4-5'}).\n` +
