...
export function buildCopyEditBatchPrompt(batch, { cwd = process.cwd() } = {}) {
  ...
  return [
    'You are the Tektone staged copy-edit batch applier.',
    '',
    ...
    '- Never copy browser edit-mode scaffolding into source: no contenteditable, data-tektone-* markers, wrapper variants, generated style/script tags, or runtime-only attributes.',
    ...
    '- If package.json defines scripts.tektone:manual-edit-validate, it must pass after edits.',
    '- Check for leftover tektone-carbonize markers or variant wrapper markers in touched files.',
    '',
...
export async function runCopyEditBatchAgent(batch, opts = {}) {
  ...
  if (provider === 'mock') {
    const delayMs = Number(env.TEKTONE_LIVE_COPY_AGENT_MOCK_DELAY_MS || 0);
  ...
  const outDir = opts.outDir || fs.mkdtempSync(path.join(os.tmpdir(), 'tektone-copy-batch-'));
...
export function runCopyEditPostApplyChecks({ cwd = process.cwd(), files = [] } = {}) {
  ...
  for (const relativeFile of uniqueFiles) {
    ...
    const markerMatch = findLeftoverTektoneMarker(content);
    if (markerMatch) failures.push({ file: relativeFile, reason: 'leftover_tektone_marker', marker: markerMatch });
...
function findLeftoverTektoneMarker(content) {
  const commentMarker = content.match(/^\s*(?:<!--|\{\/\*)\s*tektone-carbonize-(?:start|end)\b|^\s*(?:<!--|\{\/\*)\s*tektone-variants-(?:start|end)\b/m);
  if (commentMarker) return commentMarker[0];

  const attrPattern = /\bdata-tektone-(?:variants?|original-text|editable|text-wrap)\s*=/g;
...
function readManualEditValidationScript(cwd) {
  ...
  try {
    ...
    const script = pkg?.scripts?.['tektone:manual-edit-validate'];
...
function stripLiveRuntimeHtml(html) {
  ...
  return html
    .replace(/\sdata-tektone-(?:original-text|editable|text-wrap)(?:=(?:"[^"]*"|'[^']*'|[^\s>]+))?/g, '')
...
function mockBatchResult(batch, env, cwd = process.cwd()) {
  ...
  const raw = env.TEKTONE_LIVE_COPY_AGENT_MOCK_RESULT;
  if (raw) {
    const parsed = parseCopyEditBatchResult(raw);
    if (parsed) return parsed;
    throw new Error('Invalid TEKTONE_LIVE_COPY_AGENT_MOCK_RESULT JSON');
  }
  return {
    status: 'done',
...
function applyMockWrites(env, cwd) {
  const raw = env.TEKTONE_LIVE_COPY_AGENT_MOCK_WRITES;
  if (!raw) return;
  const writes = tryParseJson(raw);
  if (!writes || typeof writes !== 'object' || Array.isArray(writes)) {
    throw new Error('Invalid TEKTONE_LIVE_COPY_AGENT_MOCK_WRITES JSON');
  }
...
export function chooseCopyEditAgent({
  ...
  const mode = (env.TEKTONE_LIVE_COPY_AGENT || 'auto').trim().toLowerCase();
...
function runCodex(prompt, { cwd, env, resultPath, logPath, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  const args = [
    ...
    '-c', `model_reasoning_effort="${env.TEKTONE_LIVE_COPY_AGENT_EFFORT || 'low'}"`,
  ];
  if (env.TEKTONE_LIVE_COPY_AGENT_MODEL) {
    args.push('--model', env.TEKTONE_LIVE_COPY_AGENT_MODEL);
  }
  args.push('-');
...
export function describeNoProviderError({
  ...
  if (chatAvailable()) {
    lines.push('  • Chat: an Tektone live session is polling but selection chose another provider — unexpected; please report.');
  } else {
    lines.push('  • Chat: no Tektone live session is currently polling on this server. Start Tektone live in your chat to route Apply through the chat agent.');
  }
  lines.push('Fix one of the above, or set TEKTONE_LIVE_COPY_AGENT=mock for tests.');
  return lines.join('\n');
}
