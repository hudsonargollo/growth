 ...
 * <div data-tektone-carbonize> so the slot keeps a single root node.
 */
function buildCarbonizeReplacement({
  ...
  const pushCarbonizeBody = (bodyIndent) => {
    ...
    lines.push(bodyIndent + commentSyntax.open + ' tektone-carbonize-start ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<style data-tektone-css="' + id + '">' + (isJsx ? '{`' : ''));
    ...
    if (paramValues && Object.keys(paramValues).length > 0) {
      lines.push(
        bodyIndent + commentSyntax.open + ' tektone-param-values ' + id + ': ' + JSON.stringify(paramValues) + ' ' + commentSyntax.close,
      );
    }
    lines.push(bodyIndent + commentSyntax.open + ' tektone-carbonize-end ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<div data-tektone-variant="' + variantNum + '" ' + variantStyleAttr + '>');
  ...
  if (isJsx) {
    ...
    lines.push(indent + '<div data-tektone-carbonize="' + id + '" ' + wrapperStyle + '>');
...
function handleAccept(id, variantNum, lines, targetFile, paramValues) {
  ...
  const hasHelperAttrs = variantText.includes('data-tektone-variant');
...
function readSourceShadowPreviewMeta(content, id) {
  ...
  const wrapperRe = new RegExp('<[^>]+data-tektone-variants=(["\'])' + escaped + '\\1[^>]*>');
  const match = String(content || '').match(wrapperRe);
  if (!match) return null;
  const tag = match[0];
  if (readHtmlAttr(tag, 'data-tektone-preview') !== 'source-shadow') return null;
  const sourceFile = readHtmlAttr(tag, 'data-tektone-source-file');
  const sourceStartLine = Number(readHtmlAttr(tag, 'data-tektone-source-start'));
  const sourceEndLine = Number(readHtmlAttr(tag, 'data-tektone-source-end'));
...
function findMarkerBlock(id, lines) {
  let start = -1;
  let end = -1;
  const startPattern = 'tektone-variants-start ' + id;
  const endPattern = 'tektone-variants-end ' + id;
 ...
 * the `<div data-tektone-variants="ID">` outer wrapper so the picked
 ...
 * (with `data-tektone-variants="ID"`) and its matching `</div>`. We
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Walk back for the wrapper `<div data-tektone-variants="..."` opener.
...
function isVariantEndMarkerLine(line, id) {
  return new RegExp('tektone-variants-end\\s+' + escapeRegExp(id) + '(?:\\s|--|\\*/|$)').test(line);
}

function hasVariantWrapperAttr(line, id) {
  const escaped = escapeRegExp(id);
  return new RegExp(`data-tektone-variants\\s*=\\s*(?:"${escaped}"|'${escaped}'|\\{["']${escaped}["']\\})`).test(line);
}
 ...
 *   - CSS `@scope ([data-tektone-variant="N"])` strings that look like the
...
function extractOriginal(lines, block) {
  ...
  const inner = extractInnerByAttr(text, 'data-tektone-variant="original"');
...
function extractVariant(lines, block, variantNum) {
  const text = stripStyleAndJoin(lines, block);
  const inner = extractInnerByAttr(text, 'data-tektone-variant="' + variantNum + '"');
 ...
 *
 * Handles three shapes of `<style data-tektone-css="ID" ...>`:
...
function extractCss(lines, block, id) {
  const styleAttr = 'data-tektone-css="' + id + '"';
  let inStyle = false;
  const content = [];
...
function findSessionFile(id, cwd) {
  const marker = 'tektone-variants-start ' + id;
