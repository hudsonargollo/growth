...
// DESIGN sidecar is project-local at .tektone/design.json, with legacy
...
const DEBUG_MANUAL_EDIT_EVENTS = /^(1|true|yes)$/i.test(process.env.TEKTONE_LIVE_DEBUG_EVENTS || '');
...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    ...
    path.join(__dirname, '..', '..', '..', '..', 'cli', 'engine', 'detect-antipatterns-browser.js'),
    path.join(process.cwd(), 'node_modules', 'tektone', 'cli', 'engine', 'detect-antipatterns-browser.js'),
  ];
  let detectScript = '';
...
function createRequestHandler({ detectScript, sessionPath, livePath }) {
  return (req, res) => {
    ...
    if (p === '/live.js') {
      ...
      const body =
        `window.__TEKTONE_TOKEN__ = '${state.token}';\n` +
        `window.__TEKTONE_PORT__ = ${state.port};\n` +
        ...
        `window.__TEKTONE_VOCAB__ = ${JSON.stringify(LIVE_COMMANDS)};\n` +
        sessionScript + '\n' +
        liveScript;
      res.writeHead(200, {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .tektone/design.json
    ...
    //                          - sidecar: .tektone/design.json contents when present.
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError = 'Failed to parse .tektone/design.json: ' + err.message;
        }
      }
...
function sessionFileMetadataFromPollReply(file) {
  ...
  if (!normalized.includes('node_modules/.tektone-live/') && !normalized.includes('src/lib/tektone/')) return base;
...
function cleanupSvelteComponentSessionsBeforeExit() {
  try {
    ...
    console.warn('[tektone] Svelte component session cleanup failed:', err.message);
  }
}
...
function applyLegacyDeferredAcceptsOnStartup() {
  try {
    ...
    if (result.applied > 0 || result.failed > 0) {
      console.log('[tektone] applied legacy deferred Svelte component accepts:', JSON.stringify(result));
    }
  } catch (err) {
    console.warn('[tektone] legacy deferred Svelte component accept apply failed:', err.message);
  }
}
...
httpServer.listen(state.port, '127.0.0.1', () => {
  ...
  console.log(`\nTektone live server running on ${url}`);
