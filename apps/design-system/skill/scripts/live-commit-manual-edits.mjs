 ...
 *
 * The browser Save path stages copy edits in .tektone/live. This script is
...
function repairAttemptLimit(env = process.env) {
  const value = Number(env.TEKTONE_LIVE_MANUAL_EDIT_REPAIR_ATTEMPTS || DEFAULT_REPAIR_ATTEMPTS);
...
async function main() {
  ...
  const result = await commitManualEdits({
    ...
    timeoutMs: Number(process.env.TEKTONE_LIVE_COPY_AGENT_TIMEOUT_MS || 120000),
  });
