...
- **`.tektone/live/config.json`** (live mode): pre-configured so `{{command_prefix}}tektone live` boots straight into variant mode with no first-time detour.
...
If init was invoked as a setup blocker by another command, such as `{{command_prefix}}tektone craft landing page`, pause that command here. Complete init, then resume the original command. Your own writes are the freshest source; no reload needed. For craft, resume into shape next; init creates project context, but it is not a substitute for the task-specific shape interview and confirmed design brief.
