 ...
 *
 * Variants are real .svelte components under node_modules/.tektone-live/<session-id>/.
...
export const SVELTE_COMPONENT_ROOT = 'node_modules/.tektone-live';
export const SVELTE_RUNTIME_FILE = `${SVELTE_COMPONENT_ROOT}/__runtime.js`;
export const DEFERRED_ACCEPTS_FILE = '.tektone/live/deferred-svelte-component-accepts.json';

const MUSTACHE_RE = /\{([^{}]+)\}/g;

export function shouldUseSvelteComponentInjection(filePath) {
  if (/^(0|false|no)$/i.test(process.env.TEKTONE_LIVE_SVELTE_COMPONENT || '')) return false;
...
function buildInsertVariantStub(variantNum) {
  return `${buildPropsScript([])}<div class="tektone-insert-preview">Insert variant ${variantNum}</div>\n\n<style>\n  .tektone-insert-preview { display: block; }\n</style>\n`;
}
...
function sanitizeAcceptedSvelteCss(cssLines, variantNum, paramValues = null, rootTag = 'div') {
  ...
  if (!/data-tektone-variant|tektone-variant-ready/.test(css)) return cssLines;
...
function appendSanitizedCssRule(output, rule, variantNum, paramValues, rootTag) {
  ...
  if (!prelude || !body || /--tektone-variant-ready\s*:/.test(body)) return;

  if (/^@scope\b/i.test(prelude)) {
    if (/data-tektone-variant/.test(prelude) && !selectorHasVariant(prelude, variantNum)) return;
    ...
    for (const innerRule of inner) {
      ...
      if (!rewrittenPrelude || /--tektone-variant-ready\s*:/.test(innerRule.body)) continue;
...
function variantSelectorRegex(variantNum) {
  return new RegExp(`\\[data-tektone-variant=(["'])${escapeRegExp(String(variantNum))}\\1\\]`, 'g');
}
...
function inlineSvelteComponentInsertAccept({
  ...
  if (/\bdata-tektone-[\w-]*\s*=/.test(markup)) {
    return { handled: false, error: 'Accepted Svelte insert variant contains preview-only data-tektone attributes', ...resultBase };
  }
...
export function deferredAcceptsPath(cwd = process.cwd()) {
  ...
  return path.join(os.tmpdir(), 'tektone-live', key, 'deferred-svelte-component-accepts.json');
}
...
export function buildSvelteComponentCssAuthoring(count) {
  ...
  return {
    ...
    requirements: [
      ...
      'Declare params in componentDir/params.json keyed by variant number (e.g. {"1": [...], "2": [...]}), NOT as a data-tektone-params attribute.',
      'Do not use @scope or data-tektone-variant selectors in component files.',
    ...
    forbidden: [
      ...
      'Do not add data-tektone-* attributes inside component files. Svelte parses { in attribute values as an expression, so data-tektone-params with JSON breaks the build; use componentDir/params.json instead.',
    ],
