...
function generateCounts(rootDir, skills, buildDir) {
  ...
  // of /tektone. Count them from the command router table in SKILL.md.
  const tektoneSkill = skills.find(s => s.name === 'tektone');
  let commandCount;
  if (tektoneSkill) {
    ...
    const routerMatches = tektoneSkill.body.match(/^\| `[^`]+` \|/gm);
...
/**
 * Narrow prose check for the tektone skill source.
 *
...
`Tektone. Design fluency for AI harnesses.
https://tektone.design
...
async function build() {
  ...
  if (BUILD_OPTIONS.syncRootOutputs) {
    ...
    const deprecatedLocalSkills = [
      'frontend-design', 'teach-tektone',
      'arrange', 'normalize', 'onboard', 'extract',
      // v3.0 consolidation: standalone skills -> /tektone sub-commands
    ...
    // dirs above stay where they are because `npx skills add pbakaus/tektone`
    ...
    const claudeSkillsSrc = path.join(DIST_DIR, 'claude-code', '.claude', 'skills', 'tektone');
    if (fs.existsSync(claudeSkillsSrc)) {
      fs.mkdirSync(pluginSkillsDir, { recursive: true });
      copyDirSync(claudeSkillsSrc, path.join(pluginSkillsDir, 'tektone'));
    }
