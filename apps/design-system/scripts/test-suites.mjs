...
export const SUITES = {
  core: {
    ...
    triggers: [
      ...
      /^skill\/(SKILL\.src\.md|agents\/|reference\/|scripts\/(cleanup-deprecated|context|context-signals|critique-storage|design-parser|tektone-paths|is-generated))/,
      /^site\/(pages|content|components|layouts)\//,
      /^README(\.npm)?\.md$/,
      /^cli\/bin\//,
      /^tests\/(build|cleanup-deprecated|context|context-signals|critique-storage|design-parser|docs-integrity|tektone-paths|skills-cli|test-suites|windows-path-fix)\.test\.(js|mjs)$/,
      /^tests\/lib\//,
    ],
    commands: [
      {
  ...
  'cli-remote-e2e': {
    description: 'Remote CLI install/update smoke tests against tektone.design.',
    optIn: true,
    triggers: [
      ...COMMON_INFRA_PATTERNS,
