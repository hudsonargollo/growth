...
export function generateApiData(outDir, skills, patterns, rootDir) {
  ...
  // /tektone. Load them from command-metadata.json and include the root
  // tektone skill itself so UI surfaces like the cheatsheet can list them.
  ...
  const tektone = skills.find(s => s.name === 'tektone');
  if (!tektone) {
    throw new Error('tektone skill not found at skill/SKILL.src.md. The build system expects exactly one skill at that path.');
  }
  ...
  const commandsData = [
    {
      id: 'tektone',
      name: 'tektone',
      description: tektone.description,
      tagline: readTagline('tektone'),
      userInvocable: true,
    },
  ...
  // (skill/scripts/context.mjs) to nudge users toward `npx tektone skills
