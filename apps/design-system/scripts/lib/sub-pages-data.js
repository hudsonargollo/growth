...
const EXCLUDED_SKILLS = new Set([
  'frontend-design',   // deprecated, renamed to tektone
  'teach-tektone',  // deprecated, folded into /tektone init
...
export async function buildSubPageData(rootDir) {
  ...
  // After the v3.0 consolidation there's only one source skill (tektone).
  ...
  const tektoneSkill = rawSkills.find((s) => s.name === 'tektone');
  ...
  const allCommandNames = ['tektone', ...Object.keys(commandMetadata)];
  const resolvePlaceholders = (content) =>
    replacePlaceholders(content, 'claude-code', [], allCommandNames);

  const skills = [];

  // 1. The root tektone skill itself.
  if (tektoneSkill && !EXCLUDED_SKILLS.has(tektoneSkill.name)) {
    const editorial = readEditorialWrapper(contentDir, 'skills', 'tektone');
    const demo = commandDemos['tektone'] || null;
    skills.push({
      id: 'tektone',
      name: 'tektone',
      description: tektoneSkill.description,
      argumentHint: tektoneSkill.argumentHint,
      category: SKILL_CATEGORIES['tektone'],
      body: resolvePlaceholders(tektoneSkill.body),
      references: (tektoneSkill.references || []).map((r) => ({
        ...r,
  ...
  if (tektoneSkill) {
    for (const [cmdId, meta] of Object.entries(commandMetadata)) {
      ...
      const refFile = tektoneSkill.references?.find((r) => r.name === cmdId);
