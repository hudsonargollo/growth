...
//   New installs write project config at .tektone/live/config.json instead.
...
/**
 * Read and parse the tektone skill source.
 ...
 * through to a compiled harness dir (`.agents/skills/tektone`) instead.
 */
 ...
 *
 * Defaults to the main tektone SKILL.md but accepts any relative path so
 ...
 *
 * The tektone-evals registry at `tools/instruction-rules.ts` pins
...
const EXCLUDED_FROM_SUGGESTIONS = new Set([
  'tektone',               // foundational skill, not a steering command
  'teach-tektone',         // deprecated shim
  'frontend-design',          // deprecated shim
]);

// Sub-commands of /tektone that should appear in {{available_commands}}.
// These are the commands that audit/critique/etc. reference when suggesting next steps.
const TEKTONE_SUB_COMMANDS = [
...
export function replacePlaceholders(content, provider, commandNames = [], allSkillNames = []) {
  ...
  // After the v3.0 consolidation, commands are sub-commands of /tektone.
  // If there's only one user-invocable skill (tektone), generate sub-command references.
  ...
  if (nonExcluded.length === 0) {
    // Single-skill architecture: list sub-commands as /tektone <sub>
    commandList = TEKTONE_SUB_COMMANDS
      .map(n => `${cmdPrefix}tektone ${n}`)
      .join(', ');
  } else {
