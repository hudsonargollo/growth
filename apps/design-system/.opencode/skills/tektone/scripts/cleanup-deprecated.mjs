#!/usr/bin/env node
/**
 * Cleans up deprecated Tektone skill files, symlinks, and
 ...
 *      "tektone" (to avoid deleting unrelated user skills).
...
const DEPRECATED_NAMES = [
  // v2.0 renames
  'frontend-design',    // renamed to tektone
  'teach-tektone',   // folded into /tektone init
  // v2.1 merges
  'arrange',            // renamed to layout
  'normalize',          // merged into polish
  'onboard',            // merged into harden
  'extract',            // merged into /tektone extract
  // v3.0 consolidation: all standalone skills -> /tektone sub-commands
  'adapt',
  'animate',
  'audit',
  'bolder',
...
// "tektone" in their v2.x source. Used as a last-resort match
...
/**
 * Check whether a skill directory belongs to Tektone. Three layered
 * signals, in order of reliability:
 *   1. Lock source equals "pbakaus/tektone" (authoritative).
 *   2. SKILL.md body contains the word "tektone".
 ...
 */
export function isTektoneSkill(skillDir, { skillName, lock } = {}) {
  // 1. Authoritative: the lock file claims this skill is ours.
  if (skillName && lock?.skills?.[skillName]?.source === 'pbakaus/tektone') {
    return true;
  }
  ...
  if (/tektone/i.test(content)) return true;
 ...
 * drive deletion even when SKILL.md never mentions tektone.
    ...
    for (const name of targets) {
      ...
      if (stat.isSymbolicLink()) {
        ...
        const isMatch = targetAlive
          ? isTektoneSkill(skillPath, { skillName: name, lock })
          : true;
        if (isMatch) {
      ...
      // Regular directory -- verify it belongs to tektone
      if (isTektoneSkill(skillPath, { skillName: name, lock })) {
 ...
 * Only removes entries whose source is "pbakaus/tektone".
...
export function cleanSkillsLock(projectRoot) {
  ...
  for (const name of targets) {
    ...
    if (!entry) continue;
    // Only remove if it belongs to tektone
    if (entry.source === 'pbakaus/tektone') {
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(new URL(import.meta.url).pathname)) {
  ...
  if (result.deletedPaths.length === 0 && result.removedLockEntries.length === 0) {
    console.log('No deprecated Tektone skills found. Nothing to clean up.');
  } else {
