 ...
 *   1. Check .tektone/live/config.json (returns config_missing if first-ever run)
 ...
 *   - Open the project's live dev/preview URL in the browser (optional, if browser automation exists)—not `serverPort`; that port is the Tektone helper for /live.js and /poll
...
Prepare everything for live variant mode in a single command:
  - Checks .tektone/live/config.json (required, created once per project)
