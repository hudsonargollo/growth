...
export const TEKTONE_DIR = '.tektone';
export const LIVE_DIR = 'live';
export const CRITIQUE_DIR = 'critique';

export function getTektoneDir(cwd = process.cwd()) {
  return path.join(cwd, TEKTONE_DIR);
}

export function getDesignSidecarPath(cwd = process.cwd()) {
  return path.join(getTektoneDir(cwd), 'design.json');
}
...
export function getLiveDir(cwd = process.cwd()) {
  return path.join(getTektoneDir(cwd), LIVE_DIR);
}
...
export function resolveLiveConfigPath({ cwd = process.cwd(), scriptsDir, env = process.env } = {}) {
  if (env.TEKTONE_LIVE_CONFIG && env.TEKTONE_LIVE_CONFIG.trim()) {
    const configured = env.TEKTONE_LIVE_CONFIG.trim();
...
export function getLegacyLiveServerPath(cwd = process.cwd()) {
  return path.join(cwd, '.tektone-live.json');
}
...
export function getLegacyLiveSessionsDir(cwd = process.cwd()) {
  return path.join(cwd, '.tektone-live', 'sessions');
}
...
export function getCritiqueDir(cwd = process.cwd()) {
  return path.join(getTektoneDir(cwd), CRITIQUE_DIR);
}

export function getLegacyLiveAnnotationsDir(cwd = process.cwd()) {
  return path.join(cwd, '.tektone-live', 'annotations');
}
