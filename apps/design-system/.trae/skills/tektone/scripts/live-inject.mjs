 ...
 *
 * On first live run, the agent generates `.tektone/live/config.json`
...
import { resolveLiveConfigPath } from './lib/tektone-paths.mjs';
import {
...
const MARKER_OPEN_TEXT = 'tektone-live-start';
const MARKER_CLOSE_TEXT = 'tektone-live-end';
const IGNORE_MARKER_OPEN = '# tektone-live-ignore-start';
const IGNORE_MARKER_CLOSE = '# tektone-live-ignore-end';

export const LIVE_IGNORE_PATTERNS = Object.freeze([
  '.tektone/hook.cache.json',
  '.tektone/live/server.json',
  '.tektone/live/sessions/',
  '.tektone/live/previews/',
  '.tektone/live/annotations/',
  '.tektone/live/cache/',
  '.tektone/live/manual-edit-apply-transaction.json',
  '.tektone/live/manual-edit-events.jsonl',
  '.tektone/live/manual-edit-evidence/',
  '.tektone/live/pending-manual-edits.json',
  '.tektone/live/deferred-svelte-component-accepts.json',
  '.tektone-live.json',
  '.tektone-live/',
  'node_modules/.tektone-live/',
  'src/lib/tektone/TektoneLiveRoot.svelte',
  'src/lib/tektone/__runtime.js',
  'src/lib/tektone/[0-9a-f]*/',
]);
...
Reads configuration from .tektone/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .tektone/live/config.json exists and its content
...
function removeTag(content, _syntax) {
  const patterns = [
    /([ \t]*)<!--\s*tektone-live-start\s*-->[\s\S]*?<!--\s*tektone-live-end\s*-->([ \t]*(?:\r\n|\n|\r|$)?)/,
    /([ \t]*)\{\/\*\s*tektone-live-start\s*\*\/\}[\s\S]*?\{\/\*\s*tektone-live-end\s*\*\/\}([ \t]*(?:\r\n|\n|\r|$)?)/,
  ];
  for (const pat of patterns) {
    let changed = false;
...
// and stash the original `content` value in a `data-tektone-csp-original`
...
const CSP_MARKER_ATTR = 'data-tektone-csp-original';
