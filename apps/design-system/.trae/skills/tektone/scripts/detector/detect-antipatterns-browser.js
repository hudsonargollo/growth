/**
 * Anti-Pattern Browser Detector for Tektone
...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__TEKTONE_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip tektone's own elements
    if (el.closest && el.closest('.tektone-overlay, .tektone-label, .tektone-banner, .tektone-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE = (_myScript && _myScript.dataset.tektoneExtension === 'true')
    || document.documentElement.dataset.tektoneExtension === 'true';
  ...
  styleEl.textContent = `
    @keyframes tektone-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .tektone-overlay:not(.tektone-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: tektone-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .tektone-overlay.tektone-visible {
      animation-play-state: running;
    }
    .tektone-overlay.tektone-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .tektone-overlay.tektone-hover .tektone-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .tektone-overlay.tektone-spotlight {
      z-index: 100002 !important;
    }
    .tektone-overlay.tektone-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .tektone-spotlight-backdrop {
      position: fixed;
    ...
    .tektone-spotlight-backdrop.tektone-visible {
      opacity: 1;
    }
    .tektone-hidden .tektone-overlay${EXTENSION_MODE ? '' : ':not(.tektone-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'tektone-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__TEKTONE_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('tektone-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('tektone-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('tektone-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._tektoneOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('tektone-visible');
  ...
  function detachOverlay(overlay) {
    ...
    if (overlay._targetEl && overlay._targetEl._tektoneOverlay === overlay) {
      visibilityObserver.unobserve(overlay._targetEl);
      delete overlay._targetEl._tektoneOverlay;
    }
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('tektone-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    ...
    const onMouseEnter = () => {
      isHovered = true;
      outline.classList.add('tektone-hover');
    ...
    const onMouseLeave = () => {
      isHovered = false;
      outline.classList.remove('tektone-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'tektone-overlay tektone-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('tektone-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('tektone-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  function collectVisualContrastCandidates(options = {}) {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (el.closest('.tektone-overlay, .tektone-label, .tektone-banner, .tektone-tooltip')) continue;
      if (el.closest('[id^="tektone-live-"]')) continue;
  ...
  async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
    ...
    for (const node of nodes) {
      ...
      if (node.closest?.('.tektone-overlay, .tektone-label, .tektone-banner, .tektone-tooltip')) continue;
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[tektone] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[tektone] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  function collectBrowserFindings() {
    ...
    const _disabled = EXTENSION_MODE ? (window.__TEKTONE_CONFIG__?.disabledRules || []) : [];
    ...
    for (const el of document.querySelectorAll('*')) {
      // Skip tektone's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.tektone-overlay, .tektone-label, .tektone-banner, .tektone-tooltip')) continue;
      // Skip browser extension elements (Claude, etc.)
      const elId = el.id || '';
      if (elId.startsWith('claude-') || elId.startsWith('cic-')) continue;
      // Skip the tektone live-mode overlay (highlight, tooltip, bar, picker, toast).
    ...
    // Clone the document and strip tektone-live overlay nodes before the
    ...
    for (const node of docClone.querySelectorAll('[id^="tektone-live-"]')) {
      node.remove();
    }
  ...
  function shouldRunVisualContrast(options = {}) {
    return options.visualContrast === true || window.__TEKTONE_CONFIG__?.visualContrast === true;
  }

  function visualContrastOptions(options = {}) {
    const config = window.__TEKTONE_CONFIG__ || {};
  ...
  function postExtensionError(err) {
    if (!EXTENSION_MODE) return;
    window.postMessage({
      source: 'tektone-error',
      message: err?.message || String(err),
    }, '*');
  }

  function reportVisualContrastError(err, detail = {}) {
    window.dispatchEvent(new CustomEvent('tektone-visual-contrast-error', {
      detail: {
        ...detail,
        message: err?.message || String(err),
      },
    ...
    if (EXTENSION_MODE) {
      ...
      console.warn('[tektone] visual contrast scan failed', err);
    }
  }
        ...
        waitForVisualPaint()
          ...
          .then(result => {
            ...
            if (added) {
              ...
              window.dispatchEvent(new CustomEvent('tektone-visual-contrast-resolved', {
                detail: {
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'tektone-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__TEKTONE_CONFIG__ = e.data.config;
        try {
          scan(e.data.config || {});
        } catch (err) {
          postExtensionError(err);
      ...
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('tektone-hidden');
        document.body.classList.toggle('tektone-hidden', visible);
        window.postMessage({ source: 'tektone-overlays-toggled', visible: !visible }, '*');
      }
      if (e.data.action === 'remove') {
        clearOverlays();
        styleEl.remove();
        if (spotlightBackdrop) { spotlightBackdrop.remove(); spotlightBackdrop = null; }
        document.body.classList.remove('tektone-hidden');
      }
      if (e.data.action === 'highlight') {
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('tektone-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('tektone-spotlight', isMatch);
              o.classList.toggle('tektone-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('tektone-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('tektone-spotlight');
          o.classList.remove('tektone-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'tektone-ready' }, '*');
  } else {
    if (window.__TEKTONE_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          scan();
        } catch (err) {
          console.warn('[tektone] scan failed', err);
        }
      };
  ...
  window.tektoneDetect = detect;
  window.tektoneDetectAsync = detectAsync;
  window.tektoneScan = scan;
  window.tektoneScanAsync = scanAsync;
  window.tektoneCollectVisualContrastCandidates = collectVisualContrastCandidates;
  window.tektoneAnalyzeVisualContrast = analyzeVisualContrast;
  window.tektoneGetLastVisualContrastAnalyses = () => lastVisualContrastAnalyses.slice();
}
