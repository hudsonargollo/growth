...
function withTempProject(fn) {
  const cwd = mkdtempSync(join(tmpdir(), 'tektone-live-recovery-'));
  try { return fn(cwd); }
...
describe('live recovery CLI commands', () => {
  it('prints active durable session status without a running helper server', () => withTempProject((cwd) => {
    ...
    store.appendEvent({ type: 'generate', id: 'cli-recover-1', action: 'tektone', count: 3, pageUrl: '/', element: { outerHTML: '<button>Go</button>' } });
  ...
  it('resumes the pending event and reports the next safe agent action', () => withTempProject((cwd) => {
    const store = createLiveSessionStore({ cwd });
    store.appendEvent({ type: 'generate', id: 'cli-recover-2', action: 'tektone', count: 2, pageUrl: '/', element: { outerHTML: '<section>Hero</section>' } });
  ...
  it('marks a session completed through the canonical completion command', () => withTempProject((cwd) => {
    const store = createLiveSessionStore({ cwd });
    store.appendEvent({ type: 'generate', id: 'cli-recover-3', action: 'tektone', count: 1, pageUrl: '/', element: { outerHTML: '<p>Copy</p>' } });
