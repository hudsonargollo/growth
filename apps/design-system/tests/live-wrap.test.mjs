...
describe('findElement', () => {
  ...
  it('skips lines that contain data-tektone-variant', () => {
    const lines = [
      '<div class="hero" data-tektone-variant="original">Old</div>',
...
describe('wrapCli integration', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'tektone-wrap-test-'));
...
<body>
  <div class="hero-section">
    ...
    assert.ok(modified.includes('data-tektone-variants="test123"'));
    assert.ok(modified.includes('data-tektone-variant-count="3"'));
    assert.ok(modified.includes('data-tektone-variant="original"'));
    assert.ok(modified.includes('display: contents'));
    assert.ok(modified.includes('tektone-variants-start test123'));
    assert.ok(modified.includes('tektone-variants-end test123'));
    // Original should NOT be hidden (stays visible until variants arrive)
    assert.ok(!modified.includes('data-tektone-variant="original" style="display: none"'));
  });
  ...
  return (
    ...
    assert.ok(modified.includes('{/* tektone-variants-start jsx123'));
    assert.ok(modified.includes('data-tektone-variant-count="2"'));
  });
...
<div id="pricing">
  ...
  <p>Plans start at $10/mo.</p>
    ...
    assert.ok(modified.includes('data-tektone-variants="id123"'));
...
<div class="after">After</div>`;
    ...
    assert.ok(modified.includes('data-tektone-variants="pres123"'));
  });
  ...
  <h1>Plain title</h1>
    ...
    assert.equal(result.cssAuthoring.styleTag, '<style data-tektone-css="SESSION_ID">');
...
<section class="hero-shell">
  <h1>{title}</h1>
    ...
    assert.deepEqual(result.cssSelectorPrefixExamples, [
      '[data-tektone-variant="1"]',
      '[data-tektone-variant="2"]',
      '[data-tektone-variant="3"]',
    ]);
    assert.equal(result.cssAuthoring.mode, 'astro-global-prefixed');
    assert.equal(result.cssAuthoring.strategy, 'global-prefixed');
    assert.equal(result.cssAuthoring.styleTag, '<style is:inline data-tektone-css="SESSION_ID">');
    assert.match(result.cssAuthoring.rulePattern, /^\[data-tektone-variant="N"\]/);
    assert.ok(
...
// .tektone/live/pending-manual-edits.json from local dev tripped the
// fail-loud check in live-wrap. Clear the buffer around each test.
function clearManualEditsBuffer() {
  try {
    const p = join(process.cwd(), '.tektone/live/pending-manual-edits.json');
    rmSync(p, { force: true });
  } catch {}
}
...
describe('live-wrap — JSX / TSX correctness', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'tektone-wrap-jsx-')); clearManualEditsBuffer(); });
  ...
  return (
    ...
    assert.ok(modified.includes('tektone-variants-start wrapA'), 'wrapper was created');
    ...
    const originalMatch = modified.match(/data-tektone-variant="original"[^>]*>([\s\S]*?)\s*<\/div>/);
...
export default function App() {
  ...
  return (
    ...
    // The outer wrapper <div data-tektone-variants="..."> appears BEFORE
    // both marker comments — markers are tucked inside.
    const wrapperIdx = modified.indexOf('data-tektone-variants="frag1"');
    const startMarkerIdx = modified.indexOf('tektone-variants-start frag1');
    const endMarkerIdx = modified.indexOf('tektone-variants-end frag1');
  ...
  it('HTML wrapper keeps marker comments OUTSIDE the wrapper <div> (existing layout)', () => {
    ...
    const modified = readFileSync(join(tmp, 'page.html'), 'utf-8');
    const wrapperIdx = modified.indexOf('data-tektone-variants="htmlFrag"');
    const startMarkerIdx = modified.indexOf('tektone-variants-start htmlFrag');
  ...
  return (
    ...
    const modified = readFileSync(join(tmp, 'Page.tsx'), 'utf-8');
    const originalMatch = modified.match(/data-tektone-variant="original"[\s\S]*?<\/div>/);
  ...
  return (
    ...
    assert.ok(modified.includes('data-tektone-variants="short1"'),
  ...
  <section class="multiline-target">
    ...
    // line (the tektone-variants-end marker for HTML).
    assert.match(lines[result.endLine - 1], /tektone-variants-end ml1/,
  ...
  return (
    ...
    assert.ok(modified.includes('data-tektone-variants="dyn1"'), 'wrapped (first-match fallback)');
  });
