...
describe('live-copy-edit-agent', () => {
  it('builds a batch prompt with duplicate-card context and candidate evidence', () => {
    ...
    assert.match(prompt, /tektone:manual-edit-validate/);
  ...
  it('flags invalid JS and actual leftover carbonize markers in post-apply checks', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(tmp, 'src', 'page.html'), '<!-- tektone-carbonize-end abcdef12 -->\n<h1>Hi</h1>\n');
      const checks = runCopyEditPostApplyChecks({ cwd: tmp, files: ['src/bad.js', 'src/page.html'] });
      assert.equal(checks.ok, false);
      assert.equal(checks.failures.some((item) => item.reason === 'leftover_tektone_marker'), true);
  ...
  it('does not flag live-mode marker words when they only appear as source literals', () => {
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          'const words = "tektone-carbonize- data-tektone-variant TEKTONE_VARIANT tektone-live-variant";',
          'const selector = "[data-tektone-variant=\\"1\\"]";',
          'const example = "<div data-tektone-variant=\\"1\\">demo</div>";',
  ...
  it('respects off mode before trying local AI commands', () => {
    assert.equal(chooseCopyEditAgent({ env: { TEKTONE_LIVE_COPY_AGENT: 'off' } }), null);
    assert.equal(chooseCopyEditAgent({ env: { TEKTONE_LIVE_COPY_AGENT: 'false' } }), null);
    assert.equal(chooseCopyEditAgent({ env: { TEKTONE_LIVE_COPY_AGENT: 'mock' } }), 'mock');
  });
  ...
  it('describeNoProviderError pinpoints which CLI needs which action', () => {
    ...
    assert.match(nothingInstalled, /TEKTONE_LIVE_COPY_AGENT=mock/);
  ...
  it('auto mode picks the first authenticated provider via injected authCheck', () => {
    ...
    assert.equal(
      chooseCopyEditAgent({ env: { TEKTONE_LIVE_COPY_AGENT: 'auto' }, authCheck: claudeUnauthedCodexAuthed }),
      'codex',
    );

    const onlyClaudeAuthed = (cmd) => cmd === 'claude';
    assert.equal(
      chooseCopyEditAgent({ env: { TEKTONE_LIVE_COPY_AGENT: 'auto' }, authCheck: onlyClaudeAuthed }),
      'claude',
    );
  ...
  it('auto mode falls back to chat when no CLI is authenticated and chat is available', () => {
    ...
    assert.equal(
      chooseCopyEditAgent({
        env: { TEKTONE_LIVE_COPY_AGENT: 'auto' },
  ...
  it('describeNoProviderError mentions starting tektone live when chat is the missing piece', () => {
    ...
    assert.match(noChatPolling, /Chat: no Tektone live session is currently polling/);
    assert.match(noChatPolling, /Start Tektone live/);
  });
});
