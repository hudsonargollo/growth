...
describe('live-accept — style-element edge cases', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'tektone-accept-test-')); });
  ...
  // separate closer, so it got stuck forever and missed data-tektone-variant
  // divs that came after.
  it('finds the accepted variant after a JSX self-closing <style /> block', () => {
    const html = `<body>
  <!-- tektone-variants-start SELFC -->
  <div data-tektone-variants="SELFC" data-tektone-variant-count="3" style="display: contents">
    <div data-tektone-variant="original">
      <p class="hook">original text</p>
    </div>
    <style data-tektone-css="SELFC" dangerouslySetInnerHTML={{ __html: '@scope ([data-tektone-variant="1"]) { .hook { color: red; } }' }} />
  ...
  <!-- tektone-variants-end SELFC -->
    ...
    // no carbonize block, no data-tektone-variant wrapper (it would serve no purpose).
    assert.ok(!after.includes('tektone-carbonize-start'), 'no carbonize block (self-closing style has no body)');
    assert.ok(!after.includes('tektone-variants-start'), 'variant markers removed');
  ...
  it('finds the accepted variant after a single-line <style>…</style> block', () => {
    const html = `<body>
  <!-- tektone-variants-start ONELINE -->
  <div data-tektone-variants="ONELINE" data-tektone-variant-count="3" style="display: contents">
    <div data-tektone-variant="original"><p class="hook">original</p></div>
    <style data-tektone-css="ONELINE">@scope ([data-tektone-variant="1"]) { .hook { color: red; } }</style>
    <div data-tektone-variant="1"><p class="hook">variant one</p></div>
    <div data-tektone-variant="2" style="display: none"><p class="hook">variant two</p></div>
    <div data-tektone-variant="3" style="display: none"><p class="hook">variant three</p></div>
  </div>
  <!-- tektone-variants-end ONELINE -->
</body>`;
    writeFileSync(join(tmp, 'page.html'), html);
    ...
    const after = readFileSync(join(tmp, 'page.html'), 'utf-8');
    assert.ok(after.includes('data-tektone-variant="3"'), 'accepted wrapper for variant 3 present');
  ...
  it('finds the accepted variant after a multi-line <style>…</style> block (regression baseline)', () => {
    const html = `<body>
  <!-- tektone-variants-start MULTI -->
  <div data-tektone-variants="MULTI" data-tektone-variant-count="3" style="display: contents">
    <div data-tektone-variant="original"><p class="hook">original</p></div>
    <style data-tektone-css="MULTI">
      @scope ([data-tektone-variant="1"]) { .hook { color: red; } }
      @scope ([data-tektone-variant="2"]) { .hook { color: green; } }
    </style>
    <div data-tektone-variant="1"><p class="hook">variant one</p></div>
    <div data-tektone-variant="2" style="display: none"><p class="hook">variant two</p></div>
  </div>
  <!-- tektone-variants-end MULTI -->
</body>`;
    writeFileSync(join(tmp, 'page.html'), html);
    ...
    const after = readFileSync(join(tmp, 'page.html'), 'utf-8');
    assert.ok(after.includes('data-tektone-variant="1"'), 'accepted wrapper for variant 1 present');
  ...
  it('carbonize does not double-wrap when the variants block uses JSX template literals on their own lines', () => {
    const tsx = `export default function App() {\n` +
      ...
      `    <main>\n` +
      `      <>\n` +
      `        {/* tektone-variants-start TPL */}\n` +
      `        <div data-tektone-variants="TPL" data-tektone-variant-count="2" style={{ display: 'contents' }}>\n` +
      `          <div data-tektone-variant="original"><p className="hook">orig</p></div>\n` +
      `          <style data-tektone-css="TPL">\n` +
      "            {`\n" +
      `              @scope ([data-tektone-variant="1"]) { .hook { color: red; } }\n` +
      `              @scope ([data-tektone-variant="2"]) { .hook { color: green; } }\n` +
      "            `}\n" +
      `          </style>\n` +
      `          <div data-tektone-variant="1"><p className="hook">variant one</p></div>\n` +
      `          <div data-tektone-variant="2" style={{ display: 'none' }}><p className="hook">variant two</p></div>\n` +
      `        </div>\n` +
      `        {/* tektone-variants-end TPL */}\n` +
      `      </>\n` +
      `    </main>\n` +
      `  );\n` +
      `}\n`;
    ...
    const carbonStyleMatch = after.match(/<style data-tektone-css="TPL">([\s\S]*?)<\/style>/);
    ...
    assert.ok(inner.includes('@scope ([data-tektone-variant="1"])'), 'variant-1 scope kept');
    assert.match(
      after,
      /\n          <div data-tektone-variant="1" style=\{\{ display: 'contents' \}\}>\n            <p className="hook">variant one<\/p>\n          <\/div>/,
  ...
  it('carbonize does not double-wrap when JSX template-literal punctuation hugs the CSS lines', () => {
    const tsx = `export default function App() {\n` +
      ...
      `    <main>\n` +
      `      <>\n` +
      `        {/* tektone-variants-start INLINE */}\n` +
      `        <div data-tektone-variants="INLINE" data-tektone-variant-count="2" style={{ display: 'contents' }}>\n` +
      `          <div data-tektone-variant="original"><p className="hook">orig</p></div>\n` +
      `          <style data-tektone-css="INLINE">\n` +
      "            {`@scope ([data-tektone-variant=\"1\"]) { .hook { color: red; } }\n" +
      "             @scope ([data-tektone-variant=\"2\"]) { .hook { color: green; } }`}\n" +
      `          </style>\n` +
      `          <div data-tektone-variant="1"><p className="hook">variant one</p></div>\n` +
    ...
    const after = readFileSync(join(tmp, 'App.tsx'), 'utf-8');
    const inner = after.match(/<style data-tektone-css="INLINE">([\s\S]*?)<\/style>/)[1];
    const openCount = (inner.match(/\{`/g) || []).length;
    const closeCount = (inner.match(/`\}/g) || []).length;
  ...
  it('carbonize preserves nested JSX indentation when the wrapper starts at column 0', () => {
    const tsx = `<div data-tektone-variants="ROOTIND" data-tektone-variant-count="2" style={{ display: 'contents' }}>
  {/* tektone-variants-start ROOTIND */}
  <div data-tektone-variant="original">
    <section className="hook">
      <span>original</span>
    </section>
  </div>
  <style data-tektone-css="ROOTIND">{\`@scope ([data-tektone-variant="1"]) { .hook { color: red; } }\`}</style>
  <div data-tektone-variant="1">
    <section className="hook">
...
`;
  ...
  return (
    ...
    // the outer wrapper, and no tektone markers/data attributes.
    assert.ok(!after.includes('data-tektone-variants'),
      `outer wrapper div must be fully removed; got:\n${after}`);
    assert.ok(!after.includes('data-tektone-variant'),
      `original-div wrapper must be fully removed; got:\n${after}`);
    assert.ok(!after.includes('tektone-variants-start'),
  ...
  return (
    <main>
      <div
        className="tektone-preview-shell"
${extraAttrs}
        data-tektone-variants="LONGOPEN"
        data-tektone-variant-count="2"
        style={{ display: 'contents' }}
      >
        {/* tektone-variants-start LONGOPEN */}
        {/* Original */}
        <div data-tektone-variant="original">
          <aside className="card">
            <h1>Original</h1>
        ...
        <div data-tektone-variant="1"><aside className="card"><h1>Variant</h1></aside></div>
        {/* tektone-variants-end LONGOPEN */}
      </div>
      <div className="next-card">After</div>
    </main>
  );
    ...
    const after = readFileSync(join(tmp, 'App.tsx'), 'utf-8');
    assert.doesNotMatch(after, /data-tektone-variants/);
    assert.doesNotMatch(after, /tektone-variants-start/);
  ...
  return (
    <main>
      <div data-tektone-variants="ACTIVE" data-tektone-variant-count="2" style={{ display: 'contents' }}>
        <div className="historical-marker-note">
          {/* tektone-variants-end OLD */}
        </div>
        {/* tektone-variants-start ACTIVE */}
        {/* Original */}
        <div data-tektone-variant="original">
          <aside className="card">
            <h1>Original</h1>
    ...
    const after = readFileSync(join(tmp, 'App.tsx'), 'utf-8');
    assert.doesNotMatch(after, /data-tektone-variants/);
    assert.doesNotMatch(after, /tektone-variants-start ACTIVE/);
    assert.doesNotMatch(after, /tektone-variants-end OLD/);
    assert.match(after, /<aside className="card">\s*<h1>Original<\/h1>\s*<\/aside>/m);
    assert.ok(after.includes('<div className="next-card">After</div>'));
  });
  ...
  return (
    <main>
      <div data-tektone-variants="INDENTACC" data-tektone-variant-count="3" style={{ display: "contents" }}>
        {/* tektone-variants-start INDENTACC */}
        {/* Original */}
        <div data-tektone-variant="original">
          <aside className="card">
            <h1 className="hero-title">Hero</h1>
        ...
        {/* Variants: insert below this line */}
        <div data-tektone-variant="1"><aside className="card variant-one"><h1 className="hero-title">Hero</h1></aside></div>
        {/* tektone-variants-end INDENTACC */}
      </div>
    </main>
  );
}`;
  ...
  it('discard restores the original element after a JSX self-closing <style />', () => {
    const html = `<body>
  <!-- tektone-variants-start DISC -->
  <div data-tektone-variants="DISC" data-tektone-variant-count="2" style="display: contents">
    <div data-tektone-variant="original"><p class="hook">ORIGINAL CONTENT</p></div>
    <style data-tektone-css="DISC" dangerouslySetInnerHTML={{ __html: '@scope ([data-tektone-variant="1"]) { .hook { color: red; } }' }} />
    <div data-tektone-variant="1"><p class="hook">variant one</p></div>
    <div data-tektone-variant="2" style="display: none"><p class="hook">variant two</p></div>
  </div>
  <!-- tektone-variants-end DISC -->
</body>`;
    writeFileSync(join(tmp, 'page.html'), html);
    ...
    assert.ok(!after.includes('tektone-variants-start'), 'wrapper markers gone');
...
describe('live-accept — insert sessions', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'tektone-accept-insert-')); });
  afterEach(() => { rmSync(tmp, { recursive: true, force: true }); });

  const insertHtml = (id) => `<main>
  <section class="hero">Hero block</section>
  <!-- tektone-variants-start ${id} -->
  <div data-tektone-variants="${id}" data-tektone-mode="insert" data-tektone-variant-count="2" style="display: contents">
    <!-- Variants: insert below this line -->
    <style data-tektone-css="${id}">@scope ([data-tektone-variant="1"]) { .cta { color: red; } }</style>
    <div data-tektone-variant="1"><p class="cta">Variant one</p></div>
    <div data-tektone-variant="2" style="display: none"><p class="cta">Variant two</p></div>
  </div>
  <!-- tektone-variants-end ${id} -->
