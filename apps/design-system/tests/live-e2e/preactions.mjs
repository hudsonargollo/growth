...
export async function runPreActions(page, actions) {
  ...
  const wasActive = await page.evaluate((sel) =>
    window.__tektoneLiveQuery?.(sel)?.dataset.active === 'true',
...
async function liveUiSnapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      hasShadowRoot: Boolean(document.getElementById('tektone-live-root')?.shadowRoot),
      ...
      wrapper: wrapper ? { preview: wrapper.dataset.tektonePreview, count: wrapper.dataset.tektoneVariantCount, html: wrapper.outerHTML.slice(0, 800) } : null,
