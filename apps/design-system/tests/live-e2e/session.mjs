...
export function stageFixture(name, fixture) {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'tektone-e2e-'));
...
export function runInstall(tmp, command, { timeoutMs = readTimeoutEnv('TEKTONE_E2E_INSTALL_TIMEOUT_MS', 180_000) } = {}) {
...
export function startDevServer(tmp, runtime) {
  ...
  const ready = new Promise((resolve, reject) => {
    const readyTimeoutMs = readTimeoutEnv(
      'TEKTONE_E2E_DEV_READY_TIMEOUT_MS',
