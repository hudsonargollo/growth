 ...
 *    `<style data-tektone-css="ID">` block with `@scope ([data-tektone-variant="N"])`
 *    rules, a `data-tektone-params` JSON manifest covering range + steps + toggle
...
export const STEER_MARKER_ATTR = 'data-tektone-steer';
 ...
 * @property {string}        scopedCss   Contents of the <style data-tektone-css>
 ...
 *   - <style data-tektone-css="ID"> with @scope rules per variant
 *   - data-tektone-params manifest with range + steps + toggle kinds
  ...
  return {
    ...
    async generateVariants(event, context = {}) {
      ...
      const scopedCss = useAstroGlobalCss
        ? [
            `[data-tektone-variant="1"] > ${tag} {`,
            '  color: oklch(var(--p-lightness, 0.5) 0.25 25);',
            '}',
            `[data-tektone-variant="2"] > ${tag} { font-weight: 900; }`,
            `[data-tektone-variant="2"][data-p-face="serif"] > ${tag} { font-family: ui-serif, serif; }`,
            `[data-tektone-variant="2"][data-p-face="mono"]  > ${tag} { font-family: ui-monospace, monospace; }`,
            `[data-tektone-variant="3"] > ${tag} { text-transform: uppercase; letter-spacing: 0.04em; }`,
            `[data-tektone-variant="3"][data-p-italic] > ${tag} { font-style: italic; }`,
          ].join('\n')
        : [
...
function generateInsertFakeVariants(context = {}) {
  ...
  const scopedCss = useAstroGlobalCss
    ? [
        '[data-tektone-variant="1"] .inserted-copy {',
        '  color: oklch(var(--p-lightness, 0.5) 0.25 25);',
        '}',
        '[data-tektone-variant="2"] .inserted-copy { font-weight: 900; }',
        '[data-tektone-variant="2"][data-p-face="serif"] .inserted-copy { font-family: ui-serif, serif; }',
        '[data-tektone-variant="2"][data-p-face="mono"]  .inserted-copy { font-family: ui-monospace, monospace; }',
        '[data-tektone-variant="3"] .inserted-copy { text-transform: uppercase; letter-spacing: 0.04em; }',
        '[data-tektone-variant="3"][data-p-italic] .inserted-copy { font-style: italic; }',
      ].join('\n')
    : [
        '@scope ([data-tektone-variant="1"]) {',
        '  :scope .inserted-copy {',
...
export const HOIST_ATTR = 'data-tektone-hoist-id';
...
function normalizeVariantSelectorQuotes(css) {
  return String(css).replace(
    /\[data-tektone-variant=(['"])(\d+)\1\]/g,
    (_match, _quote, id) => `[data-tektone-variant="${id}"]`,
  );
}
...
function hasBaseVariantRule(scopedCss, variantId, styleMode) {
  ...
  if (styleMode === 'astro-global-prefixed') {
    return new RegExp(String.raw`\[data-tektone-variant=${q}\](?:\s|>|\.|#|\[${HOIST_ATTR}=)`).test(scopedCss);
  }
  return new RegExp(String.raw`@scope\s*\(\s*\[data-tektone-variant=${q}\]\s*\)`).test(scopedCss);
}

function renderBaseVariantRule(variantId, styleMode) {
  if (styleMode === 'astro-global-prefixed') {
    return [
      `[data-tektone-variant="${variantId}"] > * {`,
      '  --tektone-variant-ready: 1;',
      '}',
    ].join('\n');
  }
  return [
    `@scope ([data-tektone-variant="${variantId}"]) {`,
    '  :scope > * { --tektone-variant-ready: 1; }',
    '}',
  ].join('\n');
}
...
// `style="..."`, and tag the element with `data-tektone-hoist-id="N"`.
...
function renderHoistedInlineStyleRule({ variantId, hoistId, declarations, styleMode }) {
  ...
  if (styleMode === 'astro-global-prefixed') {
    return [
      `[data-tektone-variant="${variantId}"] ${target} {`,
 ...
 *   - data-tektone-params stays a single-quoted JSON string (JSX-legal)
 */
function renderVariantsBlock({ sessionId, indent, output, commentSyntax, file, styleMode }) {
  ...
  const styleLines = isJsx
    ? [
        indent + '  <style data-tektone-css="' + sessionId + '">{`',
        ...output.scopedCss.split('\n').map((l) => indent + '    ' + l),
        indent + '  `}</style>',
      ]
    : [
        indent + '  <style' + (isAstroGlobalCss ? ' is:inline' : '') + ' data-tektone-css="' + sessionId + '">',
        ...output.scopedCss.split('\n').map((l) => indent + '    ' + l),
        indent + '  </style>',
      ];
  ...
  const variantBlocks = output.variants.map((v, i) => {
    ...
    const paramsAttr = v.params && v.params.length
      ? " data-tektone-params='" + attrEscape(JSON.stringify(v.params), { svelte: isSvelte }) + "'"
      : '';
    let styleAttr = '';
    ...
    return [
      ...
      indent + '  <div data-tektone-variant="' + idx + '"' + styleAttr + paramsAttr + '>',
      indent + '    ' + inner,
      indent + '  </div>',
    ].join('\n');
  });
...
async function writeSvelteComponentVariants({ tmp, wrapInfo, event, output }) {
  ...
  for (let i = 0; i < output.variants.length; i++) {
    ...
    if (isInsert && /\bdata-tektone-[\w-]*\s*=/.test(markup)) {
      throw new Error(`Svelte insert variant ${variantId} contains preview-only data-tektone attributes`);
    }
...
function svelteCssForVariant(scopedCss, variantId, tag) {
  ...
  const rewritten = chunks
    .join('\n')
    .replace(new RegExp(String.raw`\\[data-tektone-variant=["']${variantId}["']\\]\\s*>\\s*`, 'g'), '')
    .replace(new RegExp(String.raw`\\[data-tektone-variant=["']${variantId}["']\\][^{]*>\\s*`, 'g'), '')
    .replace(/:scope(?:\[[^\]]+\])?\s*>\s*/g, '')
    .replace(/:scope(?:\[[^\]]+\])?/g, tag)
    .split('\n')
...
function extractVariantCssChunks(css, variantId) {
  ...
  for (const line of lines) {
    if (line.includes(`[data-tektone-variant="${variantId}"]`) || line.includes(`[data-tektone-variant='${variantId}']`)) {
      collecting = true;
      depth = 0;
 ...
 * @param {string} opts.scriptsDir Path to the tektone scripts dir.
 ...
 *
 *   1. Locate the carbonize block (bracketed by `tektone-carbonize-start`
 *      and `tektone-carbonize-end`).
 ...
 *      `<div data-tektone-variant="N" style="display: contents"|...>` wrapper
...
async function runCarbonizeCleanup({ tmp, file, sessionId /* , variant */ }) {
  ...
  const startRe = new RegExp('[ \\t]*(?:<!--|\\{/\\*)\\s*tektone-carbonize-start\\s+' + sessionId + '\\s*(?:-->|\\*/\\})\\n');
  const endRe   = new RegExp('[ \\t]*(?:<!--|\\{/\\*)\\s*tektone-carbonize-end\\s+' + sessionId + '\\s*(?:-->|\\*/\\})\\n?');
  ...
  // 2. Unwrap the temporary `<div data-tektone-variant="N" ...>` placed
  // around the accepted content. For JSX targets, live-accept also adds an
  // outer `<div data-tektone-carbonize>` so the carbonize block and accepted
  // node occupy one child slot; strip that shell after the accepted node is
  // clean.
  body = unwrapDivAttributeWrapper(body, 'data-tektone-variant', { expandSingleLineContainer: true });
  body = unwrapDivAttributeWrapper(body, 'data-tektone-carbonize');

  // 3. Strip any `data-tektone-hoist-id` attributes the normalize step
  ...
  body = body.replace(/\s+data-tektone-hoist-id="[^"]*"/g, '');
