 ...
 *
 * Selector strategy: live-browser.js uses deterministic ids (`tektone-live-*`)
...
const BAR_ID = '#tektone-live-bar';
const GLOBAL_BAR_ID = '#tektone-live-global-bar';
const PICKER_ID = '#tektone-live-picker';
const EDIT_BADGE_ID = '#tektone-live-edit-badge';
const PENDING_DOCK_ID = '#tektone-live-pending-dock';
const STEER_CHAT_ID = '#tektone-live-page-chat';
const STEER_INPUT_ID = '#tektone-live-page-chat-input';
const PICK_TOGGLE = '#tektone-live-pick-toggle';
...
const INSERT_TOGGLE = '#tektone-live-insert-toggle';
const DETECT_TOGGLE = '#tektone-live-detect-toggle';
const DETECT_BADGE = '#tektone-live-detect-badge';
const DESIGN_TOGGLE = '#tektone-live-design-toggle';
const DESIGN_HOST = '#tektone-live-design-host';
const EXIT_BUTTON = '#tektone-live-exit';
const INSERT_INPUT_ID = '#tektone-live-insert-input';
const INSERT_CREATE_ID = '#tektone-live-insert-create';
const ANNOTATION_ID = '#tektone-live-annot';
const ANNOTATION_PINS_ID = '#tektone-live-annot-pins';
const ANNOTATION_CLEAR_ID = '#tektone-live-annot-clear';

/**
 * Wait for the live handshake to complete:
 *   - window.__TEKTONE_LIVE_INIT__ set
 *   - global bar mounted
...
export async function waitForHandshake(page, { timeout = 20_000 } = {}) {
  ...
  await page.waitForFunction(
    (sel) => Boolean(window.__tektoneLiveQuery?.(sel)),
    GLOBAL_BAR_ID,
    { timeout },
  );
...
export async function assertBottomBarIdle(page, { timeout = 5_000 } = {}) {
  await installLiveQueryHelpers(page);
  await page.waitForFunction(
    ({ ids }) => ids.every((sel) => Boolean(window.__tektoneLiveQuery(sel))),
    {
      ids: [
        GLOBAL_BAR_ID,
        PICK_TOGGLE,
  ...
  const snapshot = await page.evaluate(({ pickSel, insertSel, detectSel, designSel }) => {
    const q = window.__tektoneLiveQuery;
    return {
...
function installLiveQueryHelpersInPage() {
  window.__tektoneLiveQuery = (selector) => {
    const root = window.__TEKTONE_LIVE_CHROME_CORE__?.root?.()
      || window.__TEKTONE_LIVE_UI_ROOT__
      || null;
    return root?.querySelector?.(selector) || document.querySelector(selector);
  };
  window.__tektoneLiveQueryAll = (selector) => {
    const root = window.__TEKTONE_LIVE_CHROME_CORE__?.root?.()
      || window.__TEKTONE_LIVE_UI_ROOT__
      || null;
...
async function clickLiveControl(page, selector) {
  ...
  const clicked = await page.evaluate((sel) => {
    const el = window.__tektoneLiveQuery(sel);
...
async function readControlActive(page, selector) {
  await installLiveQueryHelpers(page);
  return page.evaluate((sel) => window.__tektoneLiveQuery(sel)?.dataset.active === 'true', selector);
}
...
async function ensureLiveControlActive(page, selector, active) {
  ...
  await page.waitForFunction(
    ({ sel, expected }) => window.__tektoneLiveQuery(sel)?.dataset.active === (expected ? 'true' : 'false'),
...
export async function runPickInsertToggleSmoke(page) {
  ...
  await page.waitForFunction(
    ({ pickSel, insertSel }) =>
      window.__tektoneLiveQuery(pickSel)?.dataset.active === 'true'
      && window.__tektoneLiveQuery(insertSel)?.dataset.active === 'false',
  ...
  await page.waitForFunction(
    ({ pickSel, insertSel }) =>
      window.__tektoneLiveQuery(pickSel)?.dataset.active === 'false'
      && window.__tektoneLiveQuery(insertSel)?.dataset.active === 'true',
    { pickSel: PICK_TOGGLE, insertSel: INSERT_TOGGLE },
    { timeout: 5_000 },
  );
...
export async function runDetectSmoke(page, { expectMinCount = 1 } = {}) {
  ...
  await page.waitForFunction(
    ({ badgeSel, expectMin }) => {
      const badge = window.__tektoneLiveQuery(badgeSel);
      const count = parseInt(badge?.textContent || '0', 10);
      const overlays = document.querySelectorAll('.tektone-overlay').length;
  ...
  await ensureLiveControlActive(page, PICK_TOGGLE, true);
  await page.waitForFunction(
    () => [...document.querySelectorAll('.tektone-overlay')]
  ...
  await page.waitForFunction(
    ({ badgeSel }) => {
      const badge = window.__tektoneLiveQuery(badgeSel);
      return document.querySelectorAll('.tektone-overlay').length === 0
...
export async function runDesignPanelSmoke(page, { title = '', rawText = '' } = {}) {
  ...
  await page.waitForFunction(
    ({ hostSel, titleText }) => {
      const host = window.__tektoneLiveQuery(hostSel);
  ...
  await page.evaluate((hostSel) => {
    const root = window.__tektoneLiveQuery(hostSel)?.shadowRoot;
    const raw = [...(root?.querySelectorAll('.tab') || [])].find((btn) => /Raw/i.test(btn.textContent || ''));
    raw?.click();
  }, DESIGN_HOST);
  await page.waitForFunction(
    ({ hostSel, expected }) => {
      const text = window.__tektoneLiveQuery(hostSel)?.shadowRoot?.textContent || '';
  ...
  await page.waitForFunction(
    ({ hostSel, toggleSel }) => {
      const panel = window.__tektoneLiveQuery(hostSel)?.shadowRoot?.querySelector('.panel');
      const toggle = window.__tektoneLiveQuery(toggleSel);
...
export async function clickExitLiveMode(page) {
  ...
  await page.waitForFunction(
    ({ barSel }) => {
      const bar = window.__tektoneLiveQuery?.(barSel);
      return window.__TEKTONE_LIVE_INIT__ === false && (!bar || !bar.isConnected);
    },
...
export async function drawAnnotationPinAndStroke(page, {
  ...
  await page.waitForFunction(
    (pinsSel) => Boolean(window.__tektoneLiveQuery(pinsSel)?.querySelector('input')),
    ANNOTATION_PINS_ID,
    { timeout: 5_000 },
  );
  await page.evaluate(({ pinsSel, value }) => {
    const input = window.__tektoneLiveQuery(pinsSel)?.querySelector('input');
  ...
  await page.waitForFunction(
    ({ pinsSel, expected }) => {
      const pins = window.__tektoneLiveQuery(pinsSel);
  ...
  await page.waitForFunction(
    ({ annotSel, clearSel }) => {
      const annot = window.__tektoneLiveQuery(annotSel);
      const clear = window.__tektoneLiveQuery(clearSel);
...
async function waitForAnnotationRect(page) {
  ...
  return page.evaluate((sel) => {
    const rect = window.__tektoneLiveQuery(sel).getBoundingClientRect();
    return {
      left: rect.left,
      top: rect.top,
      width: rect.width,
...
async function hideAnnotationOverlay(page) {
  await page.evaluate(() => {
    const annot = window.__tektoneLiveQuery('#tektone-live-annot');
...
async function ensurePickerActive(page) {
  ...
  const clicked = await page.evaluate((sel) => {
    const btn = window.__tektoneLiveQuery(sel);
    if (!btn) return false;
    btn.click();
    return true;
  }, PICK_TOGGLE_ID);
  ...
  await page.waitForFunction(
    (sel) => window.__tektoneLiveQuery(sel)?.dataset.active === 'true',
    PICK_TOGGLE_ID,
    { timeout: 5_000 },
  );
}
  ...
  try {
    ...
    if (process.env.TEKTONE_E2E_DEBUG) {
      const snapshot = await page.evaluate((barSel) => {
        ...
        const root = window.__TEKTONE_LIVE_CHROME_CORE__?.root?.() || window.__TEKTONE_LIVE_UI_ROOT__ || null;
        const bar = query(barSel);
        const toast = query('#tektone-live-toast');
        const wrapper = document.querySelector('[data-tektone-variants]');
        return {
          liveInit: window.__TEKTONE_LIVE_INIT__,
          adapter: window.__TEKTONE_LIVE_ADAPTER__,
          ...
          wrapper: wrapper ? { preview: wrapper.dataset.tektonePreview, count: wrapper.dataset.tektoneVariantCount, html: wrapper.outerHTML.slice(0, 600) } : null,
          debugState: window.__TEKTONE_LIVE_CHROME_CORE__?.debugState?.() || null,
          storage: localStorage.getItem('tektone-live-session'),
          scripts: document.querySelectorAll('script[data-tektone-live-script]').length,
  ...
  try {
    ...
    return await withTimeout(
      page.evaluate((barSel) => {
        const wrapper = window.__tektoneLiveQuery('[data-tektone-variants]');
        if (wrapper) {
          ...
          const idx = visible ? parseInt(visible.dataset.tektoneVariant || '0', 10) : 0;
          if (idx > 0) return idx;
        }
        const bar = window.__tektoneLiveQuery(barSel);
        if (!bar) return null;
...
export async function clickEditCopy(page) {
  ...
  await page.waitForFunction(
    () => window.__tektoneLiveQuery('[data-tektone-editable="true"]')?.isContentEditable === true,
    { timeout: 5_000 },
  );
}
...
export async function clickSaveEdit(page) {
  ...
  await page.waitForFunction(
    () => !window.__tektoneLiveQuery('[data-tektone-editable="true"]'),
    { timeout: 5_000 },
  );
}
...
async function clickEditBadgeButton(page, label) {
  const proxyRect = await page.evaluate((text) => {
    const proxies = [...document.querySelectorAll('[data-tektone-edit-badge-proxy="true"]')];
...
export async function assertApplyDockVisible(page, expectedCount, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    ({ dockSel, expected }) => {
      const dock = window.__tektoneLiveQuery(dockSel);
...
export async function preparePageForBarInteraction(page) {
  await page.evaluate(() => {
    for (const el of window.__tektoneLiveQueryAll('astro-dev-toolbar')) {
...
export async function waitForSteerInputFocused(page, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    (inputSel) => {
      const input = window.__tektoneLiveQuery(inputSel);
      const active = window.__TEKTONE_LIVE_CHROME_CORE__?.activeElementDeep?.()
...
export async function waitForSteerInputValue(page, value, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    ({ inputSel, value: expected }) => window.__tektoneLiveQuery(inputSel)?.value === expected,
...
export async function waitForSteerLocked(page, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    (sel) => window.__tektoneLiveQuery(sel)?.dataset.processing === 'true',
    STEER_CHAT_ID,
    { timeout },
  );
}
...
export async function waitForSteerUnlocked(page, { timeout = 15_000 } = {}) {
  await page.waitForFunction(
    (sel) => {
      const chat = window.__tektoneLiveQuery(sel);
      const input = window.__tektoneLiveQuery('#tektone-live-page-chat-input');
...
async function ensureToggleActive(page, selector, shouldBeActive) {
  ...
  await page.locator(selector).click({ timeout: 5_000 });
  await page.waitForFunction(
    ({ sel, active }) => window.__tektoneLiveQuery(sel)?.dataset.active === (active ? 'true' : 'false'),
...
export async function runInsertFlow(page, {
  ...
  await page.waitForFunction(() => {
    const line = window.__tektoneLiveQuery('#tektone-live-insert-line');
  ...
  await page.waitForFunction(
    ({ sel, value }) => window.__tektoneLiveQuery(sel)?.value === value,
