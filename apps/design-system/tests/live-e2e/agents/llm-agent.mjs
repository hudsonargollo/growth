 ...
 * TEKTONE_E2E_LLM_PROVIDER=deepseek. Override the model via { model } when
 * constructing, or via TEKTONE_E2E_LLM_MODEL at the call site.
 *
...
export const VARIANT_SYSTEM_INSTRUCTIONS = [
  'You are an automated subagent inside Tektone\'s live-mode test harness.',
  ...
  '- Insert mode variants must contain visible inserted content. Do not return empty roots, placeholder-only roots, inline style= attributes, or test hooks like <div data-tektone-e2e-variant="1"></div>.',
  ...
  '- Replace mode: for non-bare elements where the existing children must stay in place, add a harmless root attribute such as data-tektone-e2e-variant="1" or another non-copy styling hook so the markup is materially changed without changing visible text.',
  ...
  '- Do NOT emit the wrapping <div data-tektone-variant="N">. The orchestrator wraps your content.',
  '- Do NOT emit the outer <style data-tektone-css> tag. Only its contents go in scopedCss.',
...
export const MANUAL_EDIT_SYSTEM_INSTRUCTIONS = [
  '<role>',
  'You are an automated source-edit planner for one Tektone live manual_edit_apply batch or chunk.',
  '</role>',
  '',
  '<workflow>',
  ...
  '- Never copy live runtime scaffolding into sourceEdits: no contenteditable, data-tektone-* attributes, variant wrappers, live markers, <style>, <script>, comments, or generated browser attributes.',
  '</source_edit_rules>',
  '',
  'OUTPUT CONTRACT — return ONLY a JSON object with this exact shape. No prose, no code fences, no commentary:',
  '',
...
const STEER_SYSTEM_INSTRUCTIONS = [
  ...
  '- Never edit temporary preview or scratch paths such as node_modules/.tektone-live; Steer edits must land in the real app source file.',
 ...
 * @property {'anthropic' | 'deepseek'=} provider Override TEKTONE_E2E_LLM_PROVIDER.
...
export function resolveLlmAgentConfig(opts = {}, env = process.env) {
  ...
  if (provider === 'anthropic') {
    return {
      provider,
      model: opts.model || env.TEKTONE_E2E_LLM_MODEL || DEFAULT_ANTHROPIC_MODEL,
  ...
  if (provider === 'deepseek') {
    return {
      provider,
      model: opts.model || env.TEKTONE_E2E_LLM_MODEL || DEFAULT_DEEPSEEK_MODEL,
  ...
  throw new Error(`Unsupported TEKTONE_E2E_LLM_PROVIDER: ${provider}`);
}

function resolveProvider(opts, env) {
  const explicit = opts.provider || env.TEKTONE_E2E_LLM_PROVIDER;
      ...
      for (let attempt = 0; attempt < MANUAL_EDIT_RESPONSE_MAX_ATTEMPTS; attempt += 1) {
        ...
        if (isInsert) {
          userMessage = [
            ...
            'Do not use data-tektone-* attributes or empty test-hook-only roots.',
...
function validateVariantInnerHtml(html) {
  ...
  if (/\bdata-tektone-variants?\s*=/.test(html)) return 'must not include Tektone wrapper attributes';
...
export function validateInsertVariantOutput(parsed, event = {}) {
  for (const [i, variant] of parsed.variants.entries()) {
    ...
    if (/\bdata-tektone-[\w-]*\s*=/.test(html)) {
      return `insert variant ${i} contains preview-only data-tektone attributes`;
    }
