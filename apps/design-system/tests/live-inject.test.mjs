...
function runInject(cwd, configPath, args) {
  try {
    const out = execFileSync('node', [INJECT, ...args], {
      cwd,
      encoding: 'utf-8',
      env: { ...process.env, TEKTONE_LIVE_CONFIG: configPath },
...
function runInjectDefault(cwd, args) {
  try {
    const out = execFileSync('node', [INJECT, ...args], {
      cwd,
      encoding: 'utf-8',
      env: { ...process.env, TEKTONE_LIVE_CONFIG: '' },
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    return JSON.parse(out.trim());
  } catch (err) {
...
describe('live-inject — insert/remove round-trip preserves file bytes', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'tektone-inject-test-')); });
  afterEach(() => { rmSync(tmp, { recursive: true, force: true }); });

  it('reports .tektone/live/config.json as the default missing config path', () => {
    const result = runInjectDefault(tmp, ['--check']);

    assert.equal(result.ok, false);
    assert.equal(result.error, 'config_missing');
    assert.equal(result.path, join(realpathSync(tmp), '.tektone', 'live', 'config.json'));
  });

  it('uses .tektone/live/config.json without an environment override', () => {
    const original = `<html>
  <body>
    <p>Content</p>
  </body>
...
`;
    ...
    const configDir = join(tmp, '.tektone', 'live');
...
`;
  ...
  it('round-trips through CSP-meta patch and revert (insert mutates the meta tag, remove restores it)', () => {
    ...
    // and stashes the original directives in `data-tektone-csp-original`.
...
`;
    ...
    assert.equal((afterInject.match(/tektone-live-start/g) || []).length, 1, 'reinjection should leave one live block');
...
`;
  ...
  it('preserves the character after an insertAfter anchor with no trailing newline (#227)', () => {
    ...
    assert.ok(
      afterInject.includes('<!-- tektone-live-end -->\nX</head>'),
