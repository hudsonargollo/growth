...
import {
  ...
  getLiveServerPath,
  getLiveSessionsDir,
} from '../skill/scripts/lib/tektone-paths.mjs';
...
function startServer(port = 8499, { cwd = REPO_ROOT, env = {} } = {}) {
  return new Promise((resolve, reject) => {
    const proc = spawn('node', [SERVER_SCRIPT, '--port=' + port], {
      ...
      env: { ...process.env, TEKTONE_LIVE_COPY_AGENT: 'off', ...env },
    });
    let output = '';
...
it('gitignores local Tektone runtime artifacts', () => {
  const ignored = execFileSync('git', [
    'check-ignore',
    '.tektone/live/manual-edit-apply-transaction.json',
    '.tektone/live/manual-edit-evidence/example.json',
    '.tektone/hook.cache.json',
    '.tektone/live/deferred-svelte-component-accepts.json',
  ], { cwd: REPO_ROOT, encoding: 'utf-8' });
  assert.match(ignored, /\.tektone\/live\/manual-edit-apply-transaction\.json/);
  assert.match(ignored, /\.tektone\/live\/manual-edit-evidence\/example\.json/);
  assert.match(ignored, /\.tektone\/hook\.cache\.json/);
  assert.match(ignored, /\.tektone\/live\/deferred-svelte-component-accepts\.json/);
});
...
describe('live-server integration', () => {
  ...
  before(async () => {
    ...
    // never land in the real repo's `.tektone/live/sessions/`. Those would
    // otherwise be replayed into the poll queue on the next real `live` run.
    serverCwd = mkdtempSync(join(tmpdir(), 'tektone-live-server-'));
    ...
    writeFileSync(join(serverCwd, 'package.json'), JSON.stringify({ name: 'tektone' }));
  ...
  it('/live.js injects the canonical command vocabulary', async () => {
    // live-browser.js builds its action picker from window.__TEKTONE_VOCAB__
    ...
    assert.match(body, /window\.__TEKTONE_VOCAB__\s*=/);
    const injected = JSON.parse(body.match(/window\.__TEKTONE_VOCAB__\s*=\s*(\[.*?\]);/s)[1]);
  ...
  it('/live.js serves script with token injected', async () => {
    ...
    assert.ok(text.includes('__TEKTONE_TOKEN__'));
    assert.ok(text.includes(server.token));
    assert.ok(text.includes('__TEKTONE_PORT__'));
    const sessionHelperIndex = text.indexOf('__TEKTONE_LIVE_SESSION__');
    const browserInitIndex = text.indexOf('__TEKTONE_LIVE_INIT__');
  ...
  it('/design-system.json reads DESIGN.md plus .tektone/design.json', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-design-system-'));
    let designServer;
    try {
...
`);
      ...
      mkdirSync(join(tmp, '.tektone'), { recursive: true });
  ...
  it('/design-system.json falls back to legacy root DESIGN.json', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-design-system-legacy-'));
    let designServer;
    try {
      writeFileSync(join(tmp, 'DESIGN.md'), `---
name: Legacy System
...
`);
  ...
  it('/manual-edit-commit runs the batched AI apply path and clears successful entries', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-commit-server-'));
    let commitServer;
    try {
      ...
      commitServer = await startServer(8522, {
        cwd: tmp,
        env: {
          TEKTONE_LIVE_COPY_AGENT: 'mock',
          TEKTONE_LIVE_COPY_AGENT_MOCK_DELAY_MS: '400',
          TEKTONE_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
            status: 'done',
  ...
  it('/manual-edit-commit async mode returns immediately and reports completion through status/SSE activity', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-commit-server-async-'));
    let asyncServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
      ...
      asyncServer = await startServer(8546, {
        ...
        env: {
          TEKTONE_LIVE_COPY_AGENT: 'mock',
          TEKTONE_LIVE_COPY_AGENT_MOCK_DELAY_MS: '300',
          TEKTONE_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
            status: 'done',
  ...
  it('/manual-edit-commit routes through the chat agent poll loop when configured', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-commit-chat-'));
    let chatServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
      ...
      chatServer = await startServer(8524, {
        cwd: tmp,
        env: { TEKTONE_LIVE_COPY_AGENT: 'chat' },
      });
  ...
  it('/manual-edit-commit includes compact source candidates in chat Apply events', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-commit-chat-candidates-'));
    let candidateServer;
    try {
      mkdirSync(join(tmp, 'site/scripts/components'), { recursive: true });
      writeFileSync(join(tmp, 'site/scripts/data.js'), [
        "export const skillFocusAreas = { tektone: [",
  ...
  it('/manual-edit-commit rejects malformed chat Apply results without rolling back before retry', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-commit-chat-invalid-result-'));
    let chatServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
  ...
  it('/manual-edit-commit chunks chat Apply events by op count and aggregates replies', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-commit-chat-chunks-'));
    let chunkServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
      ...
      chunkServer = await startServer(8528, {
        cwd: tmp,
        env: {
          TEKTONE_LIVE_COPY_AGENT: 'chat',
          TEKTONE_LIVE_MANUAL_EDIT_CHUNK_SIZE: '3',
        },
      });
  ...
  it('/manual-edit-commit keeps fitting multi-op entries together across chat chunks', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-commit-chat-entry-chunks-'));
    let chunkServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
  ...
  it('/manual-edit-commit splits one multi-op entry and clears it only after every chunk applies', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-commit-chat-split-entry-'));
    let splitServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
  ...
  it('/manual-edit-commit rolls back a split entry when a later chat chunk fails', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-commit-chat-chunk-fail-'));
    let failServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
  ...
  it('/manual-edit-commit keeps entries staged when the chat agent does not ack', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-commit-timeout-'));
    let timeoutServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      const sourcePath = join(tmp, 'src', 'page.html');
      ...
      timeoutServer = await startServer(8525, {
        ...
        env: {
          TEKTONE_LIVE_COPY_AGENT: 'chat',
          TEKTONE_LIVE_APPLY_EVENT_HARD_TIMEOUT_MS: '300',
          TEKTONE_LIVE_APPLY_EVENT_SOFT_DEADLINE_MS: '250',
        },
      });
  ...
  it('/manual-edit-commit repairs post-apply validation failures instead of rolling back', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-repair-success-'));
    let repairServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
  ...
  it('/manual-edit-commit asks for a decision after repeated repair failures', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-repair-decision-'));
    let decisionServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
      writeFileSync(join(tmp, 'package.json'), '{"type":"module"}\n');
  ...
  it('/manual-edit-discard cancels leased chat Apply events instead of redelivering them', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-discard-apply-'));
  ...
  it('/manual-edit-commit rolls back abandoned chunk transactions after server restart', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-abandoned-transaction-'));
    let abandonedServer;
    let restarted;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
  ...
  it('/manual-edit-discard only cancels in-flight Apply events for the discarded page', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-discard-page-scope-'));
    let pageScopeServer;
    try {
      mkdirSync(join(tmp, 'src'), { recursive: true });
  ...
  it('/manual-edit-discard returns discarded entries so the browser can restore visible text', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-discard-server-'));
    let discardServer;
    try {
  ...
  it('/manual-edit-stash rejects a corrupt pending buffer instead of overwriting it', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-manual-stash-corrupt-'));
    let stashServer;
    try {
  ...
  it('redelivers an unacknowledged browser event after helper server restart', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'tektone-server-restart-'));
    let firstServer;
    let restarted;
    try {
  ...
  it('/source reads project files with valid token', async () => {
    ...
    assert.equal(res.status, 200);
    const text = await res.text();
    assert.ok(text.includes('"tektone"'));
  });
