...
describe('live-browser.js regression guards', () => {
  ...
  it('uses a Svelte-gated painted-ancestor crop proxy for shader capture', () => {
    ...
    assert.match(
      SOURCE,
      /function shouldUseAncestorCropShaderProxy\(el\) \{[\s\S]{0,260}?window\.__TEKTONE_LIVE_ADAPTER__[\s\S]{0,280}?currentPreviewMode === 'svelte-component' \|\| svelteComponentSession[\s\S]{0,260}?dataset\?\.tektonePreview === 'svelte-component';/,
  ...
  it('pick mode preference persists in localStorage', () => {
    assert.match(
      SOURCE,
      /const INTERACTION_PREFS_KEY = 'tektone-live-interaction';[\s\S]{0,3000}?function saveInteractionPrefs\(\)/,
  ...
  it('detect mode shows an empty result toast once per requested scan', () => {
    ...
    assert.match(
      SOURCE,
      /window\.postMessage\(\{ source: 'tektone-command', action: 'remove' \}, '\*'\);[\s\S]{0,80}?activeDetectScanId = null;[\s\S]{0,80}?pendingDetectScanId = null;/,
  ...
  it('insert mode UI and generate payload guards', () => {
    ...
    assert.match(SOURCE, /data-tektone-insert-placeholder/, 'placeholder element must be marked');
    assert.match(
      SOURCE,
      /showHighlight\(el\)[\s\S]{0,120}?data-tektone-insert-placeholder/,
    ...
    assert.match(SOURCE, /data-tektone-placeholder-resize/, 'placeholder edge handles on annotation overlay');
