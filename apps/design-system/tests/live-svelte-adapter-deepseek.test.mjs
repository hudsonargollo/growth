  ...
  it('covers live chrome, replace, edit, annotations, insert, refresh recovery, accept/discard, and exit', async (t) => {
    ...
    try {
      ...
      const afterExit = await page.evaluate(() => ({
        ...
        hasGlobalBar: Boolean(window.__tektoneLiveQuery?.('#tektone-live-global-bar')),
      }));
...
function loadConsolidatedFixture() {
  ...
  fixture.runtime = {
    ...
    stateProbe: {
      ...
      windowProperty: '__tektoneStatefulMounts',
    ...
    steer: {
      ...
      expectSelector: 'h1.hero-title[data-tektone-steer="e2e"]',
      expectSourceContains: 'data-tektone-steer="e2e"',
    },
    probe: {
      expectLiveInit: true,
...
async function runReplaceDiscardRecoveryFlow({ page, tmp, evidence }) {
  ...
  assert.match(session.previewFile, /^node_modules\/\.tektone-live\/[^/]+\/manifest\.json$/);
...
async function runEditCopyFlow({ page, tmp, live, evidence }) {
  ...
  assert.doesNotMatch(sourceAfter, /contenteditable|data-tektone-editable|data-tektone-original-text|tektone-variants/, 'manual Apply does not leak live scaffolding');
}
...
async function runAcceptReplaceFlow({ page, tmp, evidence }) {
  ...
  assert.doesNotMatch(source, /data-tektone-variants|tektone-variants-start|data-tektone-variant=/, 'accepted replace source is clean');
...
async function runInsertFlowWithRecovery({ page, tmp, evidence }) {
  ...
  assert.doesNotMatch(source, /data-tektone-variants|tektone-variants-start|data-tektone-variant=/, 'accepted insert source is clean');
...
async function assertSvelteAdapterInjection(tmp) {
  ...
  assert.doesNotMatch(appHtml, /live\.js|tektone-live-start/, 'Svelte adapter leaves src/app.html untouched');
  assert.match(layout, /TektoneLiveRoot|tektone-live-svelte-start/, 'Svelte adapter patches layout with live root');
}

async function assertShadowChrome(page) {
  const result = await page.evaluate(() => {
    const host = document.getElementById('tektone-live-root');
    const root = host?.shadowRoot;
    return {
      hasHost: Boolean(host),
      hasShadowRoot: Boolean(root),
      hasGlobalBar: Boolean(root?.getElementById('tektone-live-global-bar')),
      documentHasGlobalBar: Boolean(document.querySelector('#tektone-live-global-bar')),
    };
  });
...
async function assertHostileCssIsolation(page) {
  ...
  const hostileStyle = await page.addStyleTag({
    content: `
      button, div, input, svg, #tektone-live-root, tektone-live-root * {
...
async function waitForGlobalBarStable(page) {
  ...
  await page.waitForFunction(
    () => {
      const bar = window.__tektoneLiveQuery?.('#tektone-live-global-bar');
      if (!bar) return false;
...
async function assertSelectedElementChrome(page) {
  ...
  const result = await page.evaluate(() => {
    ...
    const bar = q('#tektone-live-bar');
    const picker = q('#tektone-live-picker');
    const badge = q('#tektone-live-edit-badge');
    const highlight = q('#tektone-live-highlight');
    const input = q('#tektone-live-input');
    return {
      hasBar: Boolean(bar),
...
async function selectAction(page, label) {
  ...
  const opened = await page.evaluate(() => {
    const bar = window.__tektoneLiveQuery('#tektone-live-bar');
  ...
  await page.waitForFunction(
    (expected) => {
      const picker = window.__tektoneLiveQuery('#tektone-live-picker');
...
async function clickEditBadgeAction(page, label) {
  const clicked = await page.evaluate((expected) => {
    const badge = window.__tektoneLiveQuery('#tektone-live-edit-badge');
...
async function clickPendingTrash(page) {
  ...
  const clicked = await page.evaluate(() => {
    const dock = window.__tektoneLiveQuery('#tektone-live-pending-dock');
...
async function waitForSvelteAcceptComplete(tmp, session, { timeout }) {
  ...
  while (Date.now() < deadline) {
    ...
    if (
      ...
      && !/data-tektone-variants|tektone-variants-start|data-tektone-variant=/.test(source)
    ) {
      return;
    }
...
function assertCompletedRealSource(tmp, id) {
  ...
  assert.match(snapshot.previewFile || '', /node_modules\/\.tektone-live\/[^/]+\/manifest\.json/);
}
...
function readSessionSnapshot(tmp, id) {
  const file = join(tmp, '.tektone/live/sessions', `${id}.snapshot.json`);
...
async function assertNoCyclingZero(page) {
  const text = await page.evaluate(() => window.__tektoneLiveQuery('#tektone-live-bar')?.textContent || '');
...
async function readStateProbe(page) {
  return page.evaluate(() => ({
    ...
    mounts: window.__tektoneStatefulMounts,
  }));
}
...
function assertFinalSourceClean(tmp) {
  const source = readFileSync(join(tmp, ROUTE_FILE), 'utf-8');
  assert.doesNotMatch(source, /node_modules\/\.tektone-live|data-tektone-variants|tektone-variants-start|data-tektone-variant=/);
}
...
function readJournalEvents(tmp) {
  const dir = join(tmp, '.tektone/live/sessions');
...
function createEvidenceWriter({ page, tmp, live, consoleErrors }) {
  ...
  return {
    async capture(name) {
      ...
      try {
        const dom = await page.evaluate(() => {
          ...
          return {
            ...
            liveState: window.__TEKTONE_LIVE_CHROME_CORE__?.debugState?.() || null,
            localSession: localStorage.getItem('tektone-live-session'),
          };
        });
      ...
      copyDirIfExists(join(tmp, '.tektone/live/sessions'), join(dir, 'sessions'));
...
function listPreviewFiles(tmp) {
  const root = join(tmp, 'node_modules/.tektone-live');
...
function createArtifactRoot() {
  const explicit = process.env.TEKTONE_SVELTE_DEEPSEEK_ARTIFACT_DIR;
