...
import {
  ...
  cleanup,
  loadLock,
} from '../skill/scripts/cleanup-deprecated.mjs';

function makeTmpDir() {
  return mkdtempSync(join(tmpdir(), 'tektone-cleanup-test-'));
}
...
describe('cleanup-deprecated', () => {
  ...
  describe('isTektoneSkill', () => {
    it('returns true when SKILL.md mentions tektone', () => {
      const dir = writeSkill(tmp, '.claude', 'arrange', 'Invoke /tektone first.');
      assert.equal(isTektoneSkill(dir), true);
    });

    it('returns false when SKILL.md does not mention tektone', () => {
      const dir = writeSkill(tmp, '.claude', 'arrange', 'This is my custom arrange skill.');
      assert.equal(isTektoneSkill(dir), false);
    });

    it('returns false for non-existent directory', () => {
      assert.equal(isTektoneSkill(join(tmp, 'nope')), false);
    });

    it('returns true when lock source says pbakaus/tektone, even if SKILL.md never mentions it', () => {
      const dir = writeSkill(tmp, '.claude', 'harden', 'A custom skill with no pack mention.');
      const lock = {
        skills: { harden: { source: 'pbakaus/tektone' } },
      };
      assert.equal(isTektoneSkill(dir, { skillName: 'harden', lock }), true);
    });
    ...
    it('returns false when lock source is a different pack', () => {
      ...
      assert.equal(isTektoneSkill(dir, { skillName: 'harden', lock }), false);
    });

    it('falls back to SKILL.md content when no lock entry exists', () => {
      const dir = writeSkill(tmp, '.claude', 'harden', 'Invoke /tektone to harden.');
      const lock = { skills: {} };
      assert.equal(isTektoneSkill(dir, { skillName: 'harden', lock }), true);
    });
  });
  ...
  describe('loadLock', () => {
    ...
    it('parses skills-lock.json when present', () => {
      const lock = { version: 1, skills: { arrange: { source: 'pbakaus/tektone' } } };
  ...
  describe('removeDeprecatedSkills', () => {
    it('deletes tektone-owned deprecated skill directories', () => {
      writeSkill(tmp, '.claude', 'arrange', 'Invoke /tektone first.');
      writeSkill(tmp, '.claude', 'normalize', 'Run tektone teach.');
    ...
    it('does NOT delete skills that do not mention tektone', () => {
    ...
    it('deletes i-prefixed variants', () => {
      writeSkill(tmp, '.cursor', 'i-normalize', 'Invoke /tektone first.');
      const deleted = removeDeprecatedSkills(tmp);
    ...
    it('cleans across multiple harness directories', () => {
      writeSkill(tmp, '.claude', 'onboard', 'Run tektone teach first.');
      writeSkill(tmp, '.agents', 'onboard', 'Run tektone teach first.');
      writeSkill(tmp, '.cursor', 'onboard', 'Run tektone teach first.');
      const deleted = removeDeprecatedSkills(tmp);
      assert.equal(deleted.length, 3);
    });

    it('leaves non-deprecated skills alone', () => {
      writeSkill(tmp, '.claude', 'my-custom-skill', 'Invoke /tektone first.');
      writeSkill(tmp, '.claude', 'arrange', 'Invoke /tektone first.');
      const deleted = removeDeprecatedSkills(tmp);
    ...
    it('deletes a stock v2.x harden skill with no lock file via the fingerprint fallback', () => {
      ...
      // v2.x harden SKILL.md never contained the word "tektone", but
    ...
    it('deletes skills whose SKILL.md never mentions tektone when the lock claims them', () => {
      // Reproduces the "orphan dir" bug: the old SKILL.md bodies described
      // each skill on its own merits and never said the word "tektone",
      ...
      const lock = {
        version: 1,
        skills: { harden: { source: 'pbakaus/tektone', sourceType: 'github', computedHash: 'x' } },
      };
      writeFileSync(join(tmp, 'skills-lock.json'), JSON.stringify(lock), 'utf-8');
      const deleted = removeDeprecatedSkills(tmp);
      assert.equal(deleted.length, 1);
    ...
    it('handles symlinks to deprecated skills', () => {
      ...
      const canonical = writeSkill(tmp, '.agents', 'extract', 'Use tektone extract.');
  ...
  describe('cleanSkillsLock', () => {
    it('removes tektone-owned deprecated entries', () => {
      const lock = {
        version: 1,
        skills: {
          arrange: { source: 'pbakaus/tektone', sourceType: 'github', computedHash: 'abc' },
          tektone: { source: 'pbakaus/tektone', sourceType: 'github', computedHash: 'def' },
      ...
      assert.ok(updated.skills.tektone); // not deprecated
    ...
    it('removes i-prefixed entries', () => {
      const lock = {
        version: 1,
        skills: {
          'i-arrange': { source: 'pbakaus/tektone', sourceType: 'github', computedHash: 'abc' },
          'i-normalize': { source: 'pbakaus/tektone', sourceType: 'github', computedHash: 'def' },
        },
      };
      writeFileSync(join(tmp, 'skills-lock.json'), JSON.stringify(lock), 'utf-8');
      const removed = cleanSkillsLock(tmp);
  ...
  describe('cleanup (integration)', () => {
    it('cleans both files and lock entries in one pass', () => {
      ...
      writeSkill(tmp, '.claude', 'arrange', 'Invoke /tektone.');
      writeSkill(tmp, '.agents', 'arrange', 'Invoke /tektone.');
      writeSkill(tmp, '.claude', 'extract', 'Run tektone extract.');

      // Set up lock file
      const lock = {
        version: 1,
        skills: {
          arrange: { source: 'pbakaus/tektone', sourceType: 'github', computedHash: 'a' },
          extract: { source: 'pbakaus/tektone', sourceType: 'github', computedHash: 'b' },
          tektone: { source: 'pbakaus/tektone', sourceType: 'github', computedHash: 'c' },
        },
      };
      writeFileSync(join(tmp, 'skills-lock.json'), JSON.stringify(lock), 'utf-8');
    ...
    it('is a no-op when nothing needs cleaning', () => {
      writeSkill(tmp, '.claude', 'my-custom-skill', 'Invoke /tektone.');
      const result = cleanup(tmp);
