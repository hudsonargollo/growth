...
describe('live reference authoring contract', () => {
  it('keeps the live prompt focused on the foreground poll loop', () => {
    ...
    const manualAgentMd = readFileSync(join(ROOT, 'skill/agents/tektone-manual-edit-applier.md'), 'utf-8');
    ...
    assert.doesNotMatch(liveMd, /TEKTONE_LIVE_COPY_AGENT|mock/);
    ...
    assert.match(liveMd, /delegate source edits to `tektone_manual_edit_applier`/);
    ...
    assert.match(manualAgentMd, /codex-name: tektone_manual_edit_applier/);
  ...
  it('passes cssAuthoring into the LLM E2E agent instead of hard-coding scoped CSS', () => {
    ...
    assert.doesNotMatch(
      llmAgent,
      /with @scope \(\[data-tektone-variant=/,
