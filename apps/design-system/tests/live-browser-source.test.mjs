...
describe('live-browser source contracts', () => {
  it('saves copy edits to the staged buffer with rich AI context', () => {
    ...
    assert.match(
      ...
      'Discard restore should parse Tektone document refs instead of treating them as raw CSS selectors',
    );
  ...
  it('keeps sendEvent fire-and-forget by default while accept/discard opt into rejection', () => {
    assert.match(
      SOURCE,
      /function sendEvent\(msg, opts\)[\s\S]*if \(opts && opts\.throwOnError\) \{[\s\S]*console\.error\('\[tektone\] Failed to send event:', err\);[\s\S]*throw err;[\s\S]*\}[\s\S]*console\.debug\('\[tektone\] Dropped optional live event:', err\);[\s\S]*return null;/,
  ...
  it('waits for post-carbonize completion before final accepted DOM cleanup', () => {
    ...
    assert.match(
      SOURCE,
      /function acceptedDomAlreadyClean\(pending\)[\s\S]*?matches\.length > 0[\s\S]*?matches\.every[\s\S]*?data-tektone-carbonize/,
      'accepted DOM should not be considered clean while any matching root is still inside a carbonize wrapper',
    );
    assert.match(
      SOURCE,
      /function findAcceptedRuntimeWrappers\(sessionId\)[\s\S]*?querySelectorAll\('\[data-tektone-variants=[\s\S]*?querySelectorAll\('\[data-tektone-carbonize=/,
