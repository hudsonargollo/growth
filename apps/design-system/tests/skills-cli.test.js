/**
 * End-to-end tests for `tektone skills` subcommands.
 *
 ...
 * `bun run test:cli-remote-e2e` (TEKTONE_CLI_REMOTE_E2E=1), skipping
 * gracefully when tektone.design is unreachable.
 */
...
import { migrateUnprefixTektone } from '../cli/bin/commands/skills.mjs';
...
function createFakeSkills(root, skills = ['audit', 'polish', 'tektone'], providers = ['.claude']) {
  for (const provider of providers) {
    for (const skill of skills) {
      ...
      writeFileSync(join(skillDir, 'SKILL.md'), [
        ...
        'Use the tektone skill for setup.',
      ].join('\n'));
    }
  }
}
...
function createFakeLinkSource(root, providers = ['.claude']) {
  for (const provider of providers) {
    writeSkill(join(root, '.tektone', 'dist', 'universal'), provider, 'tektone');
  }
}
...
function createFakeUniversalBundle(root, providers = ['.claude', '.agents', '.cursor']) {
  ...
  for (const provider of providers) {
    const skillDir = join(bundleRoot, provider, 'skills', 'tektone');
 ...
 * skill lives at `<prefix>tektone`. Optionally drop in a third-party skill
...
function createPrefixedInstall(root, { prefix = 'i-', providers = ['.claude'], foreign = null } = {}) {
  for (const provider of providers) {
    writeSkill(root, provider, `${prefix}tektone`);
...
// Remote e2e blocks (real bundle downloads from tektone.design) run only
// under `bun run test:cli-remote-e2e` (TEKTONE_CLI_REMOTE_E2E=1). The default
...
const WANT_CLI_REMOTE_E2E = process.env.TEKTONE_CLI_REMOTE_E2E === '1';
let bundleReachable = false;
if (WANT_CLI_REMOTE_E2E) {
  try {
    execSync('curl -sfIL --max-time 10 https://tektone.design/api/download/bundle/universal -o /dev/null', { stdio: 'pipe' });
...
describe('skills install: already-installed detection', () => {
  test('detects tektone sentinel and bails', () => {
  ...
  test('detects prefixed i-tektone', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-'));
    execSync('git init', { cwd: tmp });

    const skillDir = join(tmp, '.cursor', 'skills', 'i-tektone');
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(join(skillDir, 'SKILL.md'), '---\nname: i-tektone\n---\n');
...
describe('skills link: submodule installs', () => {
  test('creates relative skill symlinks from dist/universal', () => {
    ...
    const output = run('skills link --source=.tektone --providers=claude,cursor -y', { cwd: tmp });
    expect(output).toContain('Linked tektone into: .claude, .cursor');

    for (const provider of ['.claude', '.cursor']) {
      const dest = join(tmp, provider, 'skills', 'tektone');
      const src = join(tmp, '.tektone', 'dist', 'universal', provider, 'skills', 'tektone');
  ...
  test('is idempotent when links already point at the same source', () => {
    ...
    run('skills link --source=.tektone --providers=claude -y', { cwd: tmp });
    const before = readlinkSync(join(tmp, '.claude', 'skills', 'tektone'));
    const output = run('skills link --source=.tektone --providers=claude -y', { cwd: tmp });

    expect(output).toContain('already linked');
    expect(readlinkSync(join(tmp, '.claude', 'skills', 'tektone'))).toBe(before);
  ...
  test('does not overwrite an existing real skill unless forced', () => {
    ...
    execSync('git init', { cwd: tmp });
    createFakeLinkSource(tmp);
    writeSkill(tmp, '.claude', 'tektone');

    expect(() => run('skills link --source=.tektone --providers=claude -y', { cwd: tmp })).toThrow();
    const dest = join(tmp, '.claude', 'skills', 'tektone');
    expect(lstatSync(dest).isSymbolicLink()).toBe(false);

    const output = run('skills link --source=.tektone --providers=claude -y --force', { cwd: tmp });
  ...
  test('maps codex and rovo-dev provider aliases to their install folders', () => {
    ...
    run('skills link --source=.tektone --providers=codex,rovo-dev -y', { cwd: tmp });

    expect(lstatSync(join(tmp, '.agents', 'skills', 'tektone')).isSymbolicLink()).toBe(true);
    expect(lstatSync(join(tmp, '.rovodev', 'skills', 'tektone')).isSymbolicLink()).toBe(true);
  ...
  test('deduplicates providers that share one skills directory', () => {
    ...
    run('skills link --source=.tektone --providers=claude,codex -y', { cwd: tmp });

    const dest = join(tmp, '.agents', 'skills', 'tektone');
    const src = join(tmp, '.tektone', 'dist', 'universal', '.claude', 'skills', 'tektone');
    expect(lstatSync(dest).isSymbolicLink()).toBe(true);
    expect(realpathSync(dest)).toBe(realpathSync(src));
...
// when each command was its own skill). migrateUnprefixTektone retires any
// old `<prefix>tektone` install back to the canonical `tektone`, so an
...
describe('skills: unprefix migration', () => {
  test('renames i-tektone back to tektone across every provider', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-mig-'));
    createPrefixedInstall(tmp, { prefix: 'i-', providers: ['.claude', '.cursor'] });

    const migrated = migrateUnprefixTektone(tmp);
    expect(migrated).toBe(2); // one skill x two providers

    for (const provider of ['.claude', '.cursor']) {
      const skills = readdirSync(join(tmp, provider, 'skills'));
      expect(skills).toContain('tektone');
      expect(skills).not.toContain('i-tektone');
    }

    rmSync(tmp, { recursive: true, force: true });
  });

  test('migrates a custom prefix too (x-tektone)', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-mig-x-'));
    createPrefixedInstall(tmp, { prefix: 'x-' });

    expect(migrateUnprefixTektone(tmp)).toBe(1);
    expect(readdirSync(join(tmp, '.claude', 'skills'))).toContain('tektone');
  ...
  test('REGRESSION: never touches third-party skills, even ones starting with i-', () => {
    ...
    // A foreign skill that shares the i- prefix but is NOT tektone.
    createPrefixedInstall(tmp, { prefix: 'i-', foreign: 'i-cool-skill' });

    const migrated = migrateUnprefixTektone(tmp);
    expect(migrated).toBe(1); // only i-tektone
  ...
  test('leaves a clean tektone install alone (no-op)', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-mig-clean-'));
    createFakeSkills(tmp, ['tektone'], ['.claude']);

    expect(migrateUnprefixTektone(tmp)).toBe(0);
    expect(readdirSync(join(tmp, '.claude', 'skills'))).toContain('tektone');

    rmSync(tmp, { recursive: true, force: true });
  });

  test('leaves the legacy teach-tektone name alone (cleanup owns that)', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-mig-legacy-'));
    createFakeSkills(tmp, ['teach-tektone'], ['.claude']);

    expect(migrateUnprefixTektone(tmp)).toBe(0);
    expect(readdirSync(join(tmp, '.claude', 'skills'))).toContain('teach-tektone');
...
describe('skills install/update: local universal bundle e2e', () => {
  test('installs provider-specific skills into a fresh project', () => {
    ...
    const output = run('skills install -y --providers=claude,codex,cursor', {
      cwd: tmp,
      env: { ...process.env, TEKTONE_BUNDLE_PATH: bundleRoot },
    });
    expect(output).toContain('Done!');

    for (const provider of ['.claude', '.agents', '.cursor']) {
      const skillDir = join(tmp, provider, 'skills', 'tektone');
  ...
  test('updates stale copied skills from the local bundle', () => {
    ...
    const skillDir = join(tmp, '.claude', 'skills', 'tektone');
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(join(skillDir, 'SKILL.md'), '---\nname: tektone\nstale: true\n---\nOld content.\n');
  ...
  test('--force reinstall over an old prefixed install lands on canonical tektone', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'imp-test-local-force-'));
    execSync('git init', { cwd: tmp });
    const bundleRoot = createFakeUniversalBundle(tmp, ['.claude']);
    const prefixed = join(tmp, '.claude', 'skills', 'i-tektone');
...
describeRemote('skills update: refreshes from the production universal bundle', () => {
  ...
  beforeAll(() => {
    ...
    // Stale tektone skill that the update should overwrite with fresh,
    // compiled content. v3.0 ships a single `tektone` skill (with
  ...
  test('downloads the bundle and refreshes the tektone skill', () => {
    const output = run('skills update -y', { cwd: tmp });
    expect(output).toContain('Updated');

    // The skill now carries fresh, compiled content (no 'stale: true').
    const content = readFileSync(join(tmp, '.claude', 'skills', 'tektone', 'SKILL.md'), 'utf8');
    expect(content).not.toContain('stale: true');
  ...
  test('refreshed skill ships its compiled scripts directory', () => {
    ...
    expect(existsSync(join(tmp, '.claude', 'skills', 'tektone', 'scripts'))).toBe(true);
  });
});
...
describeRemote('skills install: production universal bundle download', () => {
  ...
  test('--force reinstall over an old prefixed install lands on canonical tektone', () => {
    ...
    // retire i-tektone so we are left with the canonical name only.
    const prefixed = join(tmp, '.claude', 'skills', 'i-tektone');
    mkdirSync(prefixed, { recursive: true });
    writeFileSync(join(prefixed, 'SKILL.md'), '---\nname: i-tektone\n---\n');
