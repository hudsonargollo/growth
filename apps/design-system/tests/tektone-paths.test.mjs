/**
 * Tests for project-local Tektone path resolution.
 * Run with: node --test tests/tektone-paths.test.mjs
 */
...
describe('tektone project paths', () => {
  let tmp;

  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'tektone-paths-'));
  });
  ...
  it('resolves the generated design sidecar under .tektone', () => {
    assert.equal(getDesignSidecarPath(tmp), join(tmp, '.tektone', 'design.json'));
  ...
  it('uses .tektone/live/config.json as the default live config path', () => {
  ...
  it('falls back to legacy scripts/config.json when no new live config exists', () => {
    const scriptsDir = join(tmp, 'skills', 'tektone', 'scripts');
  ...
  it('lets TEKTONE_LIVE_CONFIG override both new and legacy config locations', () => {
    ...
    assert.equal(
      resolveLiveConfigPath({ cwd: tmp, scriptsDir: join(tmp, 'scripts'), env: { TEKTONE_LIVE_CONFIG: 'custom-live-config.json' } }),
      override,
    );
  });

  it('places live server, session, and annotation state under .tektone/live', () => {
    assert.equal(getLiveServerPath(tmp), join(tmp, '.tektone', 'live', 'server.json'));
    assert.equal(getLiveSessionsDir(tmp), join(tmp, '.tektone', 'live', 'sessions'));
    assert.equal(getLiveAnnotationsDir(tmp), join(tmp, '.tektone', 'live', 'annotations'));
  });
