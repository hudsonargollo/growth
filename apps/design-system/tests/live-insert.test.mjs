...
describe('buildInsertWrapperLines', () => {
  it('produces an insert scaffold without an original variant', () => {
    ...
    assert.match(joined, /tektone-variants-start abc12345/);
    assert.match(joined, /data-tektone-variants="abc12345"/);
    assert.match(joined, /data-tektone-mode="insert"/);
    assert.match(joined, /Variants: insert below this line/);
    assert.doesNotMatch(joined, /data-tektone-variant="original"/);
  });
  ...
  it('keeps marker comments inside the wrapper for JSX', () => {
    ...
    const joined = lines.join('\n');
    assert.match(joined, /^\s*<div data-tektone-variants="jsx12345"/m);
    assert.match(joined, /{\/\* tektone-variants-start jsx12345 \*\/}/);
    assert.doesNotMatch(joined, /data-tektone-variant="original"/);
  });
});
...
describe('live-insert CLI integration', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'tektone-insert-test-'));
  });
...
<body>
  ...
  <section class="features">
    ...
    assert.match(after, /data-tektone-mode="insert"/);
    assert.match(after, /tektone-variants-start ins12345/);
    assert.ok(after.indexOf('class="hero"') < after.indexOf('tektone-variants-start ins12345'));
    assert.ok(after.indexOf('tektone-variants-start ins12345') < after.indexOf('class="features"'));
    assert.doesNotMatch(after, /data-tektone-variant="original"/);
  });
  ...
  <section class="cta">CTA</section>
    ...
    assert.ok(after.indexOf('tektone-variants-start ins99999') < after.indexOf('class="cta"'));
    assert.ok(after.indexOf('class="hero"') < after.indexOf('tektone-variants-start ins99999'));
  });
  ...
  return (
    ...
    assert.match(after, /data-tektone-mode="insert"/);
    assert.match(after, /{\/\* tektone-variants-start jsxins01 \*\/}/);
  });
