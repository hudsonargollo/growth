 ...
 * exposes the accepted element under stale data-tektone runtime wrappers.
 *
 * Run with:
...
describe('live-e2e accept cleanup regression', () => {
  it('cleans accepted LLM DOM wrappers after carbonize cleanup', async (t) => {
    ...
    const llmConfig = resolveLlmAgentConfig({
      provider: process.env.TEKTONE_E2E_LLM_PROVIDER || 'deepseek',
    });
    ...
    try {
      ...
      assert.doesNotMatch(finalSource, /data-tektone-variants="/, 'source variants wrapper removed');
      assert.doesNotMatch(finalSource, /tektone-variants-start/, 'source variants markers removed');
      assert.doesNotMatch(finalSource, /tektone-carbonize-start/, 'source carbonize markers removed');
      assert.doesNotMatch(finalSource, /data-tektone-variant="/, 'source variant wrapper removed');
...
async function collectAcceptedDomDiagnostic(page, selector) {
  return page.evaluate((sel) => {
    const matches = [...document.querySelectorAll(sel)];
    const clean = matches.filter((el) => !el.closest('[data-tektone-variants],[data-tektone-variant],[data-tektone-carbonize]'));
    const stale = matches.filter((el) => el.closest('[data-tektone-variants],[data-tektone-variant],[data-tektone-carbonize]'));
    const local = {};
    for (const key of [
      'tektone-live-session',
      'tektone-live-session-handled',
      'tektone-live-session-scroll',
    ]) {
    ...
    return {
      ...
      remainingVariantsWrapperCount: document.querySelectorAll('[data-tektone-variants]').length,
      remainingVariantWrapperCount: document.querySelectorAll('[data-tektone-variant]').length,
      liveBarText: document.querySelector('#tektone-live-bar')?.textContent || '',
      localStorage: local,
      heroHtml: matches.map((el) => el.outerHTML.slice(0, 500)),
      staleAncestorHtml: stale.map((el) => {
        const ancestor = el.closest('[data-tektone-variants],[data-tektone-variant]');
...
function isSourceClean(source) {
  return !source.includes('data-tektone-variants=')
    && !source.includes('tektone-variants-start')
    && !source.includes('tektone-carbonize-start')
    && !source.includes('data-tektone-variant=');
}
