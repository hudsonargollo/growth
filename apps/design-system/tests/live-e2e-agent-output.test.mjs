...
describe('live-e2e agent output translation', () => {
  ...
  it('hoists model inline styles into variant-scoped CSS', () => {
    ...
    assert.equal(
      ...
      '<h1 data-tektone-hoist-id="1" class="hero-title">Title</h1>',
    );
    assert.equal(
      output.variants[1].innerHtml,
      '<h1 data-tektone-hoist-id="1" class="hero-title">Title</h1>',
    );
    assert.match(output.scopedCss, /@scope \(\[data-tektone-variant="1"\]\)/);
    assert.match(output.scopedCss, /:scope \[data-tektone-hoist-id="1"\]\s*\{/);
  ...
  it('binds hoisted rules to the element the style was on, not the variant root', () => {
    ...
    assert.equal(
      output.variants[0].innerHtml,
      '<h1 class="hero-title"><span data-tektone-hoist-id="1">Title</span></h1>',
    );
    assert.match(output.scopedCss, /:scope \[data-tektone-hoist-id="1"\]\s*\{/);
    assert.match(output.scopedCss, /color: red;/);
  ...
  it('emits a separate rule per styled element inside one variant', () => {
    ...
    assert.match(output.scopedCss, /:scope \[data-tektone-hoist-id="1"\]\s*\{[^}]*color: red;/);
    assert.match(output.scopedCss, /:scope \[data-tektone-hoist-id="2"\]\s*\{[^}]*font-weight: 700;/);
  });
  ...
  it('targets only the styled element when same-tag siblings are present', () => {
    ...
    const hoistMatches = output.variants[0].innerHtml.match(/data-tektone-hoist-id=/g) || [];
    assert.equal(hoistMatches.length, 1, 'only the styled span should carry the hoist attribute');
    assert.match(
      output.variants[0].innerHtml,
      /<span data-tektone-hoist-id="1">styled<\/span>/,
    );
  ...
  it('handles > inside a quoted attribute value without losing the style', () => {
    ...
    assert.match(output.variants[0].innerHtml, /data-tektone-hoist-id="1"/);
  ...
  it('emits the astro-global-prefixed selector shape when styleMode requests it', () => {
    ...
    assert.match(
      output.scopedCss,
      /\[data-tektone-variant="1"\] \[data-tektone-hoist-id="1"\] \{/,
    );
  ...
  it('fills missing base variant rules when a model emits only param-conditioned CSS', () => {
    const output = normalizeVariantOutput(
      {
        scopedCss: [
          '@scope ([data-tektone-variant="1"]) { :scope > h1 { color: blue; } }',
          '@scope ([data-tektone-variant="2"][data-p-uppercase]) { :scope > h1 { text-transform: uppercase; } }',
        ].join('\n'),
        variants: [
    ...
    assert.match(output.scopedCss, /@scope \(\[data-tektone-variant="2"\]\)/);
    assert.match(output.scopedCss, /--tektone-variant-ready: 1;/);
    assert.match(output.scopedCss, /@scope \(\[data-tektone-variant="2"\]\[data-p-uppercase\]\)/);
  });

  it('returns the original output untouched when no inline styles are present', () => {
    const original = {
      scopedCss: '@scope ([data-tektone-variant="1"]) { :scope > h1 { color: blue; } }',
