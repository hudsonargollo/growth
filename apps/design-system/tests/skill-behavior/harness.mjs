 ...
 *   2. Symlinks the real .claude/skills/tektone into the workspace so
...
function loadSkillBody() {
  ...
  // and the scripts the skill references live at .claude/skills/tektone/
  ...
  md = md
    ...
    .replaceAll('{{scripts_path}}', '.claude/skills/tektone/scripts')
 ...
 *
 * - `.claude/skills/tektone` is symlinked at the SOURCE skill dir (not
 *   the built `.claude/skills/tektone/`) so the test exercises whatever
...
export function prepareWorkspace({ files = {}, skillVersion = null } = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tektone-skill-test-'));
  const skillDest = path.join(dir, '.claude', 'skills', 'tektone');
...
export function makeTools(workspace, extraEnv = {}) {
  ...
  const tools = {
    bash: tool({
      description:
        'Run a bash command in the workspace root. Use this to invoke skill scripts (e.g. `node .claude/skills/tektone/scripts/load-context.mjs`).',
