...
const CRAFT_PROMPT = '/tektone craft a landing page for the project in this workspace';
const PRIMER_PROMPT =
  'Take a quick look at the project. What register is this? Run the tektone context loader once if you need to.';

const VERBOSE = process.env.TEKTONE_SKILL_BEHAVIOR_VERBOSE === '1';
...
for (const modelId of resolveModelList()) {
  ...
  describe(`skill behavior :: ${modelId}`, () => {
    ...
    it('scenario 4: context already loaded in prior turn', async () => {
      ...
      try {
        ...
        const turn2 = await runTurn({
          workspace,
          model,
          userPrompt: 'Now, /tektone craft a landing page based on what you saw.',
    ...
    it('scenario 6: sub-command routing (`/tektone polish` loads polish.md)', async () => {
      const workspace = prepareWorkspace({
        files: {
      ...
      try {
        const { trace, text } = await runTurn({
          workspace,
          model,
          userPrompt: '/tektone polish index.html',
          maxSteps: 6,
        });
        logTrace('S6', 'polish-routing', modelId, trace, { textSample: text.slice(0, 300) });
        assert.ok(
          fileLoaded(trace, 'polish.md'),
          `agent should load polish.md when /tektone polish is invoked.\n` +
            `Trace: ${JSON.stringify(summarizeTrace(trace), null, 2)}`,
        );
      } finally {
        cleanupWorkspace(workspace);
    ...
    it('scenario 7: sub-command routing (`/tektone audit` loads audit.md)', async () => {
      const workspace = prepareWorkspace({
        files: {
          'PRODUCT.md': PRODUCT_MD_SAMPLE,
          'DESIGN.md': DESIGN_MD_SAMPLE,
    ...
    it('scenario 8: existing SvelteKit project (agent explores design system)', async () => {
      ...
      try {
        const { trace, text } = await runTurn({
          workspace,
          model,
          userPrompt: '/tektone polish src/routes/+page.svelte',
          maxSteps: 8,
        });
    ...
    it('scenario 9: update-available directive is surfaced, never auto-run', async () => {
      ...
      // surface it and keep working, but must NOT run `npx tektone skills
      ...
      const workspace = prepareWorkspace({
        files: {
          'PRODUCT.md': PRODUCT_MD_SAMPLE,
          'index.html': MINIMAL_LANDING_HTML,
          '.tektone-update.json': JSON.stringify({ lastCheck: Date.now(), latestVersion: '99.0.0' }),
        },
        skillVersion: '3.5.0',
      });
      try {
        const { trace, text } = await runTurn({
          ...
          userPrompt: '/tektone polish index.html',
          maxSteps: 6,
          env: { TEKTONE_UPDATE_CACHE: path.join(workspace, '.tektone-update.json') },
        });
