...
Audit the code.`;
  ...
  test('should read skill with reference files', () => {
    const skillContent = `---
name: tektone
description: Tektone design skill
---

Tektone design instructions.`;
  ...
  test('should fall back to "tektone" when frontmatter has no name', () => {
    const skillDir = path.join(testRootDir, 'skill');
    ensureDir(skillDir);
    ...
    expect(skills).toHaveLength(1);
    expect(skills[0].name).toBe('tektone');
  });
...
describe('replacePlaceholders', () => {
  ...
  test('should exclude tektone from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['audit', 'tektone', 'polish']);
    expect(result).toBe('Commands: /audit, /polish');
  });

  test('should exclude legacy teach-tektone from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['audit', 'teach-tektone', 'polish']);
    expect(result).toBe('Commands: /audit, /polish');
  });

  test('lists /tektone sub-commands for {{available_commands}} when no command names are passed', () => {
    // v3.0 single-skill architecture: with no command names, the list falls back
    // to the TEKTONE_SUB_COMMANDS sub-commands rendered as `/tektone <sub>`.
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', []);
    expect(result.startsWith('Commands: /tektone ')).toBe(true);
    expect(result).toContain('/tektone audit');
    expect(result).toContain('/tektone polish');
  });
