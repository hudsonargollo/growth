...
// to iterate on. Set TEKTONE_E2E_ONLY=<name>[,<name>...] to scope the run.
const onlyNames = parseFixtureFilter(process.env.TEKTONE_E2E_ONLY);
...
if (missingOnlyNames.length > 0) {
  throw new Error(`Unknown TEKTONE_E2E_ONLY fixture(s): ${missingOnlyNames.join(', ')}`);
}

const manualOnly = process.env.TEKTONE_E2E_MANUAL_ONLY === '1'
  || process.env.TEKTONE_E2E_MANUAL_ONLY === 'true';
const reloadVariants = process.env.TEKTONE_E2E_RELOAD_VARIANTS === '1'
  || process.env.TEKTONE_E2E_RELOAD_VARIANTS === 'true';
const scenarioNames = parseFixtureFilter(process.env.TEKTONE_E2E_SCENARIOS);
const liveE2eTestTimeoutMs = readPositiveIntEnv('TEKTONE_E2E_TEST_TIMEOUT_MS');
...
for (const { name, fixture } of fixtures) {
  describe(`live-e2e · ${name} (${fixture.runtime.styling || 'unknown-styling'})`, () => {
    it('drives the full click → Go → cycle → accept cycle', liveE2eTestOptions, async (t) => {
      ...
      if (manualOnly || process.env.TEKTONE_E2E_MANUAL_SCENARIO) {
      ...
      // Pick the agent. `TEKTONE_E2E_AGENT=llm` opts into Claude first,
      ...
      const agentMode = process.env.TEKTONE_E2E_AGENT || 'fake';
      let agent;
      if (agentMode === 'llm') {
        const llmConfig = resolveLlmAgentConfig({
          model: process.env.TEKTONE_E2E_LLM_MODEL,
        });
        ...
        if (!agent) {
          t.skip(`TEKTONE_E2E_AGENT=llm with provider=${llmConfig.provider} requires ${llmConfig.requiredEnv}`);
          return;
        }
      ...
      const variantContentSelector = isInsert
        ? (usesSvelteComponentPreview ? '.inserted-copy' : '[data-tektone-variant="2"] .inserted-copy')
        : usesSvelteComponentPreview
        ? pickSelector
        : '[data-tektone-variant="2"] > :first-child';
      ...
      try {
        ...
        if (isInsert) {
          ...
          if (process.env.TEKTONE_E2E_DEBUG) {
            const barText = await page.evaluate(() => {
              const bar = document.querySelector('#tektone-live-bar');
        ...
        if (svelteComponentSession) {
          ...
          assert.doesNotMatch(routeBody, /data-tektone-variants="/, 'Svelte route source is not edited during generation');
        } else {
          assert.match(after, /data-tektone-variants="/, 'wrapper inserted');
        }
        if (isInsert) {
          if (svelteComponentSession) {
            assert.equal(svelteComponentSession.manifest.mode, 'insert', 'Svelte insert uses component preview mode');
          } else {
            assert.match(after, /data-tektone-mode="insert"/, 'insert mode wrapper');
            assert.doesNotMatch(after, /data-tektone-variant="original"/, 'insert has no original variant');
          }
        ...
        if (svelteComponentSession) {
          ...
          assert.match(after, /<style is:inline data-tektone-css="/, 'Astro live CSS uses an inline compiler-bypassing style block');
          assert.match(
            after,
            /\[data-tektone-variant="1"\]\s*>\s*(?:h1|\.[\w-]+)/,
          ...
          assert.doesNotMatch(after, /@scope \(\[data-tektone-variant="1"\]\)/, 'Astro live CSS does not use raw @scope');
        } else {
          assert.match(after, /<style data-tektone-css="/, 'colocated <style> block present');
          assert.match(after, /@scope \(\[data-tektone-variant="1"\]\)/, 'scoped CSS for variant 1');
          assert.match(after, /@scope \(\[data-tektone-variant="2"\]\)/, 'scoped CSS for variant 2');
          assert.match(after, /@scope \(\[data-tektone-variant="3"\]\)/, 'scoped CSS for variant 3');
        }
        ...
        if (agentMode === 'fake') {
          const paramsSource = svelteComponentSession
            ...
            : after;
          assert.match(paramsSource, svelteComponentSession ? /"1"\s*:/ : /data-tektone-params=/, 'params manifest emitted');
        ...
        for (const targetVariant of cycleSequence) {
          ...
          if (agentMode === 'fake' && targetVariant === 2 && !checkedVariantTwoStyle) {
            await page.waitForFunction((sel) => {
              const query = window.__tektoneLiveQuery || ((s) => document.querySelector(s));
            ...
            if (variantWeight !== '900') {
              const styleSnapshot = await evaluatePageWithTimeout(
                ...
                (sel) => {
                  const query = window.__tektoneLiveQuery || ((s) => document.querySelector(s));
                  const el = query(sel) || document.querySelector(sel);
                  const styleEl = document.querySelector('style[data-tektone-css]');
                  const rules = [];
        ...
        if (svelteComponentTarget) {
          ...
          const snapshotPath = join(tmp, '.tektone/live/sessions', `${svelteComponentTarget.manifest.id}.snapshot.json`);
          const snapshot = JSON.parse(readFileSync(snapshotPath, 'utf-8'));
          assert.equal(snapshot.phase, 'completed');
          assert.equal(snapshot.sourceFile, svelteComponentTarget.manifest.sourceFile);
          assert.doesNotMatch(snapshot.sourceFile, /node_modules\/\.tektone-live/);
        }
        assert.doesNotMatch(final, /data-tektone-variants="/,    'variants wrapper removed');
        assert.doesNotMatch(final, /tektone-variants-start/,      'variants-start marker removed');
        assert.doesNotMatch(final, /tektone-carbonize-start/,     'carbonize-start marker removed');
        assert.doesNotMatch(final, /tektone-carbonize-end/,       'carbonize-end marker removed');
        assert.doesNotMatch(final, /data-tektone-carbonize=/,     'carbonize wrapper removed');
        assert.doesNotMatch(final, /data-tektone-variant="/,      'no leftover variant scaffolding');
        if (isInsert) {
          if (agentMode === 'fake') {
    ...
    if (shouldRunScenario('manual') && Array.isArray(fixture.runtime.manualEditScenarios) && fixture.runtime.manualEditScenarios.length > 0) {
      const manualScenarioFilter = process.env.TEKTONE_E2E_MANUAL_SCENARIO || '';
    ...
    if (shouldRunScenario('annotations') && fixture.runtime.liveChrome?.annotations) {
      it('uploads annotations with generate and still accepts the variant', liveE2eTestOptions, async (t) => {
        ...
        try {
          ...
          assert.match(generateEvent.screenshotPath, /\.tektone\/live\/annotations\//, 'annotation screenshot is stored under live annotations');
...
async function captureLiveE2eFailure({ name, fixture, session, sourceFile, error, log = () => {} }) {
  const root = process.env.TEKTONE_E2E_ARTIFACT_DIR;
  ...
  try {
    ...
    copyDirIfExists(join(tmp, '.tektone', 'live'), join(dir, 'tektone-live'));
    copyDirIfExists(join(tmp, 'node_modules', '.tektone-live'), join(dir, 'tektone-live-preview'));
...
async function createManualScenarioAgent(t, scenario = {}) {
  const requested = (process.env.TEKTONE_E2E_MANUAL_AGENT || process.env.TEKTONE_E2E_AGENT || 'auto')
    .trim()
    .toLowerCase();
...
function maybeWrapMalformedAckProbe(agent, scenario, probeState, t) {
  ...
  return {
    ...
    async applyManualEdits(event, context = {}) {
      ...
      const buffer = JSON.parse(readFileSync(join(context.tmp, '.tektone/live/pending-manual-edits.json'), 'utf-8'));
...
async function waitForAcceptedSelectionReady(page, selector, { timeout }) {
  await page.waitForFunction(
    (sel) => {
      ...
      for (const el of all) {
        if (el.closest('[data-tektone-variants],[data-tektone-variant]')) return false;
      }
      return true;
    },
    selector,
...
async function readLiveSessionStorage(page) {
  return page.evaluate(() => {
    const raw = localStorage.getItem('tektone-live-session');
...
async function waitForVariantCounter(page, variant, count, { timeout = 15_000 } = {}) {
  try {
    await page.waitForFunction(
      ({ variant, count }) => {
        const query = window.__tektoneLiveQuery || ((sel) => document.querySelector(sel));
        const bar = query('#tektone-live-bar');
    ...
    const snapshot = await page.evaluate(() => {
      ...
      return {
        ...
        debugState: window.__TEKTONE_LIVE_CHROME_CORE__?.debugState?.() || null,
        storage: localStorage.getItem('tektone-live-session'),
        wrapper: wrapper ? { preview: wrapper.dataset.tektonePreview, count: wrapper.dataset.tektoneVariantCount, html: wrapper.outerHTML.slice(0, 500) } : null,
      };
...
async function waitForAcceptedDom(page, selector, { allowVariantRoot = false, timeout = 20_000 } = {}) {
  await page.waitForFunction(
    ({ sel, allowVariantRoot }) => {
      ...
      if (all.length < 1) return false;
      for (const el of all) {
        if (el.closest('[data-tektone-variants]')) return false;
        if (el.closest('[data-tektone-carbonize]')) return false;
        if (!allowVariantRoot && el.closest('[data-tektone-variant]')) return false;
      }
      return true;
    },
...
async function waitForSourceClean(filePath, timeoutMs, { svelteComponentTarget: knownSvelteTarget = null } = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    const dirty =
      ...
      last.includes('data-tektone-variants=') ||
      last.includes('tektone-variants-start') ||
      last.includes('tektone-carbonize-start') ||
      last.includes('data-tektone-carbonize=') ||
      last.includes('data-tektone-variant=');
...
function sourceShadowTargetFor(filePath) {
  ...
  if (!body.includes('data-tektone-preview="source-shadow"')) return null;
  const match = body.match(/\bdata-tektone-source-file=(["'])(.*?)\1/);
  if (!match) return null;
  const root = filePath.includes('/.tektone/')
    ? filePath.slice(0, filePath.indexOf('/.tektone/'))
    : dirname(filePath);
...
function svelteComponentTargetFor(filePath) {
  ...
  const markers = [
    `${sep}node_modules${sep}.tektone-live${sep}`,
    `${sep}src${sep}lib${sep}tektone${sep}`,
  ];
...
async function locateSessionFile(tmp) {
  ...
  for (const f of candidates) {
    ...
    if (
      body.includes('data-tektone-variants=') ||
      body.includes('tektone-carbonize-start') ||
      body.includes('tektone-variants-start')
    ) {
      return f;
    }
  }
...
function walkSvelteComponentManifests(root) {
  const results = [];
  const stack = [
    join(root, 'node_modules/.tektone-live'),
    join(root, 'src/lib/tektone'),
  ];
  while (stack.length) {
    const dir = stack.pop();
    let entries;
