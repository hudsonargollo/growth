...
describe('detectUrl — browser-only fixtures', () => {
  ...
  it('typography side-by-side: element-level flag cases get regular overlays', async () => {
    ...
    try {
      ...
      await page.evaluate(() => { window.__TEKTONE_CONFIG__ = { autoScan: false }; });
      await page.evaluate(browserScript);
      const result = await page.evaluate(() => {
        const groups = window.tektoneScan();
        ...
        return {
          ...
          hasBanner: Boolean(document.querySelector('.tektone-banner')),
          overlays: document.querySelectorAll('.tektone-overlay:not(.tektone-banner)').length,
        };
      });
  ...
  it('browser API: visual contrast fallback resolves readable image backgrounds without overlays', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        const before = document.querySelectorAll('.tektone-overlay, .tektone-label, .tektone-banner').length;
        const analyses = await window.tektoneAnalyzeVisualContrast({ maxCandidates: 20, scrollOffscreen: true });
        const after = document.querySelectorAll('.tektone-overlay, .tektone-label, .tektone-banner').length;
        return {
          before,
          after,
  ...
  it('browser API: visual contrast scan decorates visible findings without scrolling by default', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        const syncScanResult = window.tektoneScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const syncDetectResult = window.tektoneDetect({
          visualContrast: true,
          serialize: true,
        });
        const groups = await window.tektoneScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        return {
          ...
          overlays: document.querySelectorAll('.tektone-overlay:not(.tektone-banner)').length,
          labels: document.querySelectorAll('.tektone-label').length,
          analyses: window.tektoneGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
          scrollEvents,
          maxScrollY,
          finalScrollY: window.scrollY,
          syncScanIsArray: Array.isArray(syncScanResult),
          syncDetectIsArray: Array.isArray(syncDetectResult),
          hasAsyncApi: typeof window.tektoneScanAsync === 'function' && typeof window.tektoneDetectAsync === 'function',
        };
      });
      ...
      assert.equal(result.syncScanIsArray, true, `tektoneScan should keep a synchronous Array return: ${JSON.stringify(result)}`);
      assert.equal(result.syncDetectIsArray, true, `tektoneDetect should keep a synchronous Array return: ${JSON.stringify(result)}`);
      ...
      const refreshedOverlayResult = await page.evaluate(async () => {
        ...
        const initialGroups = window.tektoneScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        while (
          Date.now() < deadline &&
          !/low contrast/i.test(target?._tektoneOverlay?.textContent || '')
        ) {
          const nextButton = target?._tektoneOverlay?.querySelector('button:last-of-type');
          if (nextButton) nextButton.click();
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const labelVariants = [];
        const overlay = target?._tektoneOverlay;
        for (let i = 0; i < 3; i++) {
        ...
        return {
          ...
          labelText: target?._tektoneOverlay?.textContent || '',
          labelVariants,
          overlayConnected: Boolean(target?._tektoneOverlay?.isConnected),
        };
      });
      ...
      const lazyResult = await page.evaluate(async () => {
        ...
        return {
          ...
          labels: document.querySelectorAll('.tektone-label').length,
          analyses: window.tektoneGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
          targetHasOverlay: Boolean(target?._tektoneOverlay),
      ...
      const staleOverlayResult = await page.evaluate(async () => {
        ...
        window.scrollTo(0, 0);
        await new Promise(resolve => setTimeout(resolve, 50));
        await window.tektoneScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const staleCleared = !target?._tektoneOverlay;
        target?.scrollIntoView({ block: 'center', inline: 'nearest', behavior: 'instant' });
        await new Promise(resolve => setTimeout(resolve, 250));
        return {
          staleCleared,
          targetHasOverlay: Boolean(target?._tektoneOverlay),
          targetOverlayConnected: Boolean(target?._tektoneOverlay?.isConnected),
          overlays: document.querySelectorAll('.tektone-overlay:not(.tektone-banner)').length,
          analyses: window.tektoneGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
        };
      });
  ...
  it('extension mode remove cancels pending lazy visual contrast work', async () => {
    ...
    try {
      ...
      await page.evaluate(() => {
        ...
        window.__tektoneMessages = [];
        window.addEventListener('message', event => {
          if (event.source !== window || !event.data?.source?.startsWith('tektone-')) return;
          window.__tektoneMessages.push(event.data);
        });
      });
      await page.evaluate(browserScript);
      const result = await page.evaluate(async () => {
        ...
        while (
          ...
          !window.tektoneGetLastVisualContrastAnalyses()
            .some(item => item.status === 'unresolved' && item.reason === 'text outside viewport')
        ) {
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const unresolvedBeforeRemove = window.tektoneGetLastVisualContrastAnalyses()
        ...
        const resultsAfterRemove = window.__tektoneMessages
          .filter(message => message.source === 'tektone-results').length;
        return {
          unresolvedBeforeRemove,
          overlayCount: document.querySelectorAll('.tektone-overlay').length,
          targetHasOverlay: Boolean(target?._tektoneOverlay),
          resultsAfterRemove,
        };
      });
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          ...
          while (
            Date.now() < deadline &&
            !window.__tektoneMessages.some(message => message.source === 'tektone-error')
          ) {
            await new Promise(resolve => setTimeout(resolve, 25));
          }
          return {
            ready: window.__tektoneMessages.some(message => message.source === 'tektone-ready'),
            results: window.__tektoneMessages.some(message => message.source === 'tektone-results'),
            errors: window.__tektoneMessages
              .filter(message => message.source === 'tektone-error')
  ...
  it('extension mode echoes scan ids on result messages', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        while (
          Date.now() < deadline &&
          !window.__tektoneMessages.some(message =>
            message.source === 'tektone-results' &&
            message.scanId === 'scan-2'
          )
        ) {
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const resultMessage = window.__tektoneMessages.find(message =>
          message.source === 'tektone-results' &&
          message.scanId === 'scan-2'
        );
        return {
  ...
  it('browser API: tektoneDetect is pure, tektoneScan decorates', async () => {
    const puppeteer = await import('puppeteer');
    const browser = await puppeteer.default.launch({
      headless: true,
      args: process.env.CI ? ['--no-sandbox', '--disable-setuid-sandbox'] : [],
    ...
    try {
      ...
      const pure = await page.evaluate(() => {
        const before = document.querySelectorAll('.tektone-overlay, .tektone-label, .tektone-banner').length;
        const findings = window.tektoneDetect({ decorate: false, serialize: true });
        const after = document.querySelectorAll('.tektone-overlay, .tektone-label, .tektone-banner').length;
      ...
      const decorated = await page.evaluate(() => {
        const groups = window.tektoneScan();
        const overlays = document.querySelectorAll('.tektone-overlay, .tektone-label, .tektone-banner').length;
  ...
  it('browser API: async scan and detect reject instead of throwing synchronously', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          const scan = await window.tektoneScanAsync().then(
            () => ({ state: 'resolved' }),
            error => ({ state: 'rejected', message: error?.message || String(error) }),
          );
          const detect = await window.tektoneDetectAsync().then(
            () => ({ state: 'resolved' }),
            error => ({ state: 'rejected', message: error?.message || String(error) }),
          );
          return { scan, detect };
