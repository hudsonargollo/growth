...
describe('live-commit-manual-edits.mjs batched AI apply', () => {
  ...
  it('rolls back when a failed multi-op entry leaves one op in source', () => {
    ...
    const result = runCommit([], {
      TEKTONE_LIVE_COPY_AGENT_MOCK_WRITES: JSON.stringify({
        'src/page.html': [
  ...
  it('deduplicates failed entries across repeated repair attempts', async () => {
    ...
    const result = await commitManualEdits({
      cwd: tmpDir,
      provider: 'chat',
      env: { TEKTONE_LIVE_MANUAL_EDIT_REPAIR_ATTEMPTS: '2' },
  ...
  it('verifies current hero edits against Astro source hints before clearing', () => {
    ...
    const originalHook = "Great design prompts require design vocabulary. Most people don't have it. Tektone teaches your AI deep design knowledge and gives you 23 commands to steer the result.";
    ...
    writeAstro({ title: 'Tektone', hook: originalHook });
    writeBuffer(tmpDir, {
      entries: [
        entry({
          id: 'hero-title',
    ...
    writeAstro({ title: 'Tektone Wow', hook: 'YESSSSS' });
  ...
  it('rolls back source writes when the AI runner throws after writing', () => {
    ...
    const result = runCommit([], {
      ...
      TEKTONE_LIVE_COPY_AGENT_MOCK_RESULT: '{not json',
    });

    assert.equal(result.cleared, 0);
    assert.equal(result.applied.length, 0);
    assert.match(result.failed[0].reason, /Invalid TEKTONE_LIVE_COPY_AGENT_MOCK_RESULT JSON/);
    assert.deepEqual(result.rolledBackFiles, ['src/page.html']);
  ...
  it('reports a corrupt pending buffer instead of treating it as empty', () => {
    const bufferPath = path.join(tmpDir, '.tektone', 'live', 'pending-manual-edits.json');
  ...
  it('allows non-numeric count display copy when integer source data stays typed', () => {
    fs.writeFileSync(path.join(tmpDir, 'package.json'), JSON.stringify({
      scripts: {
        'tektone:manual-edit-validate': 'node src/validate.mjs',
      },
    }, null, 2));
    fs.writeFileSync(
  ...
  it('strips live edit runtime attributes from the copy-edit prompt context', () => {
    const decoratedHtml = '<section><h1 class="hero" contenteditable="true" data-tektone-editable="true" data-tektone-original-text="Original" style="user-select: text; cursor: text; outline: none; -webkit-user-modify: read-write-plaintext-only;">Edited</h1></section>';
    ...
    assert.doesNotMatch(serializedBatch, /data-tektone-original-text|data-tektone-editable|contenteditable|-webkit-user-modify/);
    assert.match(serializedBatch, /Edited/);
  });

  it('fails post-apply validation when live edit runtime attributes leak into source', () => {
    fs.writeFileSync(path.join(tmpDir, 'src', 'page.astro'), '<h1 class="hero" data-tektone-original-text="Original">Edited</h1>\n');

    const checks = runCopyEditPostApplyChecks({ cwd: tmpDir, files: ['src/page.astro'] });

    assert.equal(checks.ok, false);
    assert.equal(checks.failures[0].reason, 'leftover_tektone_marker');
    assert.match(checks.failures[0].marker, /data-tektone-original-text/);
  });
  ...
  it('keeps source for repair when Apply leaks edit-mode contenteditable attributes into source', () => {
    ...
    const result = runCommit([], {
      TEKTONE_LIVE_COPY_AGENT_MOCK_WRITES: JSON.stringify({
        'src/App.jsx': 'export default function App() { return <h1 className="hero" contenteditable="true" data-tektone-editable="true" data-tektone-original-text="Old">Old New</h1>; }\n',
      }),
      TEKTONE_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
        status: 'done',
    ...
    assert.equal(result.reason, 'manual_edit_repair_needs_decision');
    assert.equal(result.failed[0].reason, 'post_apply_validation_failed');
    assert.equal(result.failed[0].checks.some((check) => /data-tektone-(editable|original-text)/.test(check.marker)), true);
    assert.equal(result.rolledBackFiles, undefined);
  ...
  it('keeps source for repair when Apply leaves carbonize or variant scaffolding in source', () => {
    ...
    const result = runCommit([], {
      TEKTONE_LIVE_COPY_AGENT_MOCK_WRITES: JSON.stringify({
        'src/page.html': [
          '<!-- tektone-carbonize-start deadbeef -->',
          '<div data-tektone-variants="deadbeef" data-tektone-variant-count="3">',
          '  <div data-tektone-variant="original"><h1 class="hero">New</h1></div>',
          '</div>',
          '<!-- tektone-carbonize-end deadbeef -->',
          '',
        ].join('\n'),
      }),
      TEKTONE_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
    ...
    assert.equal(result.failed[0].reason, 'post_apply_validation_failed');
    assert.equal(
      result.failed[0].checks.some((check) => /tektone-carbonize-start|data-tektone-variant/.test(check.marker)),
      true,
    );
    assert.equal(result.rolledBackFiles, undefined);
    assert.match(fs.readFileSync(file, 'utf-8'), /tektone-carbonize-start/);
  ...
  it('keeps the whole batch for repair when one applied edit leaves UI scaffolding behind', () => {
    ...
    const result = runCommit([], {
      TEKTONE_LIVE_COPY_AGENT_MOCK_WRITES: JSON.stringify({
        ...
        'src/card.html': '<article class="card"><span data-tektone-text-wrap="true">New card</span></article>\n',
      }),
      TEKTONE_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
        status: 'done',
    ...
    assert.equal(result.failed[0].checks.some((check) => /data-tektone-text-wrap/.test(check.marker)), true);
    assert.equal(result.rolledBackFiles, undefined);
    assert.equal(fs.readFileSync(pageFile, 'utf-8'), '<h1 class="hero">New title</h1>\n');
    assert.match(fs.readFileSync(cardFile, 'utf-8'), /data-tektone-text-wrap/);
