 ...
 *   3. TEKTONE_CONTEXT_DIR env var as a power-user escape hatch (only
...
beforeEach(() => {
  scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'tektone-loadctx-'));
  savedEnv = process.env.TEKTONE_CONTEXT_DIR;
  delete process.env.TEKTONE_CONTEXT_DIR;
});

afterEach(() => {
  if (savedEnv === undefined) delete process.env.TEKTONE_CONTEXT_DIR;
  else process.env.TEKTONE_CONTEXT_DIR = savedEnv;
...
describe('resolveContextDir', () => {
  ...
  it('uses TEKTONE_CONTEXT_DIR as a fallback when defaults are empty (relative path)', () => {
    write('design/PRODUCT.md');
    process.env.TEKTONE_CONTEXT_DIR = 'design';
    assert.equal(resolveContextDir(scratch), path.join(scratch, 'design'));
  });

  it('uses TEKTONE_CONTEXT_DIR as a fallback when defaults are empty (absolute path)', () => {
    const elsewhere = fs.mkdtempSync(path.join(os.tmpdir(), 'tektone-elsewhere-'));
    try {
      process.env.TEKTONE_CONTEXT_DIR = elsewhere;
  ...
  it('default paths win over TEKTONE_CONTEXT_DIR (lazy escape hatch)', () => {
  ...
  it('ignores empty TEKTONE_CONTEXT_DIR', () => {
    write('PRODUCT.md');
    process.env.TEKTONE_CONTEXT_DIR = '   ';
    assert.equal(resolveContextDir(scratch), scratch);
  });
...
describe('loadContext (TEKTONE_CONTEXT_DIR escape hatch)', () => {
  ...
  it('reports a missing override directory as no-context, not as a crash', () => {
    process.env.TEKTONE_CONTEXT_DIR = 'no/such/dir';
    const ctx = loadContext(scratch);
...
describe('context.mjs CLI', () => {
  it('emits NO_PRODUCT_MD directive when no PRODUCT.md is found', async () => {
    ...
    const res = spawnSync(process.execPath, [SCRIPT_PATH], { cwd: scratch, encoding: 'utf8', env: { ...process.env, TEKTONE_NO_UPDATE_CHECK: '1' } });
...
describe('context.mjs update check', () => {
  ...
  function setup(cacheObj, { disable = false, host } = {}) {
    ...
    const env = {
      ...process.env,
      TEKTONE_UPDATE_CACHE: cachePath(),
      TEKTONE_NO_UPDATE_CHECK: disable ? '1' : '',
      ...(host ? { TEKTONE_UPDATE_HOST: host } : {}),
    };
  ...
  it('appends UPDATE_AVAILABLE when the cached latest version is newer', () => {
    ...
    assert.equal(res.status, 0);
    assert.match(res.stdout, /UPDATE_AVAILABLE: A newer Tektone skill is available/);
    assert.match(res.stdout, /installed v1\.0\.0, latest v2\.0\.0/);
    assert.match(res.stdout, /npx tektone skills update/);
  ...
  it('respects TEKTONE_NO_UPDATE_CHECK', () => {
