...
function loadFactory() {
  ...
  return context.__TEKTONE_LIVE_SESSION__.createLiveBrowserSessionState;
}
...
describe('live-browser-session state helper', () => {
  it('persists session, handled state, and scroll independently', () => {
    ...
    const state = createState({ prefix: 'tektone-live', storage, idFactory: () => 'owner-a' });
  ...
  it('carries checkpoint revision across reload-equivalent helper instances', () => {
    const createState = loadFactory();
    const storage = createMemoryStorage();
    const first = createState({ prefix: 'tektone-live', storage, idFactory: () => 'owner-a' });
    ...
    const second = createState({ prefix: 'tektone-live', storage, idFactory: () => 'owner-b' });
