...
function stageFixture(name) {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'tektone-fixture-'));
  cpSync(join(fixtureRoot, 'files'), tmp, { recursive: true });
  writeFileSync(join(tmp, '.gitignore'), gitignore);
  mkdirSync(join(tmp, '.tektone', 'live'), { recursive: true });
  writeFileSync(join(tmp, '.tektone', 'live', 'config.json'), JSON.stringify(fixture.config));
    ...
    it('live-inject --port adds the script tag to every config file', () => {
      ...
      try {
        ...
        const ignored = execFileSync('git', [
          'check-ignore',
          '.tektone/live/server.json',
          '.tektone/live/sessions/example.jsonl',
          '.tektone/live/previews/example/v1.html',
          '.tektone/live/deferred-svelte-component-accepts.json',
          'src/lib/tektone/TektoneLiveRoot.svelte',
          'src/lib/tektone/__runtime.js',
          'src/lib/tektone/a4ac4e74/v3.svelte',
        ], { cwd: tmp, encoding: 'utf-8' });
        assert.match(ignored, /\.tektone\/live\/server\.json/);
        assert.match(ignored, /\.tektone\/live\/sessions\/example\.jsonl/);
        assert.match(ignored, /\.tektone\/live\/previews\/example\/v1\.html/);
        assert.match(ignored, /\.tektone\/live\/deferred-svelte-component-accepts\.json/);
        assert.match(ignored, /src\/lib\/tektone\/TektoneLiveRoot\.svelte/);
        assert.match(ignored, /src\/lib\/tektone\/__runtime\.js/);
        assert.match(ignored, /src\/lib\/tektone\/a4ac4e74\/v3\.svelte/);
        if (result.adapter === 'sveltekit') {
          ...
          const root = readFileSync(join(tmp, 'src/lib/tektone/TektoneLiveRoot.svelte'), 'utf-8');
          assert.match(layout, /tektone-live-svelte-start/, 'SvelteKit layout got the adapter marker');
          assert.match(layout, /TektoneLiveRoot/, 'SvelteKit layout renders the adapter host');
          assert.doesNotMatch(appHtml, /tektone-live-start/, 'SvelteKit app.html must remain untouched');
        ...
        for (const r of result.results) {
          ...
          assert.match(body, /tektone-live-start/);
    ...
    it('live-inject --remove strips the script tag cleanly', () => {
      ...
      try {
        ...
        if (result.adapter === 'sveltekit') {
          const layout = readFileSync(join(tmp, 'src/routes/+layout.svelte'), 'utf-8');
          const appHtml = readFileSync(join(tmp, 'src/app.html'), 'utf-8');
          assert.doesNotMatch(layout, /TektoneLiveRoot/);
          assert.doesNotMatch(layout, /tektone-live-svelte-start/);
          assert.doesNotMatch(appHtml, /tektone-live-start/);
          assert.equal(existsSync(join(tmp, 'src/lib/tektone/TektoneLiveRoot.svelte')), false);
          return;
        }
        for (const r of result.results) {
          const body = readFileSync(join(tmp, r.file), 'utf-8');
          assert.doesNotMatch(body, /tektone-live-start/);
