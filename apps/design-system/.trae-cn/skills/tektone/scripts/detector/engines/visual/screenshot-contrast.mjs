...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `tektone-contrast-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const applied = await page.evaluate(({ selector, token, backgroundClipText }) => {
    ...
    if (!el) return false;
    let style = document.getElementById('tektone-visual-contrast-hide-style');
    if (!style) {
      style = document.createElement('style');
      style.id = 'tektone-visual-contrast-hide-style';
      style.textContent = [
        '[data-tektone-visual-contrast-target] {',
        '  color: transparent !important;',
        '  -webkit-text-fill-color: transparent !important;',
        '  text-shadow: none !important;',
        '}',
        '[data-tektone-visual-contrast-target][data-tektone-bgclip-text="true"] {',
        '  background-image: none !important;',
        '}',
      ].join('\n');
      document.head.appendChild(style);
    }
    el.setAttribute('data-tektone-visual-contrast-target', token);
    if (backgroundClipText) el.setAttribute('data-tektone-bgclip-text', 'true');
    return true;
  }, {
  ...
  try {
    ...
    await page.evaluate(({ selector }) => {
      try {
        ...
        if (el) {
          el.removeAttribute('data-tektone-visual-contrast-target');
          el.removeAttribute('data-tektone-bgclip-text');
        }
      } catch {
